...
pub extern "C" fn sqlite3_vtab_unlock(p_v_tab_1: *mut VTable) -> () {
    unsafe {
        ...
        if unsafe { (*p_v_tab_1).n_ref } == 0 {
            let p: *mut Sqlite3Vtab = unsafe { (*p_v_tab_1).p_vtab };
            if !(p).is_null() {
                unsafe {
                    (unsafe {
    ...
    unsafe {
        ...
        (*db).a_v_trans = core::ptr::null_mut();
        {
            ...
            '__b8: loop {
                ...
                '__c8: loop {
                    let mut x:
                            Option<unsafe extern "C" fn(*mut Sqlite3Vtab) -> i32> =
                        None;
            ...
            unsafe { (*db).a_v_trans = core::ptr::null_mut() };
            {
                ...
                '__b9: loop {
                    ...
                    '__c9: loop {
                        ...
                        let p: *mut Sqlite3Vtab = unsafe { (*p_v_tab).p_vtab };
                        if !(p).is_null() {
                            let mut x:
                                    Option<unsafe extern "C" fn(*mut Sqlite3Vtab) -> i32> =
                                None;
                                ...
                                Some(unsafe {
                                        *unsafe {
                                                core::mem::transmute::<*const (),
                                                        *mut unsafe extern "C" fn(*mut Sqlite3Vtab)
                                                            ->
                                                                i32>(unsafe {
...
pub extern "C" fn sqlite3_vtab_rollback(db: *mut Sqlite3) -> i32 {
    call_finaliser(db,
        core::mem::offset_of!(Sqlite3Module, x_rollback) as i32);
    return 0;
}
...
extern "C" fn vtab_call_constructor(db: *mut Sqlite3, p_tab_1: *mut Table,
    ...
    x_construct_1:
        Option<unsafe extern "C" fn(*mut Sqlite3, *mut (), i32,
            *const *const i8, *mut *mut Sqlite3Vtab, *mut *mut i8) -> i32>,
...
pub extern "C" fn sqlite3_vtab_eponymous_table_init(p_parse_1: *mut Parse,
    ...
    unsafe {
        let p_module: *const Sqlite3Module = unsafe { (*p_mod_1).p_module };
        let mut p_tab: *mut Table = core::ptr::null_mut();
        let mut z_err: *mut i8 = core::ptr::null_mut();
        let mut rc: i32 = 0;
        let db: *mut Sqlite3 = unsafe { (*p_parse_1).db };
...
extern "C" fn grow_v_trans(db: *mut Sqlite3) -> i32 {
    ...
    if unsafe { (*db).n_v_trans } % array_incr as i32 == 0 {
        ...
        let n_bytes: Sqlite3Int64 =
            (core::mem::size_of::<*mut Sqlite3Vtab>() as u64 *
                    (unsafe { (*db).n_v_trans } as Sqlite3Int64 +
                            array_incr as Sqlite3Int64) as u64) as Sqlite3Int64;
        a_v_trans =
            unsafe {
                    sqlite3_db_realloc(db,
...
pub extern "C" fn sqlite3_vtab_begin(db: *mut Sqlite3, p_v_tab_1: *mut VTable)
    ...
    unsafe {
        let mut rc: i32 = 0;
        let mut p_module: *const Sqlite3Module = core::ptr::null();
