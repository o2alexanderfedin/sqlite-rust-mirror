...
extern "C" fn exec_callback(p_not_used_1: *mut (), n_col_1: i32,
    ...
    unsafe {
        let mut i: i32 = 0;
        let z_who: *const i8 = who_am_i();
        {
            i = 0;
            '__b0: loop {
                if !(i < n_col_1) { break '__b0; }
                '__c0: loop {
                    let mut z_val: *const i8 =
                        unsafe { *a_val_1.offset(i as isize) } as *const i8;
                    let mut z_col: *const i8 =
                        unsafe { *a_col_1.offset(i as isize) } as *const i8;
                    if z_val == core::ptr::null() {
                        z_val = c"NULL".as_ptr() as *mut i8 as *const i8;
                    }
                    if z_col == core::ptr::null() {
                        z_col = c"NULL".as_ptr() as *mut i8 as *const i8;
                    }
                    unsafe {
...
extern "C" fn sql_exec(db: *mut sqlite3, z_sql_1: *const i8,
    ...
    unsafe {
        ...
        unsafe {
            printf(c"%s: %s\n".as_ptr() as *mut i8 as *const i8, who_am_i(),
                z_sql_1)
        };
        unsafe { fflush(__stdoutp) };
        rc =
            unsafe {
                sqlite3_exec(db, z_sql_1,
                    Some(if b_callback_1 != 0 {
                            exec_callback
                        } else {
                            unsafe {
        ...
        if rc != 0 || z_err != core::ptr::null_mut() {
            unsafe {
                printf(c"%s: %s: rc=%d: %s\n".as_ptr() as *mut i8 as
                        *const i8, who_am_i(), z_sql_1, rc, z_err)
            };
            unsafe { exit(1) };
        }
    }
...
extern "C" fn vfs_trace_callback(z_msg_1: *const i8, p_not_used_1: *mut ())
    ...
    unsafe {
        unsafe {
            printf(c"%s: %s".as_ptr() as *mut i8 as *const i8, who_am_i(),
                z_msg_1)
        };
        unsafe { fflush(__stdoutp) };
        return 0;
...
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    unsafe {
        ...
        let mut rc: i32 = 0;
        let mut i: i32 = 0;
        let mut use_wal: i32 = 0;
        let mut z_filename: *const i8 = core::ptr::null();
        let mut child: pid_t = 0;
        let mut c2: pid_t = 0;
        let mut status: i32 = 0;
        let mut b_commit_before_fork: i32 = 0;
        let mut n_delay_after4: i32 = 0;
        {
            i = 1;
            '__b1: loop {
                ...
                '__c1: loop {
                    ...
                    if unsafe {
                        ...
                        use_wal = 1;
                                ...
                                strcmp(z,
                                    c"-commit-before-fork".as_ptr() as *mut i8 as *const i8)
                            } == 0 {
                        b_commit_before_fork = 1;
                    } else if unsafe {
                                    strcmp(z,
                                        c"-delay-after-4".as_ptr() as *mut i8 as *const i8)
                                } == 0 && i + 1 < argc {
                        { let __p = &mut i; let __t = *__p; *__p += 1; __t };
                        n_delay_after4 =
                            unsafe {
                                ...
                                strcmp(z, c"-vfstrace".as_ptr() as *mut i8 as *const i8)
                            } == 0 {
                        unsafe {
                            vfstrace_register(c"vfstrace".as_ptr() as *mut i8 as
                                    *const i8, core::ptr::null(), Some(vfs_trace_callback),
                                core::ptr::null_mut(), 1)
                        };
                        ...
                        unsafe { exit(1) };
                    } else if z_filename != core::ptr::null() {
                        unsafe {
        ...
        rc = unsafe { sqlite3_open(z_filename, &mut db) };
        if rc != 0 {
            unsafe {
                printf(c"sqlite3_open() returns %d\n".as_ptr() as *mut i8 as
                        *const i8, rc)
            };
            unsafe { exit(1) };
        }
        ...
        sql_exec(db, c"CREATE TABLE t1(x);".as_ptr() as *mut i8 as *const i8,
            0);
        sql_exec(db,
            c"INSERT INTO t1 VALUES(\'First row\');".as_ptr() as *mut i8 as
                *const i8, 0);
        ...
        sql_exec(db,
            c"INSERT INTO t1 VALUES(\'Second row\');".as_ptr() as *mut i8 as
                *const i8, 0);
        sql_exec(db, c"SELECT x FROM t1;".as_ptr() as *mut i8 as *const i8,
            1);
        ...
        if child != 0 {
            unsafe {
                printf(c"Parent = %d\nChild = %d\n".as_ptr() as *mut i8 as
                        *const i8, unsafe { getpid() }, child)
            };
        }
        if child == 0 {
            let mut k: i32 = 0;
            unsafe { printf(c"Step 4:\n".as_ptr() as *mut i8 as *const i8) };
            unsafe { fflush(__stdoutp) };
            {
                k = 0;
                '__b2: loop {
                    ...
                    '__c2: loop {
                        let z_db_name: *const i8 =
                            unsafe { sqlite3_db_name(db, k) };
                        let mut p_jrnl: *mut sqlite3_file = core::ptr::null_mut();
                        if k == 1 { break '__c2; }
                        if z_db_name == core::ptr::null() { break '__b2; }
                        unsafe {
                            sqlite3_file_control(db, z_db_name, 43,
                                core::ptr::null_mut())
                        };
                        unsafe {
                            sqlite3_file_control(db, z_db_name, 28,
                                &raw mut p_jrnl as *mut ())
                        };
                        if !(p_jrnl).is_null() &&
                                    !(unsafe { (*p_jrnl).p_methods }).is_null() &&
                                unsafe {
                                    (*unsafe { (*p_jrnl).p_methods }).x_file_control.is_some()
                                } {
                            unsafe {
                                (unsafe {
                                        (*unsafe { (*p_jrnl).p_methods }).x_file_control.unwrap()
                                    })(p_jrnl, 43, core::ptr::null_mut())
                            };
                        }
                        break '__c2;
                    }
        ...
        if n_delay_after4 > 0 {
            unsafe {
                printf(c"%s: Delay for %d seconds\n".as_ptr() as *mut i8 as
                        *const i8, who_am_i(), n_delay_after4)
            };
            unsafe { fflush(__stdoutp) };
            unsafe { sleep(n_delay_after4 as u32) };
            unsafe {
                printf(c"%s: Continue after %d delay\n".as_ptr() as *mut i8 as
                        *const i8, who_am_i(), n_delay_after4)
            };
            unsafe { fflush(__stdoutp) };
        }
        if child != 0 {
            unsafe { printf(c"Step 5:\n".as_ptr() as *mut i8 as *const i8) };
            if (b_commit_before_fork == 0) as i32 != 0 {
                sql_exec(db, c"COMMIT".as_ptr() as *mut i8 as *const i8, 0);
            }
            sql_exec(db,
                c"SELECT x FROM t1;".as_ptr() as *mut i8 as *const i8, 1);
        }
        if child == 0 {
            unsafe { sleep(2 as u32) };
            unsafe {
                printf(c"Steps 7 and 8:\n".as_ptr() as *mut i8 as *const i8)
            };
            rc = unsafe { sqlite3_open(z_filename, &mut db) };
            if rc != 0 {
                unsafe {
                    printf(c"Child unable to reopen the database.  rc = %d\n".as_ptr()
        ...
        c2 = unsafe { wait(&mut status) };
        unsafe {
            printf(c"Process %d finished with status %d\n".as_ptr() as *mut i8
                    as *const i8, c2, status)
        };
        if child != 0 {
            unsafe { printf(c"Step 9:\n".as_ptr() as *mut i8 as *const i8) };
            sql_exec(db,
                c"SELECT * FROM t1;".as_ptr() as *mut i8 as *const i8, 1);
        }
        return Ok(());
...
extern "C" {
    ...
    fn exit(_: i32)
    -> ();
    fn vfstrace_register(z_trace_name_1: *const i8,
    z_old_vfs_name_1: *const i8,
    x_out_1: Option<unsafe extern "C" fn(*const i8, *mut ()) -> i32>,
    p_out_arg_1: *mut (), make_default_1: i32)
    -> i32;
