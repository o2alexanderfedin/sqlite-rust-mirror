...
struct MemStore {
    sz: sqlite3_int64,
    sz_alloc: sqlite3_int64,
    sz_max: sqlite3_int64,
    a_data: *mut u8,
...
extern "C" fn memdb_close(p_file: *mut sqlite3_file) -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        if !(unsafe { (*p).z_f_name }).is_null() {
            let mut i: i32 = 0;
            let p_vfs_mutex: *mut sqlite3_mutex =
                unsafe { sqlite3MutexAlloc(11) };
            unsafe { sqlite3_mutex_enter(p_vfs_mutex) };
            {
                i = 0;
                '__b0: loop {
                    if !(i < memdb_g.n_mem_store) { break '__b0; }
                    '__c0: loop {
                        if unsafe { *memdb_g.ap_mem_store.offset(i as isize) } == p
                            {
                            memdb_enter(unsafe { &*p });
                            if unsafe { (*p).n_ref } == 1 {
                                unsafe {
                                    *memdb_g.ap_mem_store.offset(i as isize) =
                                        unsafe {
                                            *memdb_g.ap_mem_store.offset({
                                                            let __p = &mut memdb_g.n_mem_store;
                                                            *__p -= 1;
                                                            *__p
                                                        } as isize)
                                        }
                                };
                                if memdb_g.n_mem_store == 0 {
                                    unsafe { sqlite3_free(memdb_g.ap_mem_store as *mut ()) };
                                    memdb_g.ap_mem_store = core::ptr::null_mut();
                                }
                            }
                            break '__b0;
                        }
            ...
            unsafe { sqlite3_mutex_leave(p_vfs_mutex) };
        } else { memdb_enter(unsafe { &*p }); }
        {
        ...
        if unsafe { (*p).n_ref } <= 0 {
            if unsafe { (*p).m_flags } & 1 as u32 != 0 {
                unsafe { sqlite3_free(unsafe { (*p).a_data } as *mut ()) };
            }
            memdb_leave(unsafe { &*p });
            unsafe { sqlite3_mutex_free(unsafe { (*p).p_mutex }) };
            unsafe { sqlite3_free(p as *mut ()) };
        } else { memdb_leave(unsafe { &*p }); }
        return 0;
    }
}
extern "C" fn memdb_read(p_file: *mut sqlite3_file, z_buf: *mut (),
    i_amt: i32, i_ofst: sqlite3_int64) -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        memdb_enter(unsafe { &*p });
        if i_ofst + i_amt as sqlite_int64 > unsafe { (*p).sz } {
            unsafe { memset(z_buf, 0, i_amt as u64) };
            if i_ofst < unsafe { (*p).sz } {
                unsafe {
                    memcpy(z_buf,
                        unsafe { unsafe { (*p).a_data.offset(i_ofst as isize) } } as
                            *const (), (unsafe { (*p).sz } - i_ofst) as u64)
                };
            }
            memdb_leave(unsafe { &*p });
            return 10 | 2 << 8;
...
extern "C" fn memdb_enlarge(p: &mut MemStore, mut new_sz_1: sqlite3_int64)
    -> i32 {
    let mut p_new: *mut u8 = core::ptr::null_mut();
    if (*p).m_flags & 2 as u32 == 0 as u32 || (*p).n_mmap > 0 { return 13; }
    if new_sz_1 > (*p).sz_max { return 13; }
    new_sz_1 *= 2 as sqlite3_int64;
    if new_sz_1 > (*p).sz_max { new_sz_1 = (*p).sz_max; }
    p_new =
        unsafe { sqlite3Realloc((*p).a_data as *mut (), new_sz_1 as u64) } as
            *mut u8;
    if p_new == core::ptr::null_mut() { return 10 | 12 << 8; }
    (*p).a_data = p_new;
    (*p).sz_alloc = new_sz_1;
    return 0;
}
extern "C" fn memdb_write(p_file: *mut sqlite3_file, z: *const (), i_amt: i32,
    ...
    unsafe {
        ...
        if i_ofst + i_amt as sqlite_int64 > unsafe { (*p).sz } {
            let mut rc: i32 = 0;
            if i_ofst + i_amt as sqlite_int64 > unsafe { (*p).sz_alloc } &&
                    {
                            rc =
                                memdb_enlarge(unsafe { &mut *p },
            ...
            if i_ofst > unsafe { (*p).sz } {
                unsafe {
                    memset(unsafe {
                                unsafe { (*p).a_data.offset(unsafe { (*p).sz } as isize) }
                            } as *mut (), 0, (i_ofst - unsafe { (*p).sz }) as u64)
                };
            }
            unsafe { (*p).sz = i_ofst + i_amt as sqlite_int64 };
        }
        unsafe {
            memcpy(unsafe { unsafe { (*p).a_data.offset(i_ofst as isize) } }
                    as *mut (), z, i_amt as u64)
        };
        memdb_leave(unsafe { &*p });
        return 0;
    }
}
extern "C" fn memdb_truncate(p_file: *mut sqlite3_file, size: sqlite3_int64)
    -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        let mut rc: i32 = 0;
...
extern "C" fn memdb_sync(p_file: *mut sqlite3_file, flags: i32) -> i32 {
    { let _ = p_file; };
    { let _ = flags; };
    return 0;
}
extern "C" fn memdb_file_size(p_file: *mut sqlite3_file,
    p_size: *mut sqlite3_int64) -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        memdb_enter(unsafe { &*p });
        unsafe { *p_size = unsafe { (*p).sz } };
        memdb_leave(unsafe { &*p });
        return 0;
    }
}
extern "C" fn memdb_lock(p_file: *mut sqlite3_file, e_lock: i32) -> i32 {
    let p_this: *mut MemFile = p_file as *mut MemFile;
    let p: *mut MemStore = unsafe { (*p_this).p_store };
    let mut rc: i32 = 0;
    if e_lock <= unsafe { (*p_this).e_lock } { return 0; }
    memdb_enter(unsafe { &*p });
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    if e_lock > 1 && unsafe { (*p).m_flags } & 4 as u32 != 0 {
        rc = 8;
    } else {
        '__s1:
            {
            match e_lock {
                1 => {
                    {
                        { let _ = 0; };
                        if unsafe { (*p).n_wr_lock } > 0 {
                            rc = 5;
                        } else {
                            {
                                let __p = unsafe { &mut (*p).n_rd_lock };
                                let __t = *__p;
                                *__p += 1;
                                __t
                            };
                    ...
                    {
                        { let _ = 0; };
                        if unsafe { (*p_this).e_lock } == 1 {
                            if unsafe { (*p).n_wr_lock } > 0 {
                                rc = 5;
                            } else { unsafe { (*p).n_wr_lock = 1 }; }
                        }
                        break '__s1;
                    }
                    {
                        { let _ = 0; };
                        { let _ = 0; };
                        if unsafe { (*p).n_rd_lock } > 1 {
                            rc = 5;
                        } else if unsafe { (*p_this).e_lock } == 1 {
                            unsafe { (*p).n_wr_lock = 1 };
                        }
                        break '__s1;
                    }
                }
    ...
    if rc == 0 { unsafe { (*p_this).e_lock = e_lock }; }
    memdb_leave(unsafe { &*p });
    return rc;
}
extern "C" fn memdb_unlock(p_file: *mut sqlite3_file, e_lock: i32) -> i32 {
    let p_this: *mut MemFile = p_file as *mut MemFile;
    let p: *mut MemStore = unsafe { (*p_this).p_store };
    if e_lock >= unsafe { (*p_this).e_lock } { return 0; }
    memdb_enter(unsafe { &*p });
    ...
    if e_lock == 1 {
        if unsafe { (*p_this).e_lock } > 1 {
            {
                let __p = unsafe { &mut (*p).n_wr_lock };
                let __t = *__p;
                *__p -= 1;
                __t
            };
    ...
    unsafe { (*p_this).e_lock = e_lock };
    memdb_leave(unsafe { &*p });
    return 0;
}
extern "C" fn memdb_file_control(p_file: *mut sqlite3_file, op: i32,
    p_arg: *mut ()) -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        let mut rc: i32 = 12;
        ...
        if op == 12 {
            unsafe {
                *(p_arg as *mut *mut i8) =
                    unsafe {
                        sqlite3_mprintf(c"memdb(%p,%lld)".as_ptr() as *mut i8 as
                                *const i8, unsafe { (*p).a_data }, unsafe { (*p).sz })
                    }
            };
            rc = 0;
        }
        if op == 36 {
            let mut i_limit: sqlite3_int64 =
                unsafe { core::ptr::read(p_arg as *mut sqlite3_int64) };
            if i_limit < unsafe { (*p).sz } {
                if i_limit < 0 as i64 {
                    i_limit = unsafe { (*p).sz_max };
                } else { i_limit = unsafe { (*p).sz }; }
            }
            unsafe { (*p).sz_max = i_limit };
            unsafe { *(p_arg as *mut sqlite3_int64) = i_limit };
            rc = 0;
        }
        memdb_leave(unsafe { &*p });
        return rc;
...
extern "C" fn memdb_device_characteristics(p_file: *mut sqlite3_file) -> i32 {
    { let _ = p_file; };
    return 1 | 4096 | 512 | 1024;
}
extern "C" fn memdb_fetch(p_file: *mut sqlite3_file, i_ofst: sqlite3_int64,
    i_amt: i32, pp: *mut *mut ()) -> i32 {
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        memdb_enter(unsafe { &*p });
        if i_ofst + i_amt as sqlite3_int64 > unsafe { (*p).sz } ||
                unsafe { (*p).m_flags } & 2 as u32 != 0 as u32 {
            unsafe { *pp = core::ptr::null_mut() };
        } else {
            {
                let __p = unsafe { &mut (*p).n_mmap };
                let __t = *__p;
                *__p += 1;
                __t
            };
...
extern "C" fn memdb_unfetch(p_file: *mut sqlite3_file, i_ofst: sqlite3_int64,
    p_page: *mut ()) -> i32 {
    let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
    { let _ = i_ofst; };
    { let _ = p_page; };
    memdb_enter(unsafe { &*p });
    { let __p = unsafe { &mut (*p).n_mmap }; let __t = *__p; *__p -= 1; __t };
    memdb_leave(unsafe { &*p });
    return 0;
}
static memdb_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 3,
        x_close: Some(memdb_close),
        x_read: Some(memdb_read),
        x_write: Some(memdb_write),
        x_truncate: Some(memdb_truncate),
        x_sync: Some(memdb_sync),
        x_file_size: Some(memdb_file_size),
        x_lock: Some(memdb_lock),
        x_unlock: Some(memdb_unlock),
        x_check_reserved_lock: None,
        x_file_control: Some(memdb_file_control),
        x_sector_size: None,
        x_device_characteristics: Some(memdb_device_characteristics),
        x_shm_map: None,
        x_shm_lock: None,
        x_shm_barrier: None,
        x_shm_unmap: None,
...
extern "C" fn memdb_from_db_schema(db: *mut sqlite3, z_schema_1: *const i8)
    ...
    let mut p_store: *mut MemStore = core::ptr::null_mut();
    let rc: i32 =
        unsafe {
            sqlite3_file_control(db, z_schema_1, 7, &raw mut p as *mut ())
        };
    if rc != 0 { return core::ptr::null_mut(); }
    if unsafe { (*p).base.p_methods } !=
            &raw const memdb_io_methods as *const sqlite3_io_methods {
        return core::ptr::null_mut();
    }
    p_store = unsafe { (*p).p_store };
    memdb_enter(unsafe { &*p_store });
    if unsafe { (*p_store).z_f_name } != core::ptr::null_mut() {
        p = core::ptr::null_mut();
    }
...
pub extern "C" fn sqlite3_serialize(db: *mut sqlite3,
    mut z_schema_1: *const i8, pi_size_1: *mut sqlite3_int64, m_flags_1: u32)
    -> *mut u8 {
    unsafe {
        let mut p: *const MemFile = core::ptr::null();
        let mut i_db: i32 = 0;
        ...
        let mut p_out: *mut u8 = core::ptr::null_mut();
        let mut z_sql: *mut i8 = core::ptr::null_mut();
        let mut rc: i32 = 0;
        let mut p_store: *const MemStore = core::ptr::null();
        let mut n_page: i32 = 0;
        let mut p_pager: *mut Pager = core::ptr::null_mut();
        let mut pgno: i32 = 0;
        let mut p_page: *mut DbPage = core::ptr::null_mut();
        let mut p_to: *mut u8 = core::ptr::null_mut();
        ...
        loop {
            ...
            '__s3:
                ...
                match __state {
                    ...
                    4 => { __state = 5; }
                    5 => { __state = 6; }
                    6 => { sz_page = 0; __state = 7; }
                    7 => { p_stmt = core::ptr::null_mut(); __state = 8; }
                    8 => { p_out = core::ptr::null_mut(); __state = 9; }
                    9 => { __state = 10; }
                    10 => { __state = 11; }
                    11 => {
                    ...
                    12 => {
                        if z_schema_1 == core::ptr::null() {
                            __state = 14;
                        } else { __state = 13; }
                    }
                    13 => {
                        p = memdb_from_db_schema(db, z_schema_1);
                        __state = 15;
                    }
                    14 => {
                        z_schema_1 =
                    ...
                    15 => {
                        i_db = unsafe { sqlite3_find_db_name(db, z_schema_1) };
                        __state = 16;
                    }
                    16 => {
                        if !(pi_size_1).is_null() {
                            __state = 18;
                        } else { __state = 17; }
                    }
                    17 => {
                    ...
                    18 => {
                        unsafe { *pi_size_1 = -1 as sqlite3_int64 };
                        __state = 17;
                    }
                    19 => {
                        if !(p).is_null() { __state = 22; } else { __state = 21; }
                    }
                    20 => { __state = 2; }
                    21 => {
                        p_bt =
                    ...
                    22 => {
                        p_store = unsafe { (*p).p_store } as *const MemStore;
                        __state = 23;
                    }
                    ...
                    25 => {
                        if m_flags_1 & 1 as u32 != 0 {
                            __state = 28;
                        } else { __state = 29; }
                    }
                    26 => {
                        unsafe { *pi_size_1 = unsafe { (*p_store).sz } };
                        __state = 25;
                    }
                    27 => { __state = 2; }
                    28 => {
                        p_out = unsafe { (*p_store).a_data };
                        __state = 27;
                    }
                    29 => {
                        p_out =
                            unsafe {
                                    sqlite3_malloc64(unsafe { (*p_store).sz } as sqlite3_uint64)
                                } as *mut u8;
                        __state = 30;
                    }
                    30 => {
                    ...
                    31 => {
                        unsafe {
                            memcpy(p_out as *mut (),
                                unsafe { (*p_store).a_data } as *const (),
                                unsafe { (*p_store).sz } as u64)
                        };
                        __state = 27;
                    }
                    32 => {
                    ...
                    33 => {
                        sz_page = unsafe { sqlite3_btree_get_page_size(p_bt) };
                        __state = 35;
                    }
                    34 => { __state = 2; }
                    35 => {
                        z_sql =
                            unsafe {
                                sqlite3_mprintf(c"PRAGMA \"%w\".page_count".as_ptr() as
                                            *mut i8 as *const i8, z_schema_1)
                            };
                        __state = 36;
                    }
                    36 => {
                    ...
                    40 => { __state = 2; }
                    41 => {
                        if rc == 100 { __state = 43; } else { __state = 42; }
                    }
                    42 => { unsafe { sqlite3_finalize(p_stmt) }; __state = 69; }
                    43 => {
                        sz =
                            unsafe { sqlite3_column_int64(p_stmt, 0) } *
                                sz_page as sqlite3_int64;
                        __state = 44;
                    }
                    44 => {
                        if sz == 0 as i64 { __state = 46; } else { __state = 45; }
                    }
                    45 => {
                        if !(pi_size_1).is_null() {
                            __state = 52;
                    ...
                    46 => { unsafe { sqlite3_reset(p_stmt) }; __state = 47; }
                    47 => {
                        unsafe {
                            sqlite3_exec(db,
                                c"BEGIN IMMEDIATE; COMMIT;".as_ptr() as *mut i8 as
                    ...
                    49 => {
                        if rc == 100 { __state = 50; } else { __state = 45; }
                    }
                    50 => {
                        sz =
                            unsafe { sqlite3_column_int64(p_stmt, 0) } *
                    ...
                    52 => { unsafe { *pi_size_1 = sz }; __state = 51; }
                    53 => { p_out = core::ptr::null_mut(); __state = 42; }
                    54 => {
                        p_out =
                    ...
                    56 => {
                        n_page = unsafe { sqlite3_column_int(p_stmt, 0) };
                        __state = 57;
                    }
                    57 => {
                    ...
                    58 => { __state = 59; }
                    59 => { pgno = 1; __state = 60; }
                    60 => {
                        if pgno <= n_page { __state = 61; } else { __state = 42; }
                    }
                    61 => { p_page = core::ptr::null_mut(); __state = 63; }
                    62 => {
                        { let __p = &mut pgno; let __t = *__p; *__p += 1; __t };
                        __state = 60;
                    }
                    63 => {
                        p_to =
                            unsafe {
                                p_out.offset((sz_page as sqlite3_int64 *
                                            (pgno - 1) as sqlite3_int64) as isize)
                            };
                        __state = 64;
                    }
                    64 => {
                        rc =
                            unsafe {
                                sqlite3_pager_get(p_pager, pgno as Pgno,
                                    &raw mut p_page as *mut *mut DbPage, 0)
                            };
                        __state = 65;
                    }
                    65 => {
                        if rc == 0 { __state = 67; } else { __state = 68; }
                    }
                    66 => {
                        unsafe { sqlite3_pager_unref(p_page) };
                        __state = 62;
                    }
                    67 => {
                        unsafe {
                            memcpy(p_to as *mut (),
                                unsafe { sqlite3_pager_get_data(p_page) } as *const (),
                                sz_page as u64)
                        };
                        __state = 66;
                    }
                    68 => {
                        unsafe { memset(p_to as *mut (), 0, sz_page as u64) };
                        __state = 66;
                    }
                    69 => { __state = 2; }
                    70 => { return p_out; }
...
pub extern "C" fn sqlite3_deserialize(db: *mut sqlite3,
    mut z_schema_1: *const i8, mut p_data_1: *mut u8, sz_db_1: sqlite3_int64,
    sz_buf_1: sqlite3_int64, m_flags_1: u32) -> i32 {
    unsafe {
        unsafe {
            let mut rc: i32 = 0;
            '__b4: loop {
                '__c4: loop {
                    ...
                    i_db = unsafe { sqlite3_find_db_name(db, z_schema_1) };
                    if i_db < 2 && i_db != 0 { rc = 1; break '__b4; }
                    z_sql =
                        unsafe {
                            sqlite3_mprintf(c"ATTACH x AS %Q".as_ptr() as *mut i8 as
                                    *const i8, z_schema_1)
                        };
                    ...
                    unsafe { (*db).init.set_reopen_memdb(1 as u32 as u32) };
                    unsafe { sqlite3_step(p_stmt) };
                    unsafe { (*db).init.set_reopen_memdb(0 as u32 as u32) };
                    ...
                    if p == core::ptr::null_mut() {
                        ...
                        let p_store: *mut MemStore = unsafe { (*p).p_store };
                        unsafe { (*p_store).a_data = p_data_1 };
                        p_data_1 = core::ptr::null_mut();
                        unsafe { (*p_store).sz = sz_db_1 };
                        unsafe { (*p_store).sz_alloc = sz_buf_1 };
                        unsafe { (*p_store).sz_max = sz_buf_1 };
                        if unsafe { (*p_store).sz_max } <
                                sqlite3Config.mx_memdb_size {
                            unsafe { (*p_store).sz_max = sqlite3Config.mx_memdb_size };
                        }
                        unsafe { (*p_store).m_flags = m_flags_1 };
                        rc = 0;
                    }
                    break '__c4;
                }
            ...
            if !(p_data_1).is_null() && m_flags_1 & 1 as u32 != 0 as u32 {
...
extern "C" fn memdb_open(p_vfs: *mut sqlite3_vfs, z_name: *const i8,
    p_fd: *mut sqlite3_file, flags: i32, p_out_flags: *mut i32) -> i32 {
    unsafe {
        let p_file: *mut MemFile = p_fd as *mut MemFile;
        let mut p: *mut MemStore = core::ptr::null_mut();
        let mut sz_name: i32 = 0;
        { let _ = p_vfs; };
        unsafe {
            memset(p_file as *mut (), 0,
                core::mem::size_of::<MemFile>() as u64)
        };
        sz_name = unsafe { sqlite3_strlen30(z_name) };
        if sz_name > 1 &&
                (unsafe { *z_name.offset(0 as isize) } as i32 == '/' as i32 ||
                    unsafe { *z_name.offset(0 as isize) } as i32 == '\\' as i32)
            {
            let mut i: i32 = 0;
            let p_vfs_mutex: *mut sqlite3_mutex =
            ...
            unsafe { sqlite3_mutex_enter(p_vfs_mutex) };
            {
                i = 0;
                '__b5: loop {
                    if !(i < memdb_g.n_mem_store) { break '__b5; }
                    '__c5: loop {
                        if unsafe {
                                    strcmp(unsafe {
                                                (*unsafe {
                            ...
                            p = unsafe { *memdb_g.ap_mem_store.offset(i as isize) };
                            break '__b5;
                        }
                        break '__c5;
                    }
            ...
            if p == core::ptr::null_mut() {
                let mut ap_new: *mut *mut MemStore = core::ptr::null_mut();
                p =
                    unsafe {
                            sqlite3Malloc(core::mem::size_of::<MemStore>() as u64 +
                                        sz_name as i64 as u64 + 3 as u64)
                        } as *mut MemStore;
                if p == core::ptr::null_mut() {
                    unsafe { sqlite3_mutex_leave(p_vfs_mutex) };
                    return 7;
                ...
                ap_new =
                    unsafe {
                            sqlite3Realloc(memdb_g.ap_mem_store as *mut (),
                                core::mem::size_of::<*mut MemStore>() as u64 *
                                    (1 as i64 + memdb_g.n_mem_store as i64) as u64)
                        } as *mut *mut MemStore;
                ...
                memdb_g.ap_mem_store = ap_new;
                unsafe {
                    memset(p as *mut (), 0,
                        core::mem::size_of::<MemStore>() as u64)
                };
                unsafe { (*p).m_flags = (2 | 1) as u32 };
                unsafe { (*p).sz_max = sqlite3Config.mx_memdb_size };
                unsafe {
                    (*p).z_f_name =
                        unsafe { &raw mut *p.offset(1 as isize) } as *mut i8
                };
                unsafe {
                    memcpy(unsafe { (*p).z_f_name } as *mut (),
                        z_name as *const (), (sz_name + 1) as u64)
                };
                unsafe { (*p).p_mutex = unsafe { sqlite3_mutex_alloc(0) } };
                if unsafe { (*p).p_mutex } == core::ptr::null_mut() {
                    {
                        let __p = &mut memdb_g.n_mem_store;
                        let __t = *__p;
                        *__p -= 1;
        ...
        unsafe { (*p_file).p_store = p };
        if p_out_flags != core::ptr::null_mut() {
            unsafe { *p_out_flags = flags | 128 };
        }
        unsafe { (*p_fd).p_methods = &memdb_io_methods };
        memdb_leave(unsafe { &*p });
        return 0;
    }
}
extern "C" fn memdb_access(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
...
extern "C" fn memdb_full_pathname(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
    n_out: i32, z_out: *mut i8) -> i32 {
    { let _ = p_vfs; };
    unsafe {
        sqlite3_snprintf(n_out, z_out, c"%s".as_ptr() as *mut i8 as *const i8,
            z_path)
    };
    return 0;
}
extern "C" fn memdb_dl_open(p_vfs: *mut sqlite3_vfs, z_path: *const i8)
    -> *mut () {
    return unsafe {
            (unsafe {
...
extern "C" fn memdb_dl_error(p_vfs: *mut sqlite3_vfs, n_byte: i32,
    z_err_msg: *mut i8) -> () {
    unsafe {
        (unsafe {
                (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn memdb_dl_sym(p_vfs: *mut sqlite3_vfs, p: *mut (),
...
extern "C" fn memdb_dl_close(p_vfs: *mut sqlite3_vfs, p_handle: *mut ())
    -> () {
    unsafe {
        (unsafe {
                (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn memdb_randomness(p_vfs: *mut sqlite3_vfs, n_byte: i32,
...
extern "C" fn memdb_sleep(p_vfs: *mut sqlite3_vfs, n_micro: i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn memdb_get_last_error(p_vfs: *mut sqlite3_vfs, a: i32,
    b: *mut i8) -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn memdb_current_time_int64(p_vfs: *mut sqlite3_vfs,
    p: *mut sqlite3_int64) -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
                                        *mut sqlite3_vfs)).x_current_time_int64.unwrap()
                })(unsafe { (*p_vfs).p_app_data } as *mut sqlite3_vfs, p)
        };
}
static mut memdb_vfs: sqlite3_vfs =
    sqlite3_vfs {
        i_version: 2,
        sz_os_file: 0,
        mx_pathname: 1024,
        p_next: core::ptr::null_mut(),
        z_name: c"memdb".as_ptr() as *const i8,
        p_app_data: core::ptr::null_mut(),
        x_open: Some(memdb_open),
        x_delete: None,
        x_access: Some(memdb_access),
        x_full_pathname: Some(memdb_full_pathname),
        x_dl_open: Some(memdb_dl_open),
        x_dl_error: Some(memdb_dl_error),
        x_dl_sym: Some(memdb_dl_sym),
        x_dl_close: Some(memdb_dl_close),
        x_randomness: Some(memdb_randomness),
        x_sleep: Some(memdb_sleep),
        x_current_time: None,
        x_get_last_error: Some(memdb_get_last_error),
        x_current_time_int64: Some(memdb_current_time_int64),
...
pub extern "C" fn sqlite3_memdb_init() -> i32 {
    unsafe {
        let p_lower: *mut sqlite3_vfs =
            unsafe { sqlite3_vfs_find(core::ptr::null()) };
        let mut sz: u32 = 0 as u32;
        if p_lower == core::ptr::null_mut() { return 1; }
        sz = unsafe { (*p_lower).sz_os_file } as u32;
        memdb_vfs.p_app_data = p_lower as *mut ();
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_is_memdb(p_vfs: *const sqlite3_vfs) -> i32 {
    unsafe {
        return (p_vfs == &raw mut memdb_vfs as *const sqlite3_vfs) as i32;
    }
}
