...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_init() -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        if !sqlite3Config.mutex.x_mutex_alloc.is_some() as i32 != 0 {
            let mut p_from: *const sqlite3_mutex_methods = core::ptr::null();
            let p_to: *mut sqlite3_mutex_methods = &mut sqlite3Config.mutex;
            if sqlite3Config.b_core_mutex != 0 {
                p_from = unsafe { sqlite3_default_mutex() };
            } else { p_from = unsafe { sqlite3_noop_mutex() }; }
            unsafe {
                (*p_to).x_mutex_init = unsafe { (*p_from).x_mutex_init }
            };
            unsafe { (*p_to).x_mutex_end = unsafe { (*p_from).x_mutex_end } };
            unsafe {
                (*p_to).x_mutex_free = unsafe { (*p_from).x_mutex_free }
            };
            unsafe {
                (*p_to).x_mutex_enter = unsafe { (*p_from).x_mutex_enter }
            };
            unsafe { (*p_to).x_mutex_try = unsafe { (*p_from).x_mutex_try } };
            unsafe {
                (*p_to).x_mutex_leave = unsafe { (*p_from).x_mutex_leave }
            };
            unsafe {
                (*p_to).x_mutex_held = unsafe { (*p_from).x_mutex_held }
            };
            unsafe {
                (*p_to).x_mutex_notheld = unsafe { (*p_from).x_mutex_notheld }
            };
            unsafe { sqlite3_memory_barrier() };
            unsafe {
                (*p_to).x_mutex_alloc = unsafe { (*p_from).x_mutex_alloc }
            };
        }
        { let _ = 0; };
        rc = unsafe { sqlite3Config.mutex.x_mutex_init.unwrap()() };
        unsafe { sqlite3_memory_barrier() };
        return rc;
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_alloc(id: i32) -> *mut sqlite3_mutex {
    unsafe {
        if id <= 1 && unsafe { sqlite3_initialize() } != 0 {
            return core::ptr::null_mut();
        }
        if id > 1 && sqlite3_mutex_init() != 0 {
            return core::ptr::null_mut();
        }
        { let _ = 0; };
        return unsafe { sqlite3Config.mutex.x_mutex_alloc.unwrap()(id) };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_free(p: *mut sqlite3_mutex) -> () {
    unsafe {
        if !(p).is_null() {
            { let _ = 0; };
            unsafe { sqlite3Config.mutex.x_mutex_free.unwrap()(p) };
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_enter(p: *mut sqlite3_mutex) -> () {
    unsafe {
        if !(p).is_null() {
            { let _ = 0; };
            unsafe { sqlite3Config.mutex.x_mutex_enter.unwrap()(p) };
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_try(p: *mut sqlite3_mutex) -> i32 {
    unsafe {
        let rc: i32 = 0;
        if !(p).is_null() {
            { let _ = 0; };
            return unsafe { sqlite3Config.mutex.x_mutex_try.unwrap()(p) };
        }
        return rc;
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_leave(p: *mut sqlite3_mutex) -> () {
    unsafe {
        if !(p).is_null() {
            { let _ = 0; };
            unsafe { sqlite3Config.mutex.x_mutex_leave.unwrap()(p) };
        }
    }
}
#[repr(C)]
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3MutexAlloc(id: i32) -> *mut sqlite3_mutex {
    unsafe {
        if (sqlite3Config.b_core_mutex == 0) as i32 != 0 {
            return core::ptr::null_mut();
        }
        { let _ = 0; };
        { let _ = 0; };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mutex_end() -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        if sqlite3Config.mutex.x_mutex_end.is_some() {
            rc = unsafe { sqlite3Config.mutex.x_mutex_end.unwrap()() };
        }
        return rc;
    }
}
