...
extern "C" fn backup_test_cmd(client_data_1: ClientData,
    ...
    let mut a_sub: [BackupSubCommand_N16BackupSubCommand; 5] =
        [BackupSubCommand_N16BackupSubCommand {
                    z_cmd: c"step".as_ptr() as *const i8,
                    e_cmd: BACKUP_STEP,
                    n_arg: 1,
                    z_arg: c"npage".as_ptr() as *const i8,
                },
                BackupSubCommand_N16BackupSubCommand {
                    z_cmd: c"finish".as_ptr() as *const i8,
                    e_cmd: BACKUP_FINISH,
                    n_arg: 0,
                    z_arg: c"".as_ptr() as *const i8,
                },
                BackupSubCommand_N16BackupSubCommand {
                    z_cmd: c"remaining".as_ptr() as *const i8,
                    e_cmd: BACKUP_REMAINING,
                    n_arg: 0,
                    z_arg: c"".as_ptr() as *const i8,
                },
                BackupSubCommand_N16BackupSubCommand {
                    z_cmd: c"pagecount".as_ptr() as *const i8,
                    e_cmd: BACKUP_PAGECOUNT,
                    n_arg: 0,
                    z_arg: c"".as_ptr() as *const i8,
                },
                BackupSubCommand_N16BackupSubCommand {
    ...
    let p: *mut sqlite3_backup = client_data_1 as *mut sqlite3_backup;
    let mut i_cmd: i32 = 0;
    let mut rc: i32 = 0;
    rc =
        unsafe {
            Tcl_GetIndexFromObjStruct(interp,
                ...
                &raw mut a_sub[0 as usize] as
                        *mut BackupSubCommand_N16BackupSubCommand as *const (),
                core::mem::size_of::<BackupSubCommand_N16BackupSubCommand>()
                    as i32, c"option".as_ptr() as *mut i8 as *const i8, 0,
                &mut i_cmd)
        };
    if rc != 0 { return rc; }
    if objc != 2 + a_sub[i_cmd as usize].n_arg {
        unsafe {
            Tcl_WrongNumArgs(interp, 2, objv, a_sub[i_cmd as usize].z_arg)
        };
        return 1;
    }
    '__s0:
        {
        match a_sub[i_cmd as usize].e_cmd {
            BACKUP_FINISH => {
                {
                    let mut z_cmd_name: *const i8 = core::ptr::null();
                    let mut cmd_info: Tcl_CmdInfo =
                        unsafe { core::mem::zeroed() };
                    z_cmd_name =
                        unsafe {
                                Tcl_GetString(unsafe { *objv.offset(0 as isize) })
                            } as *const i8;
                    unsafe {
                        Tcl_GetCommandInfo(interp, z_cmd_name, &mut cmd_info)
                    };
                    cmd_info.deleteProc = None;
                    unsafe {
                        Tcl_SetCommandInfo(interp, z_cmd_name,
                            &raw mut cmd_info as *const Tcl_CmdInfo)
                    };
                    unsafe { Tcl_DeleteCommand(interp, z_cmd_name) };
                    rc = unsafe { sqlite3_backup_finish(p) };
                    unsafe {
                        Tcl_SetResult(interp,
                            unsafe { sqlite3_err_name(rc) } as *mut i8,
                            unsafe {
                    ...
                    if 0 !=
                            unsafe {
                                Tcl_GetIntFromObj(interp,
                                    unsafe { *objv.offset(2 as isize) }, &mut n_page)
                            } {
                        return 1;
                    }
                    rc = unsafe { sqlite3_backup_step(p, n_page) };
                    unsafe {
                        Tcl_SetResult(interp,
                            unsafe { sqlite3_err_name(rc) } as *mut i8,
                            unsafe {
                ...
                unsafe {
                    Tcl_SetObjResult(interp,
                        unsafe {
                            Tcl_NewIntObj(unsafe { sqlite3_backup_remaining(p) })
                        })
                };
            }
            BACKUP_STEP => {
            ...
            BACKUP_PAGECOUNT => {
                unsafe {
                    Tcl_SetObjResult(interp,
                        unsafe {
                            Tcl_NewIntObj(unsafe { sqlite3_backup_pagecount(p) })
                        })
                };
            }
            _ => {}
...
extern "C" fn backup_test_finish(client_data_1: ClientData) -> () {
    let p_backup: *mut sqlite3_backup = client_data_1 as *mut sqlite3_backup;
...
#[repr(C)]
#[derive(Copy, Clone)]
struct BackupSubCommand_N16BackupSubCommand {
    z_cmd: *const i8,
    e_cmd: u32,
    n_arg: i32,
    z_arg: *const i8,
...
extern "C" {
    ...
    -> i32;
    fn Tcl_SetCommandInfo(interp: *mut Tcl_Interp, cmdName: *const i8,
