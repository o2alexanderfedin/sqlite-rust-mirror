    ...
    if !(p).is_null() {
        {
            ...
            '__b0: loop {
                if !(i > 0) { break '__b0; }
                '__c0: loop {
                    if sqlite3_walk_expr(p_walker, unsafe { (*p_item).p_expr })
                            != 0 {
                        return 2;
                    }
                    break '__c0;
    ...
    let mut p_win: *const Window = core::ptr::null();
    {
        ...
        '__b1: loop {
            ...
            '__c1: loop {
                ...
                if rc != 0 { return 2; }
                rc =
                    sqlite3_walk_expr(p_walker_1, unsafe { (*p_win).p_start });
                if rc != 0 { return 2; }
                rc = sqlite3_walk_expr(p_walker_1, unsafe { (*p_win).p_end });
                if rc != 0 { return 2; }
                if b_one_only_1 != 0 { break '__b1; }
                break '__c1;
            }
...
pub extern "C" fn sqlite3_walk_select_expr(p_walker: *mut Walker, p: &Select)
    -> i32 {
    unsafe {
        if sqlite3_walk_expr_list(p_walker, (*p).p_e_list) != 0 { return 2; }
        if sqlite3_walk_expr(p_walker, (*p).p_where) != 0 { return 2; }
        if sqlite3_walk_expr_list(p_walker, (*p).p_group_by) != 0 {
            return 2;
        }
        if sqlite3_walk_expr(p_walker, (*p).p_having) != 0 { return 2; }
        if sqlite3_walk_expr_list(p_walker, (*p).p_order_by) != 0 {
            return 2;
        }
        if sqlite3_walk_expr(p_walker, (*p).p_limit) != 0 { return 2; }
        if !((*p).p_win_defn).is_null() {
            let mut p_parse: *const Parse = core::ptr::null();
            if unsafe { (*p_walker).x_select_callback2 } ==
                            Some(sqlite3_walk_win_defn_dummy_callback) ||
                        { p_parse = unsafe { (*p_walker).p_parse }; p_parse } !=
                                core::ptr::null_mut() &&
                ...
                '__b2: loop {
                    ...
                    '__c2: loop {
                        if unsafe { (*p_item).fg.is_subquery() } != 0 &&
                                sqlite3_walk_select(p_walker,
                                        unsafe { (*unsafe { (*p_item).u4.p_subq }).p_select }) != 0
                            {
                            return 2;
                        }
                        if unsafe { (*p_item).fg.is_tab_func() } != 0 &&
                                sqlite3_walk_expr_list(p_walker,
                                        unsafe { (*p_item).u1.p_func_arg }) != 0 {
                            return 2;
                        }
                        break '__c2;
                    }
...
pub extern "C" fn sqlite3_walk_select(p_walker: *mut Walker,
    ...
    if !unsafe { (*p_walker).x_select_callback.is_some() } as i32 != 0 {
        return 0;
    }
    '__b3: loop {
        '__c3: loop {
            rc =
                unsafe {
                    (unsafe {
                            (*p_walker).x_select_callback.unwrap()
                        })(p_walker, p)
                };
            if rc != 0 { return rc & 2; }
            if sqlite3_walk_select_expr(p_walker, unsafe { &*p }) != 0 ||
                    sqlite3_walk_select_from(p_walker, unsafe { &*p }) != 0 {
                return 2;
            }
            if unsafe { (*p_walker).x_select_callback2.is_some() } {
                unsafe {
                    (unsafe {
                            (*p_walker).x_select_callback2.unwrap()
                        })(p_walker, p)
                };
            }
            p = unsafe { (*p).p_prior };
        ...
        if !(p != core::ptr::null_mut()) { break '__b3; }
    }
    return 0;
}
#[unsafe(no_mangle)]
    ...
    unsafe {
        ...
        loop {
            rc =
                unsafe {
                    (unsafe {
                            (*p_walker).x_expr_callback.unwrap()
                        })(p_walker, p_expr)
                };
            if rc != 0 { return rc & 2; }
            if !(unsafe { (*p_expr).flags } & (65536 | 8388608) as u32 !=
                                0 as u32) as i32 != 0 {
                { let _ = 0; };
                if !(unsafe { (*p_expr).p_left }).is_null() &&
                        sqlite3_walk_expr_nn(p_walker, unsafe { (*p_expr).p_left })
                            != 0 {
                    return 2;
                }
                if !(unsafe { (*p_expr).p_right }).is_null() {
                    { let _ = 0; };
                    p_expr = unsafe { (*p_expr).p_right };
                    continue;
                } else if unsafe { (*p_expr).flags } & 4096 as u32 != 0 as u32
                    {
                    { let _ = 0; };
                    if sqlite3_walk_select(p_walker,
                                unsafe { (*p_expr).x.p_select }) != 0 {
                        return 2;
                    }
                } else {
                    if !(unsafe { (*p_expr).x.p_list }).is_null() {
                        if sqlite3_walk_expr_list(p_walker,
                                    unsafe { (*p_expr).x.p_list }) != 0 {
                            return 2;
                        }
                    }
                    if unsafe { (*p_expr).flags } & 16777216 as u32 != 0 as u32
                        {
                        if walk_window_list(p_walker, unsafe { (*p_expr).y.p_win },
                                    1) != 0 {
                            return 2;
                        }
                    }
...
pub extern "C" fn sqlite3_walker_depth_increase(p_walker: &mut Walker,
    ...
    { let _ = p_select; };
    {
        let __p = &mut (*p_walker).walker_depth;
        let __t = *__p;
        *__p += 1;
        __t
    };
