...
extern "C" fn tcl_thread_create(not_used_1: *mut (), interp: *mut TclInterp,
    ...
    unsafe {
        let mut i: i32 = 0;
        let mut x: PthreadT = core::ptr::null_mut();
        let mut rc: i32 = 0;
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
...
pub extern "C" fn sqlitetest4_init(interp: *mut TclInterp) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        {
            ...
            '__b7: loop {
                if !((i as u64) <
                                core::mem::size_of::<[Sqlitetest4InitS0N19sqlitetest4InitS0; 15]>()
                                        as u64 / 16) {
                    break '__b7;
                }
                '__c7: loop {
...
extern "C" {
    ...
    -> i32;
    fn pthread_detach(_: PthreadT)
    -> i32;
    fn Tcl_GetInt(interp: *mut TclInterp, src: *const i8, intPtr: *mut i32)
    -> i32;
