...
#[derive(Copy, Clone)]
struct RowSet {
    p_chunk: *mut RowSetChunk,
    db: *mut sqlite3,
    p_entry: *mut RowSetEntry,
    p_last: *mut RowSetEntry,
    p_fresh: *mut RowSetEntry,
    p_forest: *mut RowSetEntry,
    n_fresh: u16,
    rs_flags: u16,
    i_batch: i32,
}
    ...
    let p: *mut RowSet =
        unsafe {
                sqlite3_db_malloc_raw_nn(db,
                    core::mem::size_of::<RowSet>() as u64)
            } as *mut RowSet;
    if !(p).is_null() {
        let n: i32 = unsafe { sqlite3_db_malloc_size(db, p as *const ()) };
        unsafe { (*p).p_chunk = core::ptr::null_mut() };
        unsafe { (*p).db = db };
        unsafe { (*p).p_entry = core::ptr::null_mut() };
        unsafe { (*p).p_last = core::ptr::null_mut() };
        unsafe { (*p).p_forest = core::ptr::null_mut() };
        unsafe {
            (*p).p_fresh =
                unsafe {
                        (p as
                                *mut i8).add((core::mem::size_of::<RowSet>() as u64 +
        ...
        unsafe {
            (*p).n_fresh =
                ((n as u64 -
                            (core::mem::size_of::<RowSet>() as u64 + 7 as u64 &
                                !7 as u64)) / core::mem::size_of::<RowSetEntry>() as u64) as
                    u16
        };
...
pub extern "C" fn sqlite3_row_set_clear(p_arg: *mut ()) -> () {
    ...
    let mut p_chunk: *mut RowSetChunk = core::ptr::null_mut();
    let mut p_next_chunk: *mut RowSetChunk = core::ptr::null_mut();
    {
        p_chunk = unsafe { (*p).p_chunk };
        '__b0: loop {
            if !(!(p_chunk).is_null()) { break '__b0; }
            '__c0: loop {
                p_next_chunk = unsafe { (*p_chunk).p_next_chunk };
                unsafe {
                    sqlite3_db_free(unsafe { (*p).db }, p_chunk as *mut ())
                };
                break '__c0;
            }
            p_chunk = p_next_chunk;
        }
    }
    unsafe { (*p).p_chunk = core::ptr::null_mut() };
    unsafe { (*p).n_fresh = 0 as u16 };
    unsafe { (*p).p_entry = core::ptr::null_mut() };
    unsafe { (*p).p_last = core::ptr::null_mut() };
    unsafe { (*p).p_forest = core::ptr::null_mut() };
    unsafe { (*p).rs_flags = 1 as u16 };
...
extern "C" fn row_set_entry_alloc(p: &mut RowSet) -> *mut RowSetEntry {
    ...
    if (*p).n_fresh as i32 == 0 {
        ...
        p_new =
            unsafe {
                    sqlite3_db_malloc_raw_nn((*p).db,
                        core::mem::size_of::<RowSetChunk>() as u64)
                } as *mut RowSetChunk;
        if p_new == core::ptr::null_mut() { return core::ptr::null_mut(); }
        unsafe { (*p_new).p_next_chunk = (*p).p_chunk };
        (*p).p_chunk = p_new;
        (*p).p_fresh =
            unsafe { &raw mut (*p_new).a_entry[0 as usize] } as
                *mut RowSetEntry;
        (*p).n_fresh =
            ((1024 - 8) as u64 / core::mem::size_of::<RowSetEntry>() as u64)
                as u16;
    }
    { let __p = &mut (*p).n_fresh; let __t = *__p; *__p -= 1; __t };
    return {
            let __p = &mut (*p).p_fresh;
            let __t = *__p;
...
pub extern "C" fn sqlite3_row_set_insert(p: *mut RowSet, rowid: i64) -> () {
    ...
    unsafe { (*p_entry).v = rowid };
    unsafe { (*p_entry).p_right = core::ptr::null_mut() };
...
extern "C" fn row_set_entry_merge(mut p_a_1: *mut RowSetEntry,
    ...
    let mut p_tail: *mut RowSetEntry = core::ptr::null_mut();
    p_tail = &mut head;
    { let _ = 0; };
    {
        '__b1: loop {
            '__c1: loop {
                { let _ = 0; };
                { let _ = 0; };
                if unsafe { (*p_a_1).v } <= unsafe { (*p_b_1).v } {
                    if unsafe { (*p_a_1).v } < unsafe { (*p_b_1).v } {
                        p_tail =
                            {
                                unsafe { (*p_tail).p_right = p_a_1 };
                                unsafe { (*p_tail).p_right }
                            };
                    }
                    p_a_1 = unsafe { (*p_a_1).p_right };
                    if p_a_1 == core::ptr::null_mut() {
                        unsafe { (*p_tail).p_right = p_b_1 };
                        break '__b1;
                    }
                } else {
                    p_tail =
...
extern "C" fn row_set_entry_sort(mut p_in_1: *mut RowSetEntry)
    ...
    let mut p_next: *mut RowSetEntry = core::ptr::null_mut();
    let mut a_bucket: [*mut RowSetEntry; 40] = [core::ptr::null_mut(); 40];
    unsafe {
        memset(&raw mut a_bucket[0 as usize] as *mut *mut RowSetEntry as
                *mut (), 0,
            core::mem::size_of::<[*mut RowSetEntry; 40]>() as u64)
    };
    while !(p_in_1).is_null() {
        p_next = unsafe { (*p_in_1).p_right };
        unsafe { (*p_in_1).p_right = core::ptr::null_mut() };
        {
            i = 0 as u32;
            '__b3: loop {
                if !(!(a_bucket[i as usize]).is_null()) { break '__b3; }
                '__c3: loop {
                    p_in_1 = row_set_entry_merge(a_bucket[i as usize], p_in_1);
                    a_bucket[i as usize] = core::ptr::null_mut();
                    break '__c3;
                }
                { let __p = &mut i; let __t = *__p; *__p += 1; __t };
            }
        }
        a_bucket[i as usize] = p_in_1;
        p_in_1 = p_next;
    }
    p_in_1 = a_bucket[0 as usize];
    {
        i = 1 as u32;
        '__b4: loop {
            if !((i as u64) <
                            core::mem::size_of::<[*mut RowSetEntry; 40]>() as u64 /
                                core::mem::size_of::<*mut RowSetEntry>() as u64) {
                break '__b4;
            }
            '__c4: loop {
                if a_bucket[i as usize] == core::ptr::null_mut() {
                    break '__c4;
                }
                p_in_1 =
                    if !(p_in_1).is_null() {
                        row_set_entry_merge(p_in_1, a_bucket[i as usize])
                    } else { a_bucket[i as usize] };
                break '__c4;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
        }
...
extern "C" fn row_set_n_deep_tree(pp_list_1: *mut *mut RowSetEntry,
    ...
    if i_depth_1 > 1 {
        ...
        unsafe {
            (*p).p_right = row_set_n_deep_tree(pp_list_1, i_depth_1 - 1)
        };
    } else {
        p = unsafe { *pp_list_1 };
        unsafe { *pp_list_1 = unsafe { (*p).p_right } };
...
extern "C" fn row_set_list_to_tree(mut p_list_1: *mut RowSetEntry)
    ...
    };
    {
        i_depth = 1;
        '__b5: loop {
            if !(!(p_list_1).is_null()) { break '__b5; }
            '__c5: loop {
                p_left = p;
                p = p_list_1;
                p_list_1 = unsafe { (*p).p_right };
                unsafe { (*p).p_left = p_left };
                unsafe {
                    (*p).p_right = row_set_n_deep_tree(&mut p_list_1, i_depth)
                };
                break '__c5;
            }
            { let __p = &mut i_depth; let __t = *__p; *__p += 1; __t };
        }
    }
    return p;
}
extern "C" fn row_set_tree_to_list(p_in_1: *mut RowSetEntry,
    ...
    -> () {
    { let _ = 0; };
    if !(unsafe { (*p_in_1).p_left }).is_null() {
        let mut p: *mut RowSetEntry = core::ptr::null_mut();
        row_set_tree_to_list(unsafe { (*p_in_1).p_left }, pp_first_1, &mut p);
        unsafe { (*p).p_right = p_in_1 };
    } else { unsafe { *pp_first_1 = p_in_1 }; }
    if !(unsafe { (*p_in_1).p_right }).is_null() {
        row_set_tree_to_list(unsafe { (*p_in_1).p_right },
            unsafe { &mut (*p_in_1).p_right }, pp_last_1);
    } else { unsafe { *pp_last_1 = p_in_1 }; }
    { let _ = 0; };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_row_set_test(p_row_set_1: *mut RowSet,
    i_batch_1: i32, i_rowid_1: sqlite3_int64) -> i32 {
    let mut p: *mut RowSetEntry = core::ptr::null_mut();
    ...
    if i_batch_1 != unsafe { (*p_row_set_1).i_batch } {
        ...
        if !(p).is_null() {
            ...
            }
            {
                ...
                '__b6: loop {
                    if !(!(p_tree).is_null()) { break '__b6; }
                    '__c6: loop {
                        pp_prev_tree = unsafe { &mut (*p_tree).p_right };
                        if unsafe { (*p_tree).p_left } == core::ptr::null_mut() {
                            unsafe { (*p_tree).p_left = row_set_list_to_tree(p) };
                            break '__b6;
                        } else {
                            let mut p_aux: *mut RowSetEntry = core::ptr::null_mut();
                            let mut p_tail: *mut RowSetEntry = core::ptr::null_mut();
                            row_set_tree_to_list(unsafe { (*p_tree).p_left },
                                &mut p_aux, &mut p_tail);
                            unsafe { (*p_tree).p_left = core::ptr::null_mut() };
                            p = row_set_entry_merge(p_aux, p);
                        }
                        break '__c6;
                    }
                    p_tree = unsafe { (*p_tree).p_right };
                }
            }
            if p_tree == core::ptr::null_mut() {
                unsafe {
                    *pp_prev_tree =
                        {
                            p_tree = row_set_entry_alloc(unsafe { &mut *p_row_set_1 });
                            p_tree
                        }
                };
                if !(p_tree).is_null() {
                    unsafe { (*p_tree).v = 0 as i64 };
                    unsafe { (*p_tree).p_right = core::ptr::null_mut() };
                    unsafe { (*p_tree).p_left = row_set_list_to_tree(p) };
                }
            }
            unsafe { (*p_row_set_1).p_entry = core::ptr::null_mut() };
            unsafe { (*p_row_set_1).p_last = core::ptr::null_mut() };
            unsafe { (*p_row_set_1).rs_flags |= 1 as u16 };
        }
        unsafe { (*p_row_set_1).i_batch = i_batch_1 };
    }
    {
        p_tree = unsafe { (*p_row_set_1).p_forest };
        '__b7: loop {
            if !(!(p_tree).is_null()) { break '__b7; }
            '__c7: loop {
                p = unsafe { (*p_tree).p_left };
                while !(p).is_null() {
                    if unsafe { (*p).v } < i_rowid_1 {
                        p = unsafe { (*p).p_right };
                    } else if unsafe { (*p).v } > i_rowid_1 {
                        p = unsafe { (*p).p_left };
                    } else { return 1; }
                }
                break '__c7;
...
pub extern "C" fn sqlite3_row_set_next(p: *mut RowSet, p_rowid_1: &mut i64)
    ...
    if unsafe { (*p).rs_flags } as i32 & 2 == 0 {
        ...
        unsafe { (*p).rs_flags |= (1 | 2) as u16 };
    }
    if !(unsafe { (*p).p_entry }).is_null() {
        *p_rowid_1 = unsafe { (*unsafe { (*p).p_entry }).v };
        unsafe {
