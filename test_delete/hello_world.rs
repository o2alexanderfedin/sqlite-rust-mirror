...
extern "C" fn sqlite3_delete83_name(z: *mut i8) -> () {
    let mut i: i32 = 0;
    let mut sz: i32 = 0;
    sz = unsafe { strlen(z as *const i8) } as i32;
    {
        i = sz - 1;
        '__b0: loop {
            if !(i > 0 &&
    ...
    if unsafe { *z.offset(i as isize) } as i32 == '.' as i32 && sz > i + 4 {
        unsafe {
            memmove(unsafe { &raw mut *z.offset((i + 1) as isize) } as
                    *mut (),
                unsafe { &raw mut *z.offset((sz - 3) as isize) } as *const (),
                4 as u64)
        };
    }
}
extern "C" fn sqlite3_delete_unlink_if_exists(p_vfs_1: *const sqlite3_vfs,
    z_file_1: *const i8, pb_exists_1: *mut i32) -> i32 {
    let mut rc: i32 = 1;
    if !(p_vfs_1 == core::ptr::null_mut()) as i32 as i64 != 0 {
        unsafe {
            __assert_rtn(c"sqlite3DeleteUnlinkIfExists".as_ptr() as *const i8,
                c"test_delete.c".as_ptr() as *mut i8 as *const i8, 76,
                c"pVfs==0".as_ptr() as *mut i8 as *const i8)
        }
    } else { { let _ = 0; } };
    rc = unsafe { access(z_file_1, 0) };
    if rc != 0 {
        if unsafe { *unsafe { __error() } } == 2 {
            if !(pb_exists_1).is_null() { unsafe { *pb_exists_1 = 0 }; }
            rc = 0;
        }
    } else {
        if !(pb_exists_1).is_null() { unsafe { *pb_exists_1 = 1 }; }
        rc = unsafe { unlink(z_file_1) };
    }
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_delete_database(z_file_1: *const i8) -> i32 {
    ...
    let az_fmt: [*const i8; 4] =
        [c"%s".as_ptr() as *const i8, c"%s-journal".as_ptr() as *const i8,
                ...
                c"%s-shm".as_ptr() as *const i8];
    let mut a_m_file: [MFile_N5MFile; 6] =
        [MFile_N5MFile {
                    z_fmt: c"%s%03d".as_ptr() as *const i8,
                    i_offset: 0,
                    b83: 0,
                },
                MFile_N5MFile {
                    z_fmt: c"%s-journal%03d".as_ptr() as *const i8,
                    i_offset: 0,
                    b83: 0,
                },
                MFile_N5MFile {
                    z_fmt: c"%s-wal%03d".as_ptr() as *const i8,
                    i_offset: 0,
                    b83: 0,
                },
                MFile_N5MFile {
    ...
    let p_vfs: *mut sqlite3_vfs = core::ptr::null_mut();
    ...
    if z_buf == core::ptr::null_mut() { return 7; }
    {
        ...
        '__b1: loop {
            ...
            '__c1: loop {
                unsafe {
                    sqlite3_snprintf(n_buf, z_buf, az_fmt[i as usize], z_file_1)
                };
                rc =
                    sqlite3_delete_unlink_if_exists(p_vfs as *const sqlite3_vfs,
                        z_buf as *const i8, core::ptr::null_mut());
                if rc == 0 && i != 0 {
                    sqlite3_delete83_name(z_buf);
                    rc =
                        sqlite3_delete_unlink_if_exists(p_vfs as *const sqlite3_vfs,
                            z_buf as *const i8, core::ptr::null_mut());
                }
    ...
    }
    {
        ...
        '__b2: loop {
            if !(rc == 0 &&
                            (i as u64) <
                                core::mem::size_of::<[MFile_N5MFile; 6]>() as u64 /
                                    core::mem::size_of::<MFile_N5MFile>() as u64) {
                break '__b2;
            }
            '__c2: loop {
                let p: *const MFile_N5MFile =
                    &raw mut a_m_file[i as usize] as *const MFile_N5MFile;
                let mut i_chunk: i32 = 0;
                {
                    i_chunk = 1;
                    '__b3: loop {
                        if !(i_chunk <= 299) { break '__b3; }
                        '__c3: loop {
                            let mut b_exists: i32 = 0;
                            unsafe {
                                sqlite3_snprintf(n_buf, z_buf, unsafe { (*p).z_fmt },
                                    z_file_1, i_chunk + unsafe { (*p).i_offset })
                            };
                            if unsafe { (*p).b83 } != 0 {
                                sqlite3_delete83_name(z_buf);
                            }
                            rc =
                                sqlite3_delete_unlink_if_exists(p_vfs as *const sqlite3_vfs,
                                    z_buf as *const i8, &mut b_exists);
                            if b_exists == 0 || rc != 0 { break '__b3; }
                            break '__c3;
                        }
                        { let __p = &mut i_chunk; let __t = *__p; *__p += 1; __t };
                    }
                }
                break '__c2;
            }
