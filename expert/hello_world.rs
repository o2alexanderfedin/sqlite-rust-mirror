...
extern "C" fn usage(argv: *const *mut i8) -> () {
    unsafe {
        ...
        unsafe {
            fprintf(__stderrp,
                c"Usage %s ?OPTIONS? DATABASE\n".as_ptr() as *mut i8 as
                    *const i8, unsafe { *argv.offset(0 as isize) })
        };
        eprintln!("");
        eprintln!("Options are:");
        eprintln!("  -sql SQL   (analyze SQL statements passed as argument)");
        eprintln!("  -file FILE (read SQL statements from file FILE)");
        eprintln!("  -verbose LEVEL (integer verbosity level. default 1)");
        eprintln!("  -sample PERCENT (percent of db to sample. default 100)");
        unsafe { exit(-1) };
    }
}
extern "C" fn read_sql_from_file(p: *mut sqlite3expert, z_file_1: *const i8,
    ...
    if in_ == core::ptr::null_mut() {
        unsafe {
            *pz_err_1 =
                unsafe {
                    sqlite3_mprintf(c"failed to open file %s\n".as_ptr() as
                                *mut i8 as *const i8, z_file_1)
                }
        };
        return 1;
    }
    ...
    if n_read != 1 as u64 {
        ...
        unsafe {
            *pz_err_1 =
                unsafe {
                    sqlite3_mprintf(c"failed to read file %s\n".as_ptr() as
                                *mut i8 as *const i8, z_file_1)
                }
        };
        return 1;
    ...
    rc = unsafe { sqlite3_expert_sql(p, p_buf as *const i8, pz_err_1) };
    unsafe { sqlite3_free(p_buf as *mut ()) };
    return rc;
}
extern "C" fn __main_inner(argc: i32, argv: *mut *mut i8) -> Result<(), i32> {
    unsafe {
        ...
        z_db = unsafe { *argv.offset((argc - 1) as isize) } as *const i8;
        if unsafe { *z_db.offset(0 as isize) } as i32 == '-' as i32 {
            usage(argv as *const *mut i8);
        }
        rc = unsafe { sqlite3_open(z_db, &mut db) };
        if rc != 0 {
            unsafe {
                fprintf(__stderrp,
                    c"Cannot open db file: %s - %s\n".as_ptr() as *mut i8 as
                        *const i8, z_db, unsafe { sqlite3_errmsg(db) })
            };
            unsafe { exit(-2) };
        }
        p = unsafe { sqlite3_expert_new(db, &mut z_err) };
        if p == core::ptr::null_mut() {
            unsafe {
                fprintf(__stderrp,
                    c"Cannot run analysis: %s\n".as_ptr() as *mut i8 as
                        *const i8, z_err)
            };
            rc = 1;
        } else {
            {
                ...
                '__b0: loop {
                    ...
                    '__c0: loop {
                        ...
                        n_arg = unsafe { strlen(z_arg as *const i8) } as i32;
                        if n_arg >= 2 &&
                                0 ==
                                    unsafe {
                                        sqlite3_strnicmp(z_arg as *const i8,
                                            c"-file".as_ptr() as *mut i8 as *const i8, n_arg)
                                    } {
                            if { let __p = &mut i; *__p += 1; *__p } == argc - 1 {
                                option_requires_argument(c"-file".as_ptr() as *mut i8 as
                                        *const i8);
                            }
                            rc =
                                read_sql_from_file(p,
                                    unsafe { *argv.offset(i as isize) } as *const i8,
                                    &mut z_err);
                        } else if n_arg >= 3 &&
                                0 ==
                                    unsafe {
                                        sqlite3_strnicmp(z_arg as *const i8,
                                            c"-sql".as_ptr() as *mut i8 as *const i8, n_arg)
                                    } {
                            if { let __p = &mut i; *__p += 1; *__p } == argc - 1 {
                                option_requires_argument(c"-sql".as_ptr() as *mut i8 as
                                        *const i8);
                            }
                            rc =
                                unsafe {
                                ...
                                0 ==
                                    unsafe {
                                        sqlite3_strnicmp(z_arg as *const i8,
                                            c"-sample".as_ptr() as *mut i8 as *const i8, n_arg)
                                    } {
                            let mut i_sample: i32 = 0;
                            if { let __p = &mut i; *__p += 1; *__p } == argc - 1 {
                                option_requires_argument(c"-sample".as_ptr() as *mut i8 as
                                        *const i8);
                            }
                            i_sample =
                                option_integer_arg(unsafe { *argv.offset(i as isize) } as
                                        *const i8);
                            unsafe { sqlite3_expert_config(p, 1, i_sample) };
                        } else if n_arg >= 2 &&
                                0 ==
                                    unsafe {
                                        sqlite3_strnicmp(z_arg as *const i8,
                                            c"-verbose".as_ptr() as *mut i8 as *const i8, n_arg)
                                    } {
                            if { let __p = &mut i; *__p += 1; *__p } == argc - 1 {
                                option_requires_argument(c"-verbose".as_ptr() as *mut i8 as
                                        *const i8);
                            }
                            i_verbose =
                                option_integer_arg(unsafe { *argv.offset(i as isize) } as
                                        *const i8);
                        } else { usage(argv as *const *mut i8); }
                        break '__c0;
                    }
        ...
        if rc == 0 { rc = unsafe { sqlite3_expert_analyze(p, &mut z_err) }; }
        if rc == 0 {
            let n_query: i32 = unsafe { sqlite3_expert_count(p) };
            if i_verbose > 0 {
                let z_cand: *const i8 =
                    unsafe { sqlite3_expert_report(p, 0, 4) };
                unsafe {
                    fprintf(__stdoutp,
                        c"-- Candidates -------------------------------\n".as_ptr()
            ...
            }
            {
                i = 0;
                '__b1: loop {
                    if !(i < n_query) { break '__b1; }
                    '__c1: loop {
                        let z_sql: *const i8 =
                            unsafe { sqlite3_expert_report(p, i, 1) };
                        let mut z_idx: *const i8 =
                            unsafe { sqlite3_expert_report(p, i, 2) };
                        let z_eqp: *const i8 =
                            unsafe { sqlite3_expert_report(p, i, 3) };
        ...
        unsafe { sqlite3_expert_destroy(p) };
...
extern "C" {
    ...
    -> i32;
    fn sqlite3_expert_new(db: *mut sqlite3, pz_err_1: *mut *mut i8)
    -> *mut sqlite3expert;
    fn sqlite3_expert_config(p: *mut sqlite3expert, op: i32, ...)
    -> i32;
    fn sqlite3_expert_sql(p: *mut sqlite3expert, z_sql_1: *const i8,
    pz_err_1: *mut *mut i8)
    -> i32;
    fn sqlite3_expert_analyze(p: *mut sqlite3expert, pz_err_1: *mut *mut i8)
    -> i32;
    fn sqlite3_expert_count(_: *mut sqlite3expert)
    -> i32;
    fn sqlite3_expert_report(_: *mut sqlite3expert, i_stmt_1: i32,
    e_report_1: i32)
    -> *const i8;
