...
pub extern "C" fn sqlite3_thread_create(pp_thread_1: &mut *mut SQLiteThread,
    ...
    p_in_1: *mut ()) -> i32 {
    let mut p: *mut SQLiteThread = core::ptr::null_mut();
    let mut rc: i32 = 0;
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    *pp_thread_1 = core::ptr::null_mut();
    p =
        unsafe { sqlite3Malloc(core::mem::size_of::<SQLiteThread>() as u64) }
            as *mut SQLiteThread;
    if p == core::ptr::null_mut() { return 7; }
    unsafe {
        memset(p as *mut (), 0, core::mem::size_of::<SQLiteThread>() as u64)
    };
    unsafe { (*p).x_task = x_task_1 };
    unsafe { (*p).p_in = p_in_1 };
    if unsafe { sqlite3_fault_sim(200) } != 0 {
        rc = 1;
    } else {
        rc =
            unsafe {
                pthread_create(unsafe { &mut (*p).tid }, core::ptr::null(),
                    x_task_1.unwrap_or(unsafe {
                            core::mem::transmute::<*const (),
                                    unsafe extern "C" fn(*mut ()) -> *mut ()>(0 as *const ())
                        }), p_in_1)
            };
    }
    if rc != 0 {
        unsafe { (*p).done = 1 };
        unsafe { (*p).p_out = unsafe { x_task_1.unwrap()(p_in_1) } };
    }
    *pp_thread_1 = p;
    return 0;
}
...
pub extern "C" fn sqlite3_thread_join(p: *mut SQLiteThread,
    ...
    if p == core::ptr::null_mut() { return 7; }
    if unsafe { (*p).done } != 0 {
        unsafe { *pp_out_1 = unsafe { (*p).p_out } };
        rc = 0;
    } else {
        rc =
            if unsafe { pthread_join(unsafe { (*p).tid }, pp_out_1) } != 0 {
                1
            } else { 0 };
    }
