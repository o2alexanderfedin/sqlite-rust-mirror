...
struct Thread {
    z_filename: *mut i8,
    x_op: Option<unsafe extern "C" fn(*mut Thread) -> ()>,
    z_arg: *mut i8,
    opnum: i32,
    busy: i32,
    completed: i32,
...
extern "C" fn test_barrier() -> () {
    let p_mutex: *mut sqlite3_mutex = unsafe { sqlite3_mutex_alloc(8) };
...
extern "C" fn test_thread_main(p_arg_1: *mut ()) -> *mut () {
    ...
    unsafe {
        sqlite3_open(unsafe { (*p).z_filename } as *const i8,
            unsafe { &mut (*p).db })
    };
    if 0 != unsafe { sqlite3_errcode(unsafe { (*p).db }) } {
        unsafe {
            (*p).z_err =
                unsafe {
                    strdup(unsafe { sqlite3_errmsg(unsafe { (*p).db }) })
                }
        };
        unsafe { sqlite3_close(unsafe { (*p).db }) };
    ...
    test_barrier();
    unsafe { (*p).completed = 1 };
    while unsafe { (*p).opnum } <= unsafe { (*p).completed } {
        unsafe { sched_yield() };
    }
    test_barrier();
    while unsafe { (*p).x_op.is_some() } {
        if !(unsafe { (*p).z_err }).is_null() &&
                unsafe { (*p).z_err } != unsafe { (*p).z_static_err } {
            unsafe { sqlite3_free(unsafe { (*p).z_err } as *mut ()) };
            unsafe { (*p).z_err = core::ptr::null_mut() };
        }
        unsafe { (unsafe { (*p).x_op.unwrap() })(p) };
        test_barrier();
        {
            let __p = unsafe { &mut (*p).completed };
            let __t = *__p;
            *__p += 1;
            __t
        };
    ...
    if !(unsafe { (*p).p_stmt }).is_null() {
        unsafe { sqlite3_finalize(unsafe { (*p).p_stmt }) };
        unsafe { (*p).p_stmt = core::ptr::null_mut() };
    }
    if !(unsafe { (*p).db }).is_null() {
        unsafe { sqlite3_close(unsafe { (*p).db }) };
...
extern "C" fn parse_thread_id(interp: *mut Tcl_Interp, z_arg_1: *const i8)
    -> i32 {
    if z_arg_1 == core::ptr::null() ||
                    unsafe { *z_arg_1.offset(0 as isize) } as i32 == 0 ||
                unsafe { *z_arg_1.offset(1 as isize) } as i32 != 0 ||
            (unsafe {
                            isupper(unsafe { *z_arg_1.offset(0 as isize) } as u8 as i32)
                        } == 0) as i32 != 0 {
        unsafe {
            Tcl_AppendResult(interp,
                c"thread ID must be an upper case letter".as_ptr() as *mut i8,
                0 as *mut ())
        };
        return -1;
    }
...
extern "C" fn tcl_thread_create(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut x: pthread_t = core::ptr::null_mut();
        let mut rc: i32 = 0;
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    ...
                    c" ID FILENAME".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        if i < 0 { return 1; }
        if threadset[i as usize].busy != 0 {
            unsafe {
                Tcl_AppendResult(interp, c"thread ".as_ptr() as *mut i8,
                    unsafe { *argv.offset(1 as isize) },
                    c" is already running".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        threadset[i as usize].busy = 1;
        unsafe { sqlite3_free(threadset[i as usize].z_filename as *mut ()) };
        threadset[i as usize].z_filename =
            unsafe {
                sqlite3_mprintf(c"%s".as_ptr() as *mut i8 as *const i8,
                    unsafe { *argv.offset(2 as isize) })
            };
        threadset[i as usize].opnum = 1;
        threadset[i as usize].completed = 0;
        rc =
            unsafe {
                pthread_create(&mut x, core::ptr::null(), test_thread_main,
                    &raw mut threadset[i as usize] as *mut ())
            };
        if rc != 0 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"failed to create the thread".as_ptr() as *mut i8,
                    0 as *mut ())
            };
            unsafe {
                sqlite3_free(threadset[i as usize].z_filename as *mut ())
            };
            threadset[i as usize].busy = 0;
            return 1;
        }
        unsafe { pthread_detach(x) };
        return 0;
    }
}
...
extern "C" fn tcl_thread_wait(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 2 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" ID".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        if i < 0 { return 1; }
        if (threadset[i as usize].busy == 0) as i32 != 0 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"no such thread".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        test_thread_wait(&threadset[i as usize]);
        return 0;
    }
}
extern "C" fn test_stop_thread(p: *mut Thread) -> () {
    ...
    unsafe { (*p).x_op = None };
    { let __p = unsafe { &mut (*p).opnum }; let __t = *__p; *__p += 1; __t };
    test_thread_wait(unsafe { &*p });
    unsafe { sqlite3_free(unsafe { (*p).z_arg } as *mut ()) };
    unsafe { (*p).z_arg = core::ptr::null_mut() };
    unsafe { sqlite3_free(unsafe { (*p).z_filename } as *mut ()) };
    unsafe { (*p).z_filename = core::ptr::null_mut() };
    unsafe { (*p).busy = 0 };
}
extern "C" fn tcl_thread_halt(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 2 {
        ...
        if unsafe {
                        ...
                        } as i32 == 0 {
            {
                ...
                '__b4: loop {
                    ...
                    '__c4: loop {
                        if threadset[i as usize].busy != 0 {
                            test_stop_thread(&mut threadset[i as usize]);
                        }
                        break '__c4;
                    }
...
extern "C" fn tcl_thread_argc(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
...
extern "C" fn tcl_thread_argv(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut n: i32 = 0;
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" ID N".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        ...
        if unsafe {
                    Tcl_GetInt(interp, unsafe { *argv.offset(2 as isize) },
                        &mut n)
                } != 0 {
            return 1;
        }
        test_thread_wait(&threadset[i as usize]);
        if n < 0 || n >= threadset[i as usize].argc {
            unsafe {
                Tcl_AppendResult(interp,
                    c"column number out of range".as_ptr() as *mut i8,
                    0 as *mut ())
            };
            return 1;
        }
        unsafe {
            Tcl_AppendResult(interp, threadset[i as usize].argv[n as usize],
                0 as *mut ())
        };
        return 0;
    }
}
extern "C" fn tcl_thread_colname(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut n: i32 = 0;
        ...
        unsafe {
            Tcl_AppendResult(interp, threadset[i as usize].colv[n as usize],
                0 as *mut ())
        };
        return 0;
    }
}
extern "C" fn tcl_thread_result(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        ...
        test_thread_wait(&threadset[i as usize]);
        z_name = unsafe { sqlite3_err_name(threadset[i as usize].rc) };
        unsafe { Tcl_AppendResult(interp, z_name, 0 as *mut ()) };
        return 0;
    }
}
extern "C" fn tcl_thread_error(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 2 {
        ...
        test_thread_wait(&threadset[i as usize]);
        unsafe {
            Tcl_AppendResult(interp, threadset[i as usize].z_err,
                0 as *mut ())
        };
        return 0;
    }
...
extern "C" fn do_compile(p: *mut Thread) -> () {
            ...
            (*p).z_err =
                {
                    unsafe {
                        (*p).z_static_err =
                            c"no database is open".as_ptr() as *mut i8
                    };
                    unsafe { (*p).z_static_err }
                }
        };
        unsafe { (*p).rc = 1 };
        return;
    ...
    unsafe {
        (*p).rc =
            unsafe {
                sqlite3_prepare(unsafe { (*p).db },
                    unsafe { (*p).z_arg } as *const i8, -1,
                    unsafe { &mut (*p).p_stmt }, core::ptr::null_mut())
            }
    };
}
extern "C" fn tcl_thread_compile(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" ID SQL".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        ...
        test_thread_wait(&threadset[i as usize]);
        threadset[i as usize].x_op = Some(do_compile);
        unsafe { sqlite3_free(threadset[i as usize].z_arg as *mut ()) };
        threadset[i as usize].z_arg =
            unsafe {
                sqlite3_mprintf(c"%s".as_ptr() as *mut i8 as *const i8,
                    unsafe { *argv.offset(2 as isize) })
            };
        test_barrier();
        {
            let __p = &mut threadset[i as usize].opnum;
            let __t = *__p;
            *__p += 1;
            __t
        };
...
extern "C" fn do_step(p: *mut Thread) -> () {
            ...
            (*p).z_err =
                {
                    unsafe {
                        (*p).z_static_err =
                            c"no virtual machine available".as_ptr() as *mut i8
                    };
                    unsafe { (*p).z_static_err }
                }
        };
    ...
    unsafe { (*p).rc = unsafe { sqlite3_step(unsafe { (*p).p_stmt }) } };
    if unsafe { (*p).rc } == 100 {
        unsafe {
            (*p).argc =
        ...
        };
        {
            ...
            '__b5: loop {
                if !(i <
                                unsafe { sqlite3_data_count(unsafe { (*p).p_stmt }) }) {
                    break '__b5;
                }
                '__c5: loop {
                    unsafe {
                        (*p).argv[i as usize] =
                            unsafe { sqlite3_column_text(unsafe { (*p).p_stmt }, i) } as
                                    *mut i8 as *const i8
                    };
                    break '__c5;
                }
...
extern "C" fn tcl_thread_step(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 2 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" IDL".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        ...
        test_thread_wait(&threadset[i as usize]);
        threadset[i as usize].x_op = Some(do_step);
        test_barrier();
        {
            let __p = &mut threadset[i as usize].opnum;
            let __t = *__p;
...
extern "C" fn tcl_thread_finalize(not_used_1: *mut (),
    interp: *mut Tcl_Interp, argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 2 {
            unsafe {
        ...
        test_thread_wait(&threadset[i as usize]);
        threadset[i as usize].x_op = Some(do_finalize);
        unsafe { sqlite3_free(threadset[i as usize].z_arg as *mut ()) };
        threadset[i as usize].z_arg = core::ptr::null_mut();
        test_barrier();
        {
            let __p = &mut threadset[i as usize].opnum;
            let __t = *__p;
...
extern "C" fn tcl_thread_swap(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut j: i32 = 0;
        let mut temp: *mut sqlite3 = core::ptr::null_mut();
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" ID1 ID2".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        ...
        test_thread_wait(&threadset[i as usize]);
        j = parse_thread_id(interp, unsafe { *argv.offset(2 as isize) });
        if j < 0 { return 1; }
        if (threadset[j as usize].busy == 0) as i32 != 0 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"no such thread".as_ptr() as *mut i8, 0 as *mut ())
            };
        ...
        test_thread_wait(&threadset[j as usize]);
...
extern "C" fn tcl_thread_stmt_get(not_used_1: *mut (),
    interp: *mut Tcl_Interp, argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut z_buf: [i8; 100] = [0; 100];
        ...
        unsafe {
            sqlite3_test_make_pointer_str(interp,
                &raw mut z_buf[0 as usize] as *mut i8,
                threadset[i as usize].p_stmt as *mut ())
        };
        threadset[i as usize].p_stmt = core::ptr::null_mut();
        unsafe {
            Tcl_AppendResult(interp, &raw mut z_buf[0 as usize] as *mut i8,
                0 as *mut i8)
        };
...
extern "C" fn tcl_thread_db_put(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        if argc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    c"wrong # args: should be \"".as_ptr() as *mut i8,
                    unsafe { *argv.offset(0 as isize) },
                    c" ID DB".as_ptr() as *mut i8, 0 as *mut ())
            };
            return 1;
        }
        i = parse_thread_id(interp, unsafe { *argv.offset(1 as isize) });
        ...
        test_thread_wait(&threadset[i as usize]);
        { let _ = 0; };
        threadset[i as usize].db =
            unsafe {
                    sqlite3_test_text_to_ptr(unsafe {
...
extern "C" fn tcl_thread_db_get(not_used_1: *mut (), interp: *mut Tcl_Interp,
    argc: i32, argv: *mut *const i8) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        let mut z_buf: [i8; 100] = [0; 100];
        ...
        unsafe {
            sqlite3_test_make_pointer_str(interp,
                &raw mut z_buf[0 as usize] as *mut i8,
                threadset[i as usize].db as *mut ())
        };
        threadset[i as usize].db = core::ptr::null_mut();
        unsafe {
            Tcl_AppendResult(interp, &raw mut z_buf[0 as usize] as *mut i8,
                0 as *mut i8)
        };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlitetest4_init(interp: *mut Tcl_Interp) -> i32 {
    unsafe {
        let mut i: i32 = 0;
        {
            i = 0;
            '__b7: loop {
                if !((i as u64) <
                                core::mem::size_of::<[sqlitetest4_init_s0_N19sqlitetest4_init_s0; 15]>()
                                        as u64 / 16) {
                    break '__b7;
                }
                '__c7: loop {
...
#[repr(C)]
#[derive(Copy, Clone)]
struct sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
    z_name: *mut i8,
    x_proc: Option<unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
        *mut *const i8) -> i32>,
}
static mut a_cmd: [sqlitetest4_init_s0_N19sqlitetest4_init_s0; 15] =
    [sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_create".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_create as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_wait".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_wait as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_halt".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_halt as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_argc".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_argc as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_argv".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_argv as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_colname".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_colname as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_result".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_result as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_error".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_error as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_compile".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_compile as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_step".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_step as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_finalize".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_finalize as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_swap".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_swap as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_db_get".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_db_get as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_db_put".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
                                    *mut *const i8) -> i32>(tcl_thread_db_put as *const ())
                    }),
            },
            sqlitetest4_init_s0_N19sqlitetest4_init_s0 {
                z_name: c"thread_stmt_get".as_ptr() as *mut i8,
                x_proc: Some(unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn(*mut (), *mut Tcl_Interp, i32,
...
extern "C" {
    ...
    -> i32;
    fn pthread_detach(_: pthread_t)
    -> i32;
    fn Tcl_GetInt(interp: *mut Tcl_Interp, src: *const i8, intPtr: *mut i32)
    -> i32;
