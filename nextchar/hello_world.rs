...
extern "C" fn next_char_append(p: &mut nextCharContext, c: u32) -> () {
    let mut i: i32 = 0;
    {
        i = 0;
        '__b0: loop {
            if !(i < (*p).n_used) { break '__b0; }
            '__c0: loop {
                if unsafe { *(*p).a_result.offset(i as isize) } == c {
                    return;
                }
                break '__c0;
            }
    ...
    if (*p).n_used + 1 > (*p).n_alloc {
        let mut a_new: *mut u32 = core::ptr::null_mut();
        let n: i32 = (*p).n_alloc * 2 + 30;
        a_new =
            unsafe {
                    sqlite3_realloc64((*p).a_result as *mut (),
                        n as u64 * core::mem::size_of::<u32>() as u64)
                } as *mut u32;
        if a_new == core::ptr::null_mut() {
            (*p).malloc_failed = 1;
            return;
        } else { (*p).a_result = a_new; (*p).n_alloc = n; }
    }
    unsafe {
        *(*p).a_result.offset({
...
extern "C" fn write_utf8(z: *mut u8, c: u32) -> i32 {
    ...
    if c < 2048 as u32 {
        ...
        unsafe {
            *z.offset(1 as isize) = (128 + (c & 63 as u32) as u8 as i32) as u8
        };
        return 2;
    }
    if c < 65536 as u32 {
        ...
        unsafe {
            *z.offset(2 as isize) = (128 + (c & 63 as u32) as u8 as i32) as u8
        };
        return 3;
    }
    unsafe {
    ...
    unsafe {
        *z.offset(3 as isize) = (128 + (c & 63 as u32) as u8 as i32) as u8
    };
    return 4;
}
extern "C" fn read_utf8(z: *const u8, p_out_1: &mut u32) -> i32 {
    ...
    if c < 192 as u32 {
        *p_out_1 = c;
        ...
        let mut n: i32 = 1;
        c = valid_bits[(c - 192 as u32) as usize] as u32;
        while unsafe { *z.offset(n as isize) } as i32 & 192 == 128 {
            c =
                (c << 6) +
                    (63 &
                            unsafe {
...
extern "C" fn find_next_chars(p: *mut nextCharContext) -> () {
    ...
    let mut rc: i32 = 0;
    {
        '__b2: loop {
            '__c2: loop {
                unsafe {
                    sqlite3_bind_text(unsafe { (*p).p_stmt }, 1,
                        unsafe { (*p).z_prefix } as *mut i8 as *const i8,
                        unsafe { (*p).n_prefix }, None)
                };
                n =
                    write_utf8(&raw mut z_prev[0 as usize] as *mut u8,
                        c_prev + 1 as u32);
                unsafe {
                    sqlite3_bind_text(unsafe { (*p).p_stmt }, 2,
                        &raw mut z_prev[0 as usize] as *mut i8 as *const i8, n,
                        None)
                };
                ...
                if rc == 101 {
                    ...
                    unsafe { (*p).other_error = rc };
                    return;
                } else {
                    let z_out: *const u8 =
                        unsafe { sqlite3_column_text(unsafe { (*p).p_stmt }, 0) };
                    let mut c_next: u32 = 0 as u32;
                    if unsafe {
                                sqlite3_column_bytes(unsafe { (*p).p_stmt }, 0)
                            } <= unsafe { (*p).n_prefix } {
                        unsafe { sqlite3_reset(unsafe { (*p).p_stmt }) };
                        break '__c2;
                    }
                    n =
                        read_utf8(unsafe {
                                z_out.offset(unsafe { (*p).n_prefix } as isize)
                            }, &mut c_next);
                    unsafe { sqlite3_reset(unsafe { (*p).p_stmt }) };
                    next_char_append(unsafe { &mut *p }, c_next);
                    c_prev = c_next;
                    if unsafe { (*p).malloc_failed } != 0 { return; }
                }
                break '__c2;
            }
        }
...
extern "C" fn next_char_func(context: *mut sqlite3_context, argc: i32,
    argv: *mut *mut sqlite3_value) -> () {
    let mut c: nextCharContext = unsafe { core::mem::zeroed() };
    let z_table: *const u8 =
        unsafe { sqlite3_value_text(unsafe { *argv.offset(1 as isize) }) };
    let z_field: *const u8 =
        unsafe { sqlite3_value_text(unsafe { *argv.offset(2 as isize) }) };
    let mut z_where: *const u8 = core::ptr::null();
    let mut z_coll_name: *const u8 = core::ptr::null();
    let mut z_where_clause: *mut i8 = core::ptr::null_mut();
    let mut z_coll: *mut i8 = core::ptr::null_mut();
    let mut z_sql: *mut i8 = core::ptr::null_mut();
    let mut rc: i32 = 0;
    unsafe {
        memset(&raw mut c as *mut (), 0,
            core::mem::size_of::<nextCharContext>() as u64)
    };
    c.db = unsafe { sqlite3_context_db_handle(context) };
    c.z_prefix =
        unsafe { sqlite3_value_text(unsafe { *argv.offset(0 as isize) }) };
    c.n_prefix =
        unsafe { sqlite3_value_bytes(unsafe { *argv.offset(0 as isize) }) };
    if z_table == core::ptr::null() || z_field == core::ptr::null() ||
            c.z_prefix == core::ptr::null() {
        return;
    }
    if argc >= 4 &&
                {
            ...
            unsafe { *z_where.offset(0 as isize) } as i32 != 0 {
        z_where_clause =
            unsafe {
                sqlite3_mprintf(c"AND (%s)".as_ptr() as *mut i8 as *const i8,
                    z_where)
            };
        if z_where_clause == core::ptr::null_mut() {
            unsafe { sqlite3_result_error_nomem(context) };
            return;
        }
    } else { z_where_clause = c"".as_ptr() as *mut i8; }
    if argc >= 5 &&
                {
                        z_coll_name =
                            unsafe {
            ...
            unsafe { *z_coll_name.offset(0 as isize) } as i32 != 0 {
        z_coll =
            unsafe {
                sqlite3_mprintf(c"collate \"%w\"".as_ptr() as *mut i8 as
                        *const i8, z_coll_name)
            };
        if z_coll == core::ptr::null_mut() {
            unsafe { sqlite3_result_error_nomem(context) };
            if unsafe { *z_where_clause.offset(0 as isize) } != 0 {
                unsafe { sqlite3_free(z_where_clause as *mut ()) };
            }
            return;
        }
    } else { z_coll = c"".as_ptr() as *mut i8; }
    z_sql =
        unsafe {
            sqlite3_mprintf(c"SELECT %s FROM %s WHERE %s>=(?1 || ?2) %s   AND %s<=(?1 || char(1114111)) %s   %s ORDER BY 1 %s ASC LIMIT 1".as_ptr()
                        as *mut i8 as *const i8, z_field, z_table, z_field, z_coll,
                z_field, z_coll, z_where_clause, z_coll)
        };
    if unsafe { *z_where_clause.offset(0 as isize) } != 0 {
        unsafe { sqlite3_free(z_where_clause as *mut ()) };
    }
    if unsafe { *z_coll.offset(0 as isize) } != 0 {
        unsafe { sqlite3_free(z_coll as *mut ()) };
    }
    ...
    rc =
        unsafe {
            sqlite3_prepare_v2(c.db, z_sql as *const i8, -1, &mut c.p_stmt,
                core::ptr::null_mut())
        };
    unsafe { sqlite3_free(z_sql as *mut ()) };
    if rc != 0 {
        unsafe {
            sqlite3_result_error(context, unsafe { sqlite3_errmsg(c.db) }, -1)
        };
        return;
    }
    find_next_chars(&mut c);
    if c.malloc_failed != 0 {
        unsafe { sqlite3_result_error_nomem(context) };
    } else {
        let mut p_res: *mut u8 = core::ptr::null_mut();
        p_res =
            unsafe { sqlite3_malloc64((c.n_used * 4 + 1) as sqlite3_uint64) }
                as *mut u8;
        if p_res == core::ptr::null_mut() {
            unsafe { sqlite3_result_error_nomem(context) };
        } else {
            let mut i: i32 = 0;
            let mut n: i32 = 0;
            {
                i = 0;
                '__b3: loop {
                    if !(i < c.n_used) { break '__b3; }
                    '__c3: loop {
                        n +=
                            write_utf8(unsafe { p_res.offset(n as isize) },
                                unsafe { *c.a_result.offset(i as isize) });
                        break '__c3;
                    }
                    { let __p = &mut i; let __t = *__p; *__p += 1; __t };
                }
            }
            unsafe { *p_res.offset(n as isize) = 0 as u8 };
            unsafe {
                sqlite3_result_text(context, p_res as *const i8, n,
                    Some(sqlite3_free))
            };
        }
    }
...
pub extern "C" fn sqlite3_nextchar_init(db: *mut sqlite3,
    ...
    if rc == 0 {
        rc =
            unsafe {
                sqlite3_create_function(db,
                    c"next_char".as_ptr() as *mut i8 as *const i8, 4,
                    1 | 2097152, core::ptr::null_mut(), Some(next_char_func),
                    None, None)
            };
    }
    if rc == 0 {
        rc =
            unsafe {
                sqlite3_create_function(db,
                    c"next_char".as_ptr() as *mut i8 as *const i8, 5,
                    1 | 2097152, core::ptr::null_mut(), Some(next_char_func),
                    None, None)
            };
    }
