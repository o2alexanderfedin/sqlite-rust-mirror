...
pub extern "C" fn sqlite3_hash_init(p_new: &mut Hash) -> () {
    { let _ = 0; };
    (*p_new).first = core::ptr::null_mut();
    (*p_new).count = 0 as u32;
...
extern "C" fn find_element_with_hash(p_h_1: &Hash, p_key_1: *const i8,
    ...
    unsafe {
        ...
        h = str_hash(p_key_1);
        if !((*p_h_1).ht).is_null() {
            let mut p_entry: *const _ht = core::ptr::null();
            p_entry =
                unsafe {
                    (*p_h_1).ht.add((h % (*p_h_1).htsize as u32) as usize)
                };
            elem = unsafe { (*p_entry).chain };
            count = unsafe { (*p_entry).count };
        } else { elem = (*p_h_1).first; count = (*p_h_1).count as u32; }
        if !(p_hash_1).is_null() { unsafe { *p_hash_1 = h }; }
        while count != 0 {
            { let _ = 0; };
            if h == unsafe { (*elem).h } &&
                    unsafe {
                            sqlite3_str_i_cmp(unsafe { (*elem).p_key }, p_key_1)
                        } == 0 {
                return elem;
            }
            elem = unsafe { (*elem).next };
            { let __p = &mut count; let __t = *__p; *__p -= 1; __t };
        }
...
pub extern "C" fn sqlite3_hash_clear(p_h: &mut Hash) -> () {
    ...
    unsafe { sqlite3_free((*p_h).ht as *mut ()) };
    (*p_h).ht = core::ptr::null_mut();
    (*p_h).htsize = 0 as u32;
    while !(elem).is_null() {
        let next_elem: *mut HashElem = unsafe { (*elem).next };
...
extern "C" fn remove_element(p_h_1: *mut Hash, elem: *mut HashElem) -> () {
    let mut p_entry: *mut _ht = core::ptr::null_mut();
    if !(unsafe { (*elem).prev }).is_null() {
        unsafe { (*unsafe { (*elem).prev }).next = unsafe { (*elem).next } };
    } else { unsafe { (*p_h_1).first = unsafe { (*elem).next } }; }
    if !(unsafe { (*elem).next }).is_null() {
        unsafe { (*unsafe { (*elem).next }).prev = unsafe { (*elem).prev } };
    }
    if !(unsafe { (*p_h_1).ht }).is_null() {
        p_entry =
            unsafe {
                unsafe {
                    (*p_h_1).ht.add((unsafe { (*elem).h } %
                                unsafe { (*p_h_1).htsize }) as usize)
                }
            };
        if unsafe { (*p_entry).chain } == elem {
            unsafe { (*p_entry).chain = unsafe { (*elem).next } };
        }
        { let _ = 0; };
        {
            let __p = unsafe { &mut (*p_entry).count };
            let __t = *__p;
            *__p -= 1;
            __t
        };
    ...
    unsafe { sqlite3_free(elem as *mut ()) };
    {
        let __p = unsafe { &mut (*p_h_1).count };
        let __t = *__p;
        *__p -= 1;
        __t
    };
    if unsafe { (*p_h_1).count } == 0 as u32 {
        { let _ = 0; };
        { let _ = 0; };
        sqlite3_hash_clear(unsafe { &mut *p_h_1 });
    }
}
extern "C" fn insert_element(p_h_1: &mut Hash, p_entry_1: *mut _ht,
    p_new_1: *mut HashElem) -> () {
    let mut p_head: *mut HashElem = core::ptr::null_mut();
    if !(p_entry_1).is_null() {
        p_head =
            if unsafe { (*p_entry_1).count } != 0 {
                unsafe { (*p_entry_1).chain }
            } else { core::ptr::null_mut() };
        {
            let __p = unsafe { &mut (*p_entry_1).count };
            let __t = *__p;
            *__p += 1;
            __t
        };
        unsafe { (*p_entry_1).chain = p_new_1 };
    } else { p_head = core::ptr::null_mut(); }
    if !(p_head).is_null() {
        unsafe { (*p_new_1).next = p_head };
        unsafe { (*p_new_1).prev = unsafe { (*p_head).prev } };
        if !(unsafe { (*p_head).prev }).is_null() {
            unsafe { (*unsafe { (*p_head).prev }).next = p_new_1 };
        } else { (*p_h_1).first = p_new_1; }
        unsafe { (*p_head).prev = p_new_1 };
    } else {
        unsafe { (*p_new_1).next = (*p_h_1).first };
        if !((*p_h_1).first).is_null() {
            unsafe { (*(*p_h_1).first).prev = p_new_1 };
        }
        unsafe { (*p_new_1).prev = core::ptr::null_mut() };
        (*p_h_1).first = p_new_1;
    }
}
extern "C" fn rehash(p_h_1: *mut Hash, mut new_size: u32) -> i32 {
    let mut new_ht: *mut _ht = core::ptr::null_mut();
    let mut elem: *mut HashElem = core::ptr::null_mut();
    let mut next_elem: *mut HashElem = core::ptr::null_mut();
    if new_size as u64 * core::mem::size_of::<_ht>() as u64 > 1024 as u64 {
        new_size = (1024 as u64 / core::mem::size_of::<_ht>() as u64) as u32;
    }
    if new_size == unsafe { (*p_h_1).htsize } { return 0; }
    unsafe { sqlite3_begin_benign_malloc() };
    new_ht =
        unsafe {
                sqlite3Malloc(new_size as u64 *
                        core::mem::size_of::<_ht>() as u64)
            } as *mut _ht;
    unsafe { sqlite3_end_benign_malloc() };
    if new_ht == core::ptr::null_mut() { return 0; }
    unsafe { sqlite3_free(unsafe { (*p_h_1).ht } as *mut ()) };
    unsafe { (*p_h_1).ht = new_ht };
    unsafe {
        (*p_h_1).htsize =
            {
                new_size =
                    (unsafe { sqlite3_malloc_size(new_ht as *const ()) } as u64
                            / core::mem::size_of::<_ht>() as u64) as u32;
                new_size
            }
    };
    unsafe {
        memset(new_ht as *mut (), 0,
            new_size as u64 * core::mem::size_of::<_ht>() as u64)
    };
    {
        {
            elem = unsafe { (*p_h_1).first };
            unsafe { (*p_h_1).first = core::ptr::null_mut() }
        };
        '__b3: loop {
            if !(!(elem).is_null()) { break '__b3; }
            '__c3: loop {
                next_elem = unsafe { (*elem).next };
                insert_element(unsafe { &mut *p_h_1 },
                    unsafe {
                        &mut *new_ht.add((unsafe { (*elem).h } % new_size) as usize)
                    }, elem);
                break '__c3;
            }
            elem = next_elem;
...
pub extern "C" fn sqlite3_hash_insert(p_h: *mut Hash, p_key: *const i8,
    ...
    if !(unsafe { (*elem).data }).is_null() {
        let old_data: *mut () = unsafe { (*elem).data };
        if data == core::ptr::null_mut() {
            remove_element(p_h, elem);
        } else {
            unsafe { (*elem).data = data };
            unsafe { (*elem).p_key = p_key };
        }
        return old_data;
    }
    if data == core::ptr::null_mut() { return core::ptr::null_mut(); }
    new_elem =
        unsafe { sqlite3Malloc(core::mem::size_of::<HashElem>() as u64) } as
            *mut HashElem;
    if new_elem == core::ptr::null_mut() { return data; }
    unsafe { (*new_elem).p_key = p_key };
    unsafe { (*new_elem).h = h };
    unsafe { (*new_elem).data = data };
    {
        let __p = unsafe { &mut (*p_h).count };
        let __t = *__p;
        *__p += 1;
        __t
    };
    if unsafe { (*p_h).count } >= 5 as u32 &&
            unsafe { (*p_h).count } > 2 as u32 * unsafe { (*p_h).htsize } {
        rehash(p_h, unsafe { (*p_h).count } * 3 as u32);
    }
    insert_element(unsafe { &mut *p_h },
        if !(unsafe { (*p_h).ht }).is_null() {
            unsafe {
                &mut *unsafe {
                            (*p_h).ht.add((unsafe { (*new_elem).h } %
                                        unsafe { (*p_h).htsize }) as usize)
                        }
            }
...
pub extern "C" fn sqlite3_hash_find(p_h: *const Hash, p_key: *const i8)
    ...
    { let _ = 0; };
    return unsafe {
            (*find_element_with_hash(unsafe { &*p_h }, p_key,
