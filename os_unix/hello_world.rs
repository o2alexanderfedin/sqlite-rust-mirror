...
#[derive(Copy, Clone)]
struct unixFile {
    p_method: *const sqlite3_io_methods,
    p_vfs: *mut sqlite3_vfs,
    ...
    sz_chunk: i32,
    n_fetch_out: i32,
    mmap_size: sqlite3_int64,
    mmap_size_actual: sqlite3_int64,
    mmap_size_max: sqlite3_int64,
    p_map_region: *mut (),
    sector_size: i32,
...
struct unixInodeInfo {
    file_id: unixFileId,
    p_lock_mutex: *mut sqlite3_mutex,
    n_shared: i32,
    n_lock: i32,
    e_file_lock: u8,
    b_process_lock: u8,
...
extern "C" fn robust_open(z: *const i8, f: i32, m: mode_t) -> i32 {
    unsafe {
        let mut fd: i32 = 0;
        let m2: mode_t = if m != 0 { m as i32 } else { 420 } as mode_t;
        loop {
            fd =
                unsafe {
                    (unsafe {
...
static mut a_syscall: [unix_syscall; 29] =
    [unix_syscall {
    ...
    unsafe {
        ...
        if (*p_file_1).ctrl_flags as i32 & (1 | 2) == 1 {
            if unsafe { (*p_inode).b_process_lock } as i32 == 0 {
                ...
                lock.l_start = (sqlite3_pending_byte + 2) as off_t;
                lock.l_len = 510 as off_t;
...
extern "C" fn posix_unlock(id: *mut sqlite3_file, e_file_lock_1: i32,
            ...
            '__c4: loop {
                ...
                if unsafe { (*p_file).e_file_lock } as i32 > 1 {
                    ...
                    if e_file_lock_1 == 1 {
                        if handle_nfs_unlock_1 != 0 {
                            ...
                            let div_size: off_t = (510 - 1) as off_t;
                            ...
                            lock.l_whence = 0 as i16;
                            lock.l_start =
                                (sqlite3_pending_byte + 2) as off_t + div_size;
                            lock.l_len = 510 as off_t - div_size;
                            if unix_file_lock(unsafe { &*p_file }, &mut lock) == -1 {
                                t_errno = unsafe { *unsafe { __error() } };
                                rc = 10 | 8 << 8;
                                store_last_errno(unsafe { &mut *p_file }, t_errno);
                    ...
                    lock.l_type = 2 as i16;
                    lock.l_whence = 0 as i16;
                    lock.l_start = sqlite3_pending_byte as off_t;
                    lock.l_len = 2 as off_t;
                    { let _ = 0; };
                    if unix_file_lock(unsafe { &*p_file }, &mut lock) == 0 {
                ...
                if e_file_lock_1 == 0 {
                    ...
                    if unsafe { (*p_inode).n_shared } == 0 {
                        lock.l_type = 2 as i16;
                        lock.l_whence = 0 as i16;
                        lock.l_start = { lock.l_len = 0 as off_t; lock.l_len };
                        if unix_file_lock(unsafe { &*p_file }, &mut lock) == 0 {
...
extern "C" fn unix_unlock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    { let _ = 0; };
    return posix_unlock(id, e_file_lock_1, 0);
}
static mut unix_big_lock: *mut sqlite3_mutex = core::ptr::null_mut();
...
extern "C" fn unix_unmapfile(p_fd_1: &mut unixFile) -> () {
    unsafe {
        ...
        if !((*p_fd_1).p_map_region).is_null() {
            ...
            (*p_fd_1).mmap_size = 0 as sqlite3_int64;
            (*p_fd_1).mmap_size_actual = 0 as sqlite3_int64;
        }
    }
}
extern "C" fn close_unix_file(id: *mut sqlite3_file) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
...
extern "C" fn unix_close(id: *mut sqlite3_file) -> i32 {
    let mut rc: i32 = 0;
    let p_file: *mut unixFile = id as *mut unixFile;
...
extern "C" fn seek_and_read(id: *mut unixFile, mut offset: sqlite3_int64,
...
extern "C" fn unix_read(id: *mut sqlite3_file, mut p_buf_1: *mut (),
    mut amt: i32, mut offset: sqlite3_int64) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
    let mut got: i32 = 0;
    { let _ = 0; };
    { let _ = 0; };
    ...
    if offset < unsafe { (*p_file).mmap_size } {
        if offset + amt as sqlite3_int64 <= unsafe { (*p_file).mmap_size } {
            unsafe {
                memcpy(p_buf_1,
                    unsafe {
            ...
            p_buf_1 =
                unsafe { (p_buf_1 as *mut u8).offset(n_copy as isize) } as
                    *mut ();
            amt -= n_copy;
            offset += n_copy as sqlite3_int64;
        }
    }
...
extern "C" fn unix_write(id: *mut sqlite3_file, mut p_buf_1: *const (),
    mut amt: i32, mut offset: sqlite3_int64) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
    let mut wrote: i32 = 0;
    { let _ = 0; };
...
extern "C" fn robust_ftruncate(h: i32, sz: sqlite3_int64) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        '__b9: loop {
            '__c9: loop {
...
extern "C" fn unix_truncate(id: *mut sqlite3_file, mut n_byte_1: i64) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
    let mut rc: i32 = 0;
    { let _ = 0; };
...
extern "C" fn unix_sync(id: *mut sqlite3_file, flags: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let p_file: *mut unixFile = id as *mut unixFile;
...
extern "C" fn unix_file_size(id: *mut sqlite3_file, p_size_1: *mut i64)
    -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let mut buf: stat = unsafe { core::mem::zeroed() };
...
extern "C" fn unix_is_sharing_shm_node(p_file_1: &unixFile) -> i32 {
    unsafe {
        ...
        lock.l_whence = 0 as i16;
        lock.l_start = ((22 + 8) * 4 + 8) as off_t;
        lock.l_len = 1 as off_t;
        lock.l_type = 3 as i16;
        unsafe {
            (unsafe {
                    core::mem::transmute::<*const (),
...
extern "C" fn unix_lock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let mut p_inode: *mut unixInodeInfo = core::ptr::null_mut();
        '__b10: loop {
            '__c10: loop {
                ...
                if e_file_lock_1 == 1 {
                    ...
                    { let _ = 0; };
                    if e_file_lock_1 == 2 {
                        lock.l_start = (sqlite3_pending_byte + 1) as off_t;
                        lock.l_len = 1 as off_t;
                    } else {
                        lock.l_start = (sqlite3_pending_byte + 2) as off_t;
                        lock.l_len = 510 as off_t;
...
extern "C" fn unix_check_reserved_lock(id: *mut sqlite3_file,
...
extern "C" fn unix_remapfile(p_fd_1: &mut unixFile, mut n_new_1: i64) -> () {
    unsafe {
        ...
        if !(p_orig).is_null() {
            ...
            let n_reuse: i64 =
                (*p_fd_1).mmap_size & !(sz_syspage - 1) as sqlite3_int64;
            let p_req: *mut u8 =
        ...
        if p_new as *mut () == -1i32 as *mut () {
            ...
            (*p_fd_1).mmap_size_max = 0 as sqlite3_int64;
        }
    ...
    unsafe {
        if unsafe { (*p_file_1).sz_chunk } > 0 {
            ...
            if n_size > buf.st_size as i64 {
                ...
                i_write =
                    buf.st_size / n_blk as off_t * n_blk as off_t +
                            n_blk as off_t - 1 as off_t;
                { let _ = 0; };
                { let _ = 0; };
                {
                    '__b11: loop {
...
struct proxyLockingContext {
    ...
    p_old_method: *const sqlite3_io_methods,
}
        ...
        rc =
            unsafe {
                (unsafe {
                    ...
                    })(conch_file as *mut sqlite3_file, 0)
            };
    }
    unsafe { (*p_ctx).conch_held = 0 };
    return rc;
}
extern "C" fn proxy_close(id: *mut sqlite3_file) -> i32 {
    if !(id).is_null() {
        let p_file: *mut unixFile = id as *mut unixFile;
            ...
            rc =
                unsafe {
                    (unsafe {
                        ...
                        })(lock_proxy as *mut sqlite3_file, 0)
                };
            if rc != 0 { return rc; }
            rc =
                unsafe {
                    (unsafe {
                        ...
                        })(lock_proxy as *mut sqlite3_file)
                };
            if rc != 0 { return rc; }
            ...
            rc =
                unsafe {
                    (unsafe {
                        ...
                        })(conch_file as *mut sqlite3_file)
                };
            if rc != 0 { return rc; }
...
extern "C" fn unix_sleep(not_used: *mut sqlite3_vfs, microseconds: i32)
    -> i32 {
    let mut sp: timespec = unsafe { core::mem::zeroed() };
    sp.tv_sec = (microseconds / 1000000) as __darwin_time_t;
            ...
            '__c14: loop {
                ...
                fd =
                    robust_open(&raw mut t_path[0 as usize] as *mut i8 as
                            *const i8, 2 | 512 | 2048 | 256, 0 as mode_t);
                if fd < 0 {
                    unsafe {
                        sqlite3_snprintf(core::mem::size_of::<[i8; 64]>() as i32,
                            &raw mut errmsg[0 as usize] as *mut i8,
                ...
                rc =
                    unsafe {
                        (unsafe {
                            ...
                            })(conch_file as *mut sqlite3_file, lock_type_1)
                    };
                        ...
                        if lock_type_1 == 4 {
                            rc =
                                unsafe {
                                    (unsafe {
                                            (*unsafe { (*conch_file).p_method }).x_lock.unwrap()
                                        })(conch_file as *mut sqlite3_file, 1)
                                };
                        }
                        if (rc == 0) as i32 != 0 {
                            rc =
...
extern "C" fn nolock_close(id: *mut sqlite3_file) -> i32 {
    return close_unix_file(id);
}
extern "C" fn nolock_lock(not_used_1: *mut sqlite3_file, not_used2_1: i32)
    -> i32 {
    { { let _ = not_used_1; }; { let _ = not_used2_1; } };
    return 0;
}
extern "C" fn nolock_unlock(not_used_1: *mut sqlite3_file, not_used2_1: i32)
    -> i32 {
    { { let _ = not_used_1; }; { let _ = not_used2_1; } };
    return 0;
}
extern "C" fn nolock_check_reserved_lock(not_used_1: *mut sqlite3_file,
    p_res_out_1: *mut i32) -> i32 {
    { let _ = not_used_1; };
...
extern "C" fn unix_sector_size(id: *mut sqlite3_file) -> i32 {
    let p_fd: *mut unixFile = id as *mut unixFile;
    set_device_characteristics(unsafe { &mut *p_fd });
    return unsafe { (*p_fd).sector_size };
}
extern "C" fn unix_device_characteristics(id: *mut sqlite3_file) -> i32 {
    let p_fd: *mut unixFile = id as *mut unixFile;
    set_device_characteristics(unsafe { &mut *p_fd });
...
extern "C" fn unix_shm_lock(fd: *mut sqlite3_file, ofst: i32, n: i32,
    flags: i32) -> i32 {
...
extern "C" fn unix_shm_barrier(fd: *mut sqlite3_file) -> () {
    { let _ = fd; };
...
extern "C" fn unix_shm_unmap(fd: *mut sqlite3_file, delete_flag_1: i32)
    -> i32 {
    unsafe {
        let mut p: *mut unixShm = core::ptr::null_mut();
        let mut p_shm_node: *mut unixShmNode = core::ptr::null_mut();
...
extern "C" fn unix_fetch(fd: *mut sqlite3_file, i_off_1: i64, n_amt_1: i32,
    pp: *mut *mut ()) -> i32 {
...
extern "C" fn unix_unfetch(fd: *mut sqlite3_file, i_off_1: i64, p: *mut ())
    -> i32 {
    let p_fd: *mut unixFile = fd as *mut unixFile;
    { let _ = i_off_1; };
    { let _ = 0; };
...
static nolock_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 3,
...
type finder_type =
    unsafe extern "C" fn(*const i8, *mut unixFile)
        -> *const sqlite3_io_methods;
extern "C" fn nfs_unlock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    return posix_unlock(id, e_file_lock_1, 1);
}
static nfs_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
...
extern "C" fn afp_set_lock(path: *const i8, p_file_1: *mut unixFile,
    ...
    err =
        unsafe {
            fsctl(path,
                (2147483648u32 as __uint32_t | 1073741824 as __uint32_t) as
                                u64 |
...
extern "C" fn afp_unlock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let p_file: *mut unixFile = id as *mut unixFile;
        let mut p_inode: *mut unixInodeInfo = core::ptr::null_mut();
...
extern "C" fn afp_close(id: *mut sqlite3_file) -> i32 {
    let mut rc: i32 = 0;
    let p_file: *mut unixFile = id as *mut unixFile;
    { let _ = 0; };
    afp_unlock(id, 0);
...
extern "C" fn afp_lock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let p_file: *mut unixFile = id as *mut unixFile;
...
extern "C" fn afp_check_reserved_lock(id: *mut sqlite3_file,
    p_res_out_1: *mut i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let mut reserved: i32 = 0;
...
static afp_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
...
extern "C" fn dotlock_unlock(id: *mut sqlite3_file, e_file_lock_1: i32)
    -> i32 {
    unsafe {
        let p_file: *mut unixFile = id as *mut unixFile;
...
extern "C" fn dotlock_close(id: *mut sqlite3_file) -> i32 {
...
extern "C" fn dotlock_lock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    unsafe {
        let p_file: *mut unixFile = id as *mut unixFile;
        let z_lock_file: *mut i8 =
            unsafe { (*p_file).locking_context } as *mut i8;
...
extern "C" fn dotlock_check_reserved_lock(id: *mut sqlite3_file,
    p_res_out_1: *mut i32) -> i32 {
    unsafe {
        let p_file: *const unixFile = id as *mut unixFile as *const unixFile;
        if unsafe { (*p_file).e_file_lock } as i32 >= 1 {
...
static dotlock_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
...
extern "C" fn fill_in_unix_file(p_vfs_1: *mut sqlite3_vfs, mut h: i32,
    p_id_1: *mut sqlite3_file, z_filename_1: *const i8, ctrl_flags_1: i32)
    -> i32 {
    unsafe {
        let mut p_locking_style: *const sqlite3_io_methods =
            core::ptr::null();
                            ...
                            *unsafe {
                                    core::mem::transmute::<*const (),
                                            *mut unsafe extern "C" fn(*const i8, *mut unixFile)
                                                ->
                                                    *const sqlite3_io_methods>(unsafe { (*p_vfs_1).p_app_data }
                                            as *const ())
                                }
                        })(z_filename_1, p_new)
                };
        ...
        if p_locking_style ==
                    &raw const posix_io_methods as *const sqlite3_io_methods ||
                p_locking_style ==
                    &raw const nfs_io_methods as *const sqlite3_io_methods {
            unix_enter_mutex();
            ...
            unix_leave_mutex();
                ...
                &raw const afp_io_methods as *const sqlite3_io_methods {
...
extern "C" fn proxy_create_unix_file(path: *const i8,
    ...
    let mut dummy_vfs: sqlite3_vfs = unsafe { core::mem::zeroed() };
    ...
    loop {
        ...
        '__s28:
            ...
            match __state {
                ...
                17 => {
                    fd = robust_open(path, open_flags, 0 as mode_t);
                    __state = 18;
                }
                18 => {
                ...
                41 => {
                    unsafe {
                        memset(&raw mut dummy_vfs as *mut (), 0,
                            core::mem::size_of::<sqlite3_vfs>() as u64)
                    };
                    __state = 42;
                }
                42 => {
                ...
                47 => {
                    rc =
                        fill_in_unix_file(&mut dummy_vfs, fd,
                            p_new as *mut sqlite3_file, path, 0);
                    __state = 48;
                }
...
extern "C" fn proxy_take_conch(p_file_1: *mut unixFile) -> i32 {
    unsafe {
        ...
        if unsafe { (*p_ctx).conch_held } != 0 {
            ...
            let mut conch_file: *mut unixFile = core::ptr::null_mut();
            let mut my_host_id: uuid_t = [0; 16];
            ...
            let mut buf: stat = unsafe { core::mem::zeroed() };
            let mut err: i32 = 0;
            let mut cmode: mode_t = 0 as mode_t;
            let mut fd: i32 = 0;
                    ...
                    match __state {
                        ...
                        66 => {
                            robust_ftruncate(unsafe { (*conch_file).h },
                                write_size as sqlite3_int64);
                            __state = 67;
                        }
                        67 => {
                            rc =
                                unix_write(conch_file as *mut sqlite3_file,
                                    &raw mut write_buffer[0 as usize] as *mut i8 as *const (),
                                    write_size, 0 as sqlite3_int64);
                            __state = 68;
                        }
                        68 => {
                        ...
                        82 => {
                            fd =
                                robust_open(unsafe { (*p_ctx).db_path } as *const i8,
                                    unsafe { (*p_file_1).open_flags }, 0 as mode_t);
                            __state = 83;
                        }
                        83 => { __state = 84; }
                        84 => {
...
extern "C" fn proxy_lock(id: *mut sqlite3_file, e_file_lock: i32) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
            ...
            rc =
                unsafe {
                    (unsafe {
                        ...
                        })(proxy as *mut sqlite3_file, e_file_lock)
                };
            unsafe {
...
extern "C" fn proxy_unlock(id: *mut sqlite3_file, e_file_lock: i32) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
    let mut rc: i32 = proxy_take_conch(p_file);
    if rc == 0 {
        let p_ctx: *const proxyLockingContext =
...
extern "C" fn proxy_check_reserved_lock(id: *mut sqlite3_file,
        ...
        if unsafe { (*p_ctx).conch_held } > 0 {
            ...
            return unsafe {
                    (unsafe {
                        ...
                        })(proxy as *mut sqlite3_file, p_res_out)
                };
        } else { p_res_out = core::ptr::null_mut(); }
    }
    return rc;
}
static proxy_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
...
extern "C" fn proxy_create_conch_pathname(db_path_1: *const i8,
    ...
    *p_conch_path_1 =
        {
            conch_path =
                unsafe { sqlite3_malloc64((len + 8) as sqlite3_uint64) } as
                    *mut i8;
            conch_path
        };
            ...
            if !(unsafe { (*p_ctx).conch_file }).is_null() {
                unsafe {
                    (unsafe {
                        ...
                        })(unsafe { (*p_ctx).conch_file } as *mut sqlite3_file)
                };
                unsafe {
...
extern "C" fn proxy_file_control(id: *mut sqlite3_file, op: i32,
    p_arg: *mut ()) -> i32 {
    '__s34:
        {
        match op {
            2 => {
                {
                    ...
                    if unsafe { (*p_file).p_method } ==
                            &raw const proxy_io_methods as *const sqlite3_io_methods {
                        let p_ctx: *const proxyLockingContext =
                            unsafe { (*p_file).locking_context } as
                ...
                }
                {
                    ...
                    let is_proxy_style: i32 =
                        (unsafe { (*p_file_1).p_method } ==
                                &raw const proxy_io_methods as *const sqlite3_io_methods) as
                            i32;
                    if p_arg == 0 as *mut () ||
    ...
    unsafe {
        ...
        *pi_out_1 = 0;
        if !((*p_file_1).p_shm).is_null() {
            ...
            f.l_type = 3 as i16;
            f.l_whence = 0 as i16;
            f.l_start = ((22 + 8) * 4 + 3) as off_t;
            f.l_len = (8 - 3) as off_t;
            unsafe {
...
extern "C" fn unix_file_control(id: *mut sqlite3_file, op: i32,
    p_arg_1: *mut ()) -> i32 {
    unsafe {
        let p_file: *mut unixFile = id as *mut unixFile;
        '__s35:
...
extern "C" fn robust_fchown(fd: i32, uid: uid_t, gid: gid_t) -> i32 {
    unsafe {
        return if unsafe {
                        (unsafe {
                                core::mem::transmute::<*const (),
            ...
            if (*p_shm_node_1).is_readonly != 0 {
                ...
                rc =
                    unix_shm_system_lock(unsafe { &*p_db_fd_1 }, 3,
                        (22 + 8) * 4 + 8, 1);
                if rc == 0 &&
                        robust_ftruncate((*p_shm_node_1).h_shm, 3 as sqlite3_int64)
                            != 0 {
                    rc =
                            ...
                            unsafe {
                                (*p_shm_node).h_shm =
                                    robust_open(z_shm as *const i8, 2 | 512 | 256,
                                        (s_stat.st_mode as i32 & 511) as mode_t)
                            };
                        }
                        if unsafe { (*p_shm_node).h_shm } < 0 {
                            unsafe {
...
extern "C" fn unix_shm_map(fd: *mut sqlite3_file, i_region_1: i32,
        ...
        loop {
            ...
            '__s38:
                ...
                match __state {
                    ...
                    42 => {
                        i_pg = s_stat.st_size / pgsz_1 as off_t;
                        __state = 43;
                    }
                    43 => {
                    ...
                    61 => {
                        p_mem =
                            unsafe { sqlite3_malloc64(n_map as sqlite3_uint64) };
                        __state = 65;
                    }
                    62 => {
...
static posix_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 3,
        x_close: Some(unix_close),
        x_read: Some(unix_read),
...
extern "C" fn autolock_io_finder_impl(file_path_1: *const i8,
    p_new_1: *mut unixFile) -> *const sqlite3_io_methods {
    unsafe {
        let mut i: i32 = 0;
        let mut fs_info: statfs = unsafe { core::mem::zeroed() };
...
static mut randomness_pid: pid_t = 0;
extern "C" fn get_file_mode(z_file_1: *const i8, p_mode_1: *mut mode_t,
    p_uid_1: *mut uid_t, p_gid_1: *mut gid_t) -> i32 {
    unsafe {
        let mut s_stat: stat = unsafe { core::mem::zeroed() };
        let mut rc: i32 = 0;
        if 0 ==
            ...
            unsafe { *p_mode_1 = (s_stat.st_mode as i32 & 511) as mode_t };
...
extern "C" fn find_create_file_mode(z_path_1: *const i8, flags: i32,
    p_mode_1: *mut mode_t, p_uid_1: *mut uid_t, p_gid_1: *mut gid_t) -> i32 {
    let mut rc: i32 = 0;
    unsafe { *p_mode_1 = 0 as mode_t };
    unsafe { *p_uid_1 = 0 as uid_t };
    unsafe { *p_gid_1 = 0 as gid_t };
...
extern "C" fn unix_open(p_vfs_1: *mut sqlite3_vfs, z_path_1: *const i8,
    p_file_1: *mut sqlite3_file, mut flags: i32, p_out_flags_1: *mut i32)
    -> i32 {
    unsafe {
        ...
        '__b41: loop {
            '__c41: loop {
                ...
                { let _ = 0; };
                { let _ = 0; };
                if randomness_pid != unsafe { getpid() } as pid_t {
                    randomness_pid = unsafe { getpid() } as pid_t;
                ...
                open_flags |= 0 | 0 | 256;
                if fd < 0 {
                    let mut open_mode: mode_t = 0 as mode_t;
...
extern "C" fn unix_delete(not_used_1: *mut sqlite3_vfs, z_path_1: *const i8,
    dir_sync_1: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        { let _ = not_used_1; };
...
extern "C" fn unix_access(not_used_1: *mut sqlite3_vfs, z_path_1: *const i8,
...
extern "C" fn unix_full_pathname(p_vfs_1: *mut sqlite3_vfs,
...
extern "C" fn unix_dl_open(not_used_1: *mut sqlite3_vfs,
    z_filename_1: *const i8) -> *mut () {
    { let _ = not_used_1; };
    return unsafe { dlopen(z_filename_1, 2 | 8) };
}
extern "C" fn unix_dl_error(not_used_1: *mut sqlite3_vfs, n_buf_1: i32,
    z_buf_out_1: *mut i8) -> () {
...
extern "C" fn unix_dl_sym(not_used_1: *mut sqlite3_vfs, p: *mut (),
...
extern "C" fn unix_dl_close(not_used_1: *mut sqlite3_vfs, p_handle_1: *mut ())
    -> () {
    { let _ = not_used_1; };
    unsafe { dlclose(p_handle_1) };
}
extern "C" fn unix_randomness(not_used_1: *mut sqlite3_vfs, mut n_buf_1: i32,
    z_buf_1: *mut i8) -> i32 {
    unsafe {
        { let _ = not_used_1; };
        { let _ = 0; };
        ...
        randomness_pid = unsafe { getpid() } as pid_t;
        {
            ...
            if fd < 0 {
                ...
                unsafe {
                    memcpy(unsafe {
                                &raw mut *z_buf_1.add(core::mem::size_of::<time_t>() as
                                                usize)
                ...
                n_buf_1 =
                    (core::mem::size_of::<time_t>() as u64 +
                            core::mem::size_of::<pid_t>() as u64) as i32;
            } else {
                '__b45: loop {
                    '__c45: loop {
                        got =
...
extern "C" fn unix_current_time_int64(not_used_1: *mut sqlite3_vfs,
    pi_now_1: *mut sqlite3_int64) -> i32 {
    let rc: i32 = 0;
    let mut s_now: timeval = unsafe { core::mem::zeroed() };
    { let _ = unsafe { gettimeofday(&mut s_now, core::ptr::null_mut()) }; };
    unsafe {
        *pi_now_1 =
            unix_epoch + 1000 as sqlite3_int64 * s_now.tv_sec as sqlite3_int64
                + (s_now.tv_usec / 1000) as sqlite3_int64
    };
    { let _ = not_used_1; };
    return rc;
}
extern "C" fn unix_current_time(not_used_1: *mut sqlite3_vfs,
...
extern "C" fn unix_get_last_error(not_used_1: *mut sqlite3_vfs,
...
extern "C" fn unix_set_system_call(p_not_used_1: *mut sqlite3_vfs,
    ...
    unsafe {
        ...
        if z_name_1 == core::ptr::null() {
            rc = 0;
            {
                ...
                '__b46: loop {
                    if !((i as u64) <
                                    core::mem::size_of::<[unix_syscall; 29]>() as u64 /
                                        core::mem::size_of::<unix_syscall>() as u64) {
                        break '__b46;
                    }
                    '__c46: loop {
...
extern "C" fn unix_get_system_call(p_not_used_1: *mut sqlite3_vfs,
...
extern "C" fn unix_next_system_call(p: *mut sqlite3_vfs, z_name_1: *const i8)
    -> *const i8 {
    unsafe {
        let mut i: i32 = -1;
        { let _ = p; };
        if !(z_name_1).is_null() {
            {
                ...
                '__b49: loop {
                    if !(i <
                                    (core::mem::size_of::<[unix_syscall; 29]>() as u64 /
                                                core::mem::size_of::<unix_syscall>() as u64) as i32 - 1) {
                        break '__b49;
                    }
                    '__c49: loop {
                        if unsafe { strcmp(z_name_1, a_syscall[i as usize].z_name) }
        ...
        }
        {
            ...
            '__b50: loop {
                if !(i <
                                (core::mem::size_of::<[unix_syscall; 29]>() as u64 /
                                        core::mem::size_of::<unix_syscall>() as u64) as i32) {
                    break '__b50;
                }
                '__c50: loop {
...
extern "C" fn flock_unlock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    let p_file: *mut unixFile = id as *mut unixFile;
    { let _ = 0; };
    { let _ = 0; };
...
extern "C" fn flock_close(id: *mut sqlite3_file) -> i32 {
    { let _ = 0; };
    flock_unlock(id, 0);
    return close_unix_file(id);
}
extern "C" fn flock_lock(id: *mut sqlite3_file, e_file_lock_1: i32) -> i32 {
    let mut rc: i32 = 0;
    let p_file: *mut unixFile = id as *mut unixFile;
    { let _ = 0; };
    if unsafe { (*p_file).e_file_lock } as i32 > 0 {
...
extern "C" fn flock_check_reserved_lock(id: *mut sqlite3_file,
    p_res_out_1: *mut i32) -> i32 {
    { let _ = id; };
    { let _ = 0; };
    { let _ = 0; };
...
static flock_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
...
pub extern "C" fn sqlite3_os_init() -> i32 {
    unsafe {
        ...
        { let _ = 0; };
        {
            ...
            '__b52: loop {
                if !((i as u64) <
                                core::mem::size_of::<[sqlite3_vfs; 9]>() as u64 /
                                    core::mem::size_of::<sqlite3_vfs>() as u64) {
                    break '__b52;
                }
                '__c52: loop {
...
#[repr(C)]
#[derive(Copy, Clone)]
struct Mapping_N7Mapping {
    z_filesystem: *const i8,
    p_methods: *const sqlite3_io_methods,
}
static pgsz_1: i32 = 4096 as i32;
static mut a_map: [Mapping_N7Mapping; 6] =
    [Mapping_N7Mapping {
...
static unix_epoch: sqlite3_int64 =
    (24405875 as sqlite3_int64 * 8640000 as sqlite3_int64) as sqlite3_int64;
static mut a_vfs: [sqlite3_vfs; 9] =
    [sqlite3_vfs {
                i_version: 3,
...
extern "C" {
    ...
    -> i64;
    fn pread(__fd: i32, __buf: *mut (), __nbyte: u64, __offset: off_t)
    -> i64;
    fn write(__fd: i32, __buf: *const (), __nbyte: u64)
    -> i64;
    fn pwrite(__fd: i32, __buf: *const (), __nbyte: u64, __offset: off_t)
    -> i64;
    fn fchmod(_: i32, _: mode_t)
    -> i32;
    ...
    fn rmdir(_: *const i8)
    -> i32;
    fn fchown(_: i32, _: uid_t, _: gid_t)
    -> i32;
    fn geteuid()
    -> uid_t;
    fn mmap(_: *mut (), _: u64, _: i32, _: i32, _: i32, _: off_t)
    -> *mut ();
