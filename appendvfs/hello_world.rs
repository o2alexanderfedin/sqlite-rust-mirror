...
type ApndVfs = sqlite3_vfs;
#[repr(C)]
#[derive(Copy, Clone)]
struct ApndFile {
    base: sqlite3_file,
    i_pg_one: sqlite3_int64,
    i_mark: sqlite3_int64,
}
extern "C" fn apnd_close(mut p_file: *mut sqlite3_file) -> i32 {
    p_file =
...
extern "C" fn apnd_read(mut p_file: *mut sqlite3_file, z_buf: *mut (),
...
extern "C" fn apnd_write_mark(paf: *mut ApndFile, p_file_1: *mut sqlite3_file,
    mut i_write_end_1: sqlite_int64) -> i32 {
    let mut i_pg_one: sqlite_int64 = unsafe { (*paf).i_pg_one };
    let mut a: [u8; 25] = [0; 25];
    let mut i: i32 = 8;
    let mut rc: i32 = 0;
    if !(p_file_1 ==
                            unsafe { (paf as *mut ApndFile).offset(1 as isize) } as
                                *mut sqlite3_file) as i32 as i64 != 0 {
        unsafe {
    ...
    while { let __p = &mut i; *__p -= 1; *__p } >= 0 {
        a[(17 + i) as usize] = (i_pg_one & 255 as sqlite_int64) as u8;
        i_pg_one >>= 8 as sqlite_int64;
    }
...
extern "C" fn apnd_write(mut p_file: *mut sqlite3_file, z_buf: *const (),
    i_amt: i32, i_ofst: sqlite3_int64) -> i32 {
    let paf: *mut ApndFile = p_file as *mut ApndFile;
    let i_write_end: sqlite_int64 = i_ofst + i_amt as sqlite_int64;
...
extern "C" fn apnd_truncate(mut p_file: *mut sqlite3_file,
    ...
    return unsafe {
            (unsafe {
                ...
                unsafe { (*paf).i_mark } + (17 + 8) as sqlite3_int64)
        };
}
extern "C" fn apnd_sync(mut p_file: *mut sqlite3_file, flags: i32) -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    return unsafe {
...
extern "C" fn apnd_file_size(p_file: *mut sqlite3_file,
...
extern "C" fn apnd_lock(mut p_file: *mut sqlite3_file, e_lock: i32) -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    return unsafe {
...
extern "C" fn apnd_unlock(mut p_file: *mut sqlite3_file, e_lock: i32) -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    return unsafe {
...
extern "C" fn apnd_check_reserved_lock(mut p_file: *mut sqlite3_file,
...
extern "C" fn apnd_file_control(mut p_file: *mut sqlite3_file, op: i32,
    p_arg: *mut ()) -> i32 {
    let paf: *const ApndFile = p_file as *mut ApndFile as *const ApndFile;
    let mut rc: i32 = 0;
    p_file =
    ...
    if op == 5 {
        unsafe {
            *(p_arg as *mut sqlite3_int64) += unsafe { (*paf).i_pg_one }
        };
    }
    rc =
        unsafe {
...
extern "C" fn apnd_sector_size(mut p_file: *mut sqlite3_file) -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    return unsafe {
...
extern "C" fn apnd_device_characteristics(mut p_file: *mut sqlite3_file)
    -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
...
extern "C" fn apnd_shm_map(mut p_file: *mut sqlite3_file, i_pg: i32,
...
extern "C" fn apnd_shm_lock(mut p_file: *mut sqlite3_file, offset: i32,
    n: i32, flags: i32) -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
...
extern "C" fn apnd_shm_barrier(mut p_file: *mut sqlite3_file) -> () {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    unsafe {
...
extern "C" fn apnd_shm_unmap(mut p_file: *mut sqlite3_file, delete_flag: i32)
    -> i32 {
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
...
extern "C" fn apnd_fetch(mut p_file: *mut sqlite3_file, i_ofst: sqlite3_int64,
    i_amt: i32, pp: *mut *mut ()) -> i32 {
    let p: *const ApndFile = p_file as *mut ApndFile as *const ApndFile;
    if unsafe { (*p).i_mark } < 0 as i64 ||
            i_ofst + i_amt as sqlite3_int64 > unsafe { (*p).i_mark } {
        return 10;
    }
    p_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
...
extern "C" fn apnd_unfetch(mut p_file: *mut sqlite3_file,
...
static apnd_io_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 3,
...
extern "C" fn apnd_read_mark(sz: sqlite3_int64, p_file_1: *mut sqlite3_file)
    -> sqlite3_int64 {
    let mut rc: i32 = 0;
    let mut i: i32 = 0;
    let mut i_mark: sqlite3_int64 = 0 as sqlite3_int64;
    let mut msbs: i32 = 8 * (8 - 1);
    let mut a: [u8; 25] = [0; 25];
    if (17 + 8) as sqlite3_int64 != sz & 511 as sqlite3_int64 {
    ...
    i_mark = ((a[17 as usize] as i32 & 127) as sqlite3_int64) << msbs;
    {
        i = 1;
        '__b1: loop {
            if !(i < 8) { break '__b1; }
            '__c1: loop {
                msbs -= 8;
                i_mark |= (a[(17 + i) as usize] as sqlite3_int64) << msbs;
                break '__c1;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
        }
    }
    if i_mark > sz - (17 + 8) as sqlite3_int64 - 512 as sqlite3_int64 {
        return -1 as sqlite3_int64;
    }
    if i_mark & 511 as sqlite3_int64 != 0 { return -1 as sqlite3_int64; }
    return i_mark;
}
...
extern "C" fn apnd_is_appendvfs_database(sz: sqlite3_int64,
    p_file_1: *mut sqlite3_file) -> i32 {
    let mut rc: i32 = 0;
    let mut z_hdr: [i8; 16] = [0; 16];
    let i_mark: sqlite3_int64 = apnd_read_mark(sz, p_file_1);
    if i_mark >= 0 as i64 {
        rc =
            unsafe {
                (unsafe {
        ...
        if 0 == rc &&
                        unsafe {
                            ...
                            } == 0 && sz & 511 as sqlite3_int64 == (17 + 8) as i64 &&
                sz >= (512 + (17 + 8)) as i64 {
            return 1;
        }
    }
...
extern "C" fn apnd_is_ordinary_database_file(sz: sqlite3_int64,
    p_file_1: *mut sqlite3_file) -> i32 {
    let mut z_hdr: [i8; 16] = [0; 16];
    if apnd_is_appendvfs_database(sz, p_file_1) != 0 ||
                    sz & 511 as sqlite3_int64 != 0 as i64 ||
                0 !=
                    unsafe {
                        (unsafe {
                                (*unsafe { (*p_file_1).p_methods }).x_read.unwrap()
...
extern "C" fn apnd_open(p_apnd_vfs: *mut sqlite3_vfs, z_name: *const i8,
    p_file: *mut sqlite3_file, flags: i32, p_out_flags: *mut i32) -> i32 {
    let p_apnd_file: *mut ApndFile = p_file as *mut ApndFile;
    let p_base_file: *mut sqlite3_file =
        unsafe { (p_file as *mut ApndFile).offset(1 as isize) } as
            *mut sqlite3_file;
    let p_base_vfs: *mut sqlite3_vfs =
        unsafe { (*p_apnd_vfs).p_app_data } as *mut sqlite3_vfs;
    let mut rc: i32 = 0;
    ...
    unsafe { (*p_apnd_file).i_mark = -1 as sqlite3_int64 };
    rc =
        unsafe {
            (unsafe {
                    (*p_base_vfs).x_open.unwrap()
    ...
    if unsafe { (*p_apnd_file).i_pg_one } >= 0 as i64 {
        unsafe { (*p_apnd_file).i_mark = sz - (17 + 8) as sqlite3_int64 };
        return 0;
    }
    if flags & 4 == 0 {
        unsafe {
        ...
        unsafe {
            (*p_apnd_file).i_pg_one =
                sz + (4096 - 1) as sqlite3_int64 &
                    !((4096 - 1) as sqlite3_int64)
        };
    }
    return rc;
}
extern "C" fn apnd_delete(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
    dir_sync: i32) -> i32 {
    return unsafe {
            (unsafe {
...
extern "C" fn apnd_access(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
...
extern "C" fn apnd_full_pathname(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
...
extern "C" fn apnd_dl_open(p_vfs: *mut sqlite3_vfs, z_path: *const i8)
    -> *mut () {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_dl_error(p_vfs: *mut sqlite3_vfs, n_byte: i32,
    z_err_msg: *mut i8) -> () {
    unsafe {
        (unsafe {
                (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_dl_sym(p_vfs: *mut sqlite3_vfs, p: *mut (),
...
extern "C" fn apnd_dl_close(p_vfs: *mut sqlite3_vfs, p_handle: *mut ())
    -> () {
    unsafe {
        (unsafe {
                (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_randomness(p_vfs: *mut sqlite3_vfs, n_byte: i32,
...
extern "C" fn apnd_sleep(p_vfs: *mut sqlite3_vfs, n_micro: i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_current_time(p_vfs: *mut sqlite3_vfs, p_time_out: *mut f64)
    -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_get_last_error(p_vfs: *mut sqlite3_vfs, a: i32, b: *mut i8)
    -> i32 {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_current_time_int64(p_vfs: *mut sqlite3_vfs,
...
extern "C" fn apnd_set_system_call(p_vfs: *mut sqlite3_vfs, z_name: *const i8,
...
extern "C" fn apnd_get_system_call(p_vfs: *mut sqlite3_vfs, z_name: *const i8)
    -> unsafe extern "C" fn() -> () {
    return unsafe {
            (unsafe {
                    (*(unsafe { (*p_vfs).p_app_data } as
...
extern "C" fn apnd_next_system_call(p_vfs: *mut sqlite3_vfs,
...
static mut apnd_vfs: sqlite3_vfs =
    sqlite3_vfs {
        i_version: 3,
        sz_os_file: 0,
        mx_pathname: 1024,
...
pub extern "C" fn sqlite3_appendvfs_init(db: *const sqlite3,
    ...
    unsafe {
        let mut rc: i32 = 0;
        let mut p_orig: *mut sqlite3_vfs = core::ptr::null_mut();
        { let _ = p_api_1; };
        { let _ = pz_err_msg_1; };
        { let _ = db; };
