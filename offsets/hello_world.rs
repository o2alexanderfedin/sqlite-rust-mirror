...
extern "C" fn ofst_varint(p: &GState, p_ofst_1: &mut i32) -> sqlite3_int64 {
    ...
    if n == 8 {
        x = (x << 8) + unsafe { *a.offset(0 as isize) } as sqlite3_int64;
    } else {
        x = (x << 7) + unsafe { *a.offset(0 as isize) } as sqlite3_int64;
    }
    *p_ofst_1 += n + 1;
    return x;
}
