...
pub extern "C" fn sqlite3_function_search(h: i32, z_func: *const i8)
    ...
    unsafe {
        unsafe {
            ...
            {
                p = sqlite3_builtin_functions.a[h as usize];
                '__b0: loop {
                    if !(!(p).is_null()) { break '__b0; }
                    '__c0: loop {
                        { let _ = 0; };
                        if unsafe {
                                    sqlite3_str_i_cmp(unsafe { (*p).z_name }, z_func)
                                } == 0 {
                            return p;
                        }
                        break '__c0;
    ...
    unsafe {
        unsafe {
            let mut i: i32 = 0;
            {
                i = 0;
                '__b1: loop {
                    if !(i < n_def) { break '__b1; }
                    '__c1: loop {
                        let mut p_other: *mut FuncDef = core::ptr::null_mut();
                        let z_name: *const i8 =
                            unsafe { (*a_def.offset(i as isize)).z_name };
                        let n_name: i32 = unsafe { sqlite3_strlen30(z_name) };
                        let h: i32 =
                            (unsafe { *z_name.offset(0 as isize) } as i32 + n_name) %
                                23;
                        { let _ = 0; };
                        ...
                        if !(p_other).is_null() {
                            { let _ = 0; };
                            unsafe {
                                (*a_def.offset(i as isize)).p_next =
                                    unsafe { (*p_other).p_next }
                            };
                            unsafe {
                                (*p_other).p_next = unsafe { a_def.offset(i as isize) }
                            };
                        } else {
                            unsafe {
                                (*a_def.offset(i as isize)).p_next = core::ptr::null_mut()
                            };
                            unsafe {
                                (*a_def.offset(i as isize)).u.p_hash =
                                    sqlite3_builtin_functions.a[h as usize]
                            };
                            sqlite3_builtin_functions.a[h as usize] =
                                unsafe { a_def.offset(i as isize) };
                        }
                        break '__c1;
                    }
...
extern "C" fn match_quality(p: &FuncDef, n_arg_1: i32, enc: u8) -> i32 {
    let mut match_: i32 = 0;
    { let _ = 0; };
    { let _ = 0; };
    if (*p).n_arg as i32 != n_arg_1 {
        if n_arg_1 == -2 {
            return if !(*p).x_s_func.is_some() as i32 != 0 { 0 } else { 6 };
        }
        if (*p).n_arg as i32 >= 0 { return 0; }
        if ((*p).n_arg as i32) < -2 && n_arg_1 < -2 - (*p).n_arg as i32 {
            return 0;
        }
    }
    if (*p).n_arg as i32 == n_arg_1 { match_ = 4; } else { match_ = 1; }
    if enc as u32 == (*p).func_flags & 3 as u32 {
        match_ += 2;
    } else if enc as u32 & (*p).func_flags & 2 as u32 != 0 as u32 {
        match_ += 1;
    }
    return match_;
}
...
pub extern "C" fn sqlite3_find_function(db: *mut sqlite3, z_name: *const i8,
    ...
    unsafe {
        ...
        if (create_flag == 0) as i32 != 0 &&
                (p_best == core::ptr::null_mut() ||
                    unsafe { (*db).m_db_flags } & 2 as u32 != 0 as u32) {
            best_score = 0;
            h =
                (unsafe {
                                *(sqlite3_upper_to_lower.as_ptr() as
                                            *const u8).add(unsafe { *z_name.offset(0 as isize) } as u8
                                            as usize)
                            } as i32 + n_name) % 23;
        ...
        if create_flag != 0 && best_score < 6 &&
                {
                        p_best =
                            unsafe {
                                    sqlite3_db_malloc_zero(db,
                                        core::mem::size_of::<FuncDef>() as u64 + n_name as u64 +
                                            1 as u64)
                                } as *mut FuncDef;
                        p_best
                    } != core::ptr::null_mut() {
            ...
            unsafe {
                (*p_best).z_name =
                    unsafe { &raw mut *p_best.offset(1 as isize) } as *const i8
            };
            unsafe { (*p_best).n_arg = n_arg as u16 as i16 };
            unsafe { (*p_best).func_flags = enc as u32 };
            unsafe {
                memcpy(unsafe { &raw mut *p_best.offset(1 as isize) } as
                            *mut i8 as *mut (), z_name as *const (),
                    (n_name + 1) as u64)
            };
            {
                z = unsafe { (*p_best).z_name } as *mut u8;
                '__b4: loop {
                    if !(unsafe { *z } != 0) { break '__b4; }
                    '__c4: loop {
                        unsafe {
                            *z =
                                unsafe {
                                        *(sqlite3_upper_to_lower.as_ptr() as
                                                    *const u8).add(unsafe { *z } as usize)
                                    } as u8
                        };
                        break '__c4;
                    }
            ...
            p_other =
                unsafe {
                        sqlite3_hash_insert(unsafe { &mut (*db).a_func },
                            unsafe { (*p_best).z_name }, p_best as *mut ())
                    } as *mut FuncDef;
            if p_other == p_best {
                unsafe { sqlite3_db_free(db, p_best as *mut ()) };
                unsafe { sqlite3_oom_fault(db) };
                return core::ptr::null_mut();
            } else { unsafe { (*p_best).p_next = p_other }; }
        }
        if !(p_best).is_null() &&
                (unsafe { (*p_best).x_s_func.is_some() } || create_flag != 0)
            {
            return p_best;
        }
        return core::ptr::null_mut();
...
extern "C" fn find_coll_seq_entry(db: *mut sqlite3, z_name_1: *const i8,
    ...
    if core::ptr::null_mut() == p_coll && create != 0 {
        let n_name: i32 = unsafe { sqlite3_strlen30(z_name_1) } + 1;
        p_coll =
            unsafe {
                    sqlite3_db_malloc_zero(db,
                        3 as u64 * core::mem::size_of::<CollSeq>() as u64 +
                            n_name as u64)
                } as *mut CollSeq;
        if !(p_coll).is_null() {
            let mut p_del: *mut CollSeq = core::ptr::null_mut();
            unsafe {
                (*p_coll.offset(0 as isize)).z_name =
                    unsafe { &raw mut *p_coll.offset(3 as isize) } as *mut i8
            };
            unsafe { (*p_coll.offset(0 as isize)).enc = 1 as u8 };
            unsafe {
                (*p_coll.offset(1 as isize)).z_name =
                    unsafe { &raw mut *p_coll.offset(3 as isize) } as *mut i8
            };
            unsafe { (*p_coll.offset(1 as isize)).enc = 2 as u8 };
            unsafe {
                (*p_coll.offset(2 as isize)).z_name =
                    unsafe { &raw mut *p_coll.offset(3 as isize) } as *mut i8
            };
            unsafe { (*p_coll.offset(2 as isize)).enc = 3 as u8 };
            unsafe {
                memcpy(unsafe { (*p_coll.offset(0 as isize)).z_name } as
                        *mut (), z_name_1 as *const (), n_name as u64)
            };
            p_del =
                unsafe {
                        sqlite3_hash_insert(unsafe { &mut (*db).a_coll_seq },
...
pub extern "C" fn sqlite3_find_coll_seq(db: *mut sqlite3, enc: u8,
    ...
    } else { p_coll = unsafe { (*db).p_dflt_coll }; }
    return p_coll;
}
extern "C" fn call_coll_needed(db: *mut sqlite3, enc: i32,
    z_name_1: *const i8) -> () {
    { let _ = 0; };
    if unsafe { (*db).x_coll_needed.is_some() } {
        let z_external: *mut i8 = unsafe { sqlite3_db_str_dup(db, z_name_1) };
        if (z_external).is_null() as i32 != 0 { return; }
        unsafe {
            (unsafe {
                    (*db).x_coll_needed.unwrap()
                })(unsafe { (*db).p_coll_needed_arg }, db, enc,
                z_external as *const i8)
        };
        unsafe { sqlite3_db_free(db, z_external as *mut ()) };
    }
    if unsafe { (*db).x_coll_needed16.is_some() } {
        let mut z_external_1: *const i8 = core::ptr::null();
        let p_tmp: *mut sqlite3_value = unsafe { sqlite3_value_new(db) };
        unsafe {
            sqlite3_value_set_str(p_tmp, -1, z_name_1 as *const (), 1 as u8,
                None)
        };
        z_external_1 =
            unsafe { sqlite3ValueText(p_tmp, 2 as u8) } as *const i8;
        if !(z_external_1).is_null() {
            unsafe {
                (unsafe {
                        (*db).x_coll_needed16.unwrap()
                    })(unsafe { (*db).p_coll_needed_arg }, db,
                    unsafe { (*db).enc } as i32, z_external_1 as *const ())
            };
        }
        unsafe { sqlite3ValueFree(p_tmp) };
    }
}
extern "C" fn synth_coll_seq(db: *mut sqlite3, p_coll_1: *mut CollSeq)
    -> i32 {
    let mut p_coll2: *const CollSeq = core::ptr::null();
    let z: *const i8 = unsafe { (*p_coll_1).z_name } as *const i8;
    let mut i: i32 = 0;
    {
        i = 0;
        '__b5: loop {
            ...
            '__c5: loop {
                p_coll2 =
                    sqlite3_find_coll_seq(db, a_enc[i as usize], z as *const i8,
                        0);
                if unsafe { (*p_coll2).x_cmp.is_some() } {
                    unsafe {
                        memcpy(p_coll_1 as *mut (), p_coll2 as *const (),
...
pub extern "C" fn sqlite3_get_coll_seq(p_parse: *mut Parse, enc: u8,
    ...
    if !(p).is_null() && !unsafe { (*p).x_cmp.is_some() } as i32 != 0 &&
            synth_coll_seq(db, p) != 0 {
        p = core::ptr::null_mut();
    }
    { let _ = 0; };
    if p == core::ptr::null_mut() {
        unsafe {
            sqlite3_error_msg(p_parse,
                c"no such collation sequence: %s".as_ptr() as *mut i8 as
                    *const i8, z_name)
        };
        unsafe { (*p_parse).rc = 1 | 1 << 8 };
    }
    return p;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_locate_coll_seq(p_parse: *mut Parse,
    ...
    let initbusy: u8 = unsafe { (*db).init.busy };
    let mut p_coll: *mut CollSeq = core::ptr::null_mut();
    p_coll = sqlite3_find_coll_seq(db, enc, z_name, initbusy as i32);
    if (initbusy == 0) as i32 != 0 &&
            ((p_coll).is_null() as i32 != 0 ||
                !unsafe { (*p_coll).x_cmp.is_some() } as i32 != 0) {
        ...
        unsafe {
            (*db).p_dflt_coll =
                sqlite3_find_coll_seq(db, enc,
                    sqlite3_str_binary.as_ptr() as *const i8, 0)
        };
        unsafe { sqlite3_expire_prepared_statements(db, 1) };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_check_coll_seq(p_parse: *mut Parse,
    ...
    if !(p_coll).is_null() &&
            !unsafe { (*p_coll).x_cmp.is_some() } as i32 != 0 {
        let z_name: *const i8 = unsafe { (*p_coll).z_name } as *const i8;
        let db: *const sqlite3 = unsafe { (*p_parse).db } as *const sqlite3;
        let p: *const CollSeq =
            sqlite3_get_coll_seq(p_parse, unsafe { (*db).enc }, p_coll,
                    z_name) as *const CollSeq;
        if (p).is_null() as i32 != 0 { return 1; }
        { let _ = 0; };
    }
    return 0;
}
...
pub extern "C" fn sqlite3_schema_clear(p: *mut ()) -> () {
    ...
    let mut temp2: Hash = unsafe { core::mem::zeroed() };
    let mut p_elem: *mut HashElem = core::ptr::null_mut();
    let p_schema: *mut Schema = p as *mut Schema;
    let mut xdb: sqlite3 = unsafe { core::mem::zeroed() };
    unsafe {
        memset(&raw mut xdb as *mut (), 0,
            core::mem::size_of::<sqlite3>() as u64)
    };
    temp1 = unsafe { (*p_schema).tbl_hash };
    temp2 = unsafe { (*p_schema).trig_hash };
    unsafe { sqlite3_hash_init(unsafe { &mut (*p_schema).trig_hash }) };
    unsafe { sqlite3_hash_clear(unsafe { &mut (*p_schema).idx_hash }) };
    {
        p_elem = unsafe { (*&mut temp2).first };
        '__b6: loop {
            if !(!(p_elem).is_null()) { break '__b6; }
            '__c6: loop {
                unsafe {
                    sqlite3_delete_trigger(&mut xdb,
                        unsafe { (*p_elem).data } as *mut Trigger)
                };
                break '__c6;
            }
    ...
    unsafe { sqlite3_hash_clear(&mut temp2) };
    unsafe { sqlite3_hash_init(unsafe { &mut (*p_schema).tbl_hash }) };
    {
        p_elem = unsafe { (*&mut temp1).first };
        '__b7: loop {
            if !(!(p_elem).is_null()) { break '__b7; }
            '__c7: loop {
                let p_tab: *mut Table =
                    unsafe { (*p_elem).data } as *mut Table;
                unsafe { sqlite3_delete_table(&mut xdb, p_tab) };
                break '__c7;
            }
            p_elem = unsafe { (*p_elem).next };
        }
    }
    unsafe { sqlite3_hash_clear(&mut temp1) };
    unsafe { sqlite3_hash_clear(unsafe { &mut (*p_schema).fkey_hash }) };
    unsafe { (*p_schema).p_seq_tab = core::ptr::null_mut() };
    if unsafe { (*p_schema).schema_flags } as i32 & 1 != 0 {
        {
            let __p = unsafe { &mut (*p_schema).i_generation };
            let __t = *__p;
            *__p += 1;
            __t
        };
...
pub extern "C" fn sqlite3_schema_get(db: *mut sqlite3, p_bt: *mut Btree)
    -> *mut Schema {
    let mut p: *mut Schema = core::ptr::null_mut();
    if !(p_bt).is_null() {
        p =
            unsafe {
                    sqlite3_btree_schema(p_bt,
                        core::mem::size_of::<Schema>() as i32,
                        Some(sqlite3_schema_clear))
                } as *mut Schema;
    } else {
        p =
            unsafe {
                    sqlite3_db_malloc_zero(core::ptr::null_mut(),
                        core::mem::size_of::<Schema>() as u64)
                } as *mut Schema;
    }
    if (p).is_null() as i32 != 0 {
        unsafe { sqlite3_oom_fault(db) };
    } else if 0 == unsafe { (*p).file_format } as i32 {
        unsafe { sqlite3_hash_init(unsafe { &mut (*p).tbl_hash }) };
        unsafe { sqlite3_hash_init(unsafe { &mut (*p).idx_hash }) };
        unsafe { sqlite3_hash_init(unsafe { &mut (*p).trig_hash }) };
        unsafe { sqlite3_hash_init(unsafe { &mut (*p).fkey_hash }) };
        unsafe { (*p).enc = 1 as u8 };
    }
    return p;
}
static a_enc: [u8; 3] = [3 as u8, 2 as u8, 1 as u8];
extern "C" {
