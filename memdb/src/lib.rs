...
extern "C" fn memdb_close(p_file: *mut Sqlite3File) -> i32 {
    unsafe {
        ...
        if !(unsafe { (*p).z_f_name }).is_null() {
            let mut i: i32 = 0;
            let p_vfs_mutex: *mut Sqlite3Mutex =
...
extern "C" fn memdb_read(p_file: *mut Sqlite3File, z_buf: *mut (), i_amt: i32,
    ...
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        memdb_enter(unsafe { &*p });
        if i_ofst + i_amt as SqliteInt64 > unsafe { (*p).sz } {
...
extern "C" fn memdb_write(p_file: *mut Sqlite3File, z: *const (), i_amt: i32,
    ...
    unsafe {
        ...
        if i_ofst + i_amt as SqliteInt64 > unsafe { (*p).sz } {
            let mut rc: i32 = 0;
            if i_ofst + i_amt as SqliteInt64 > unsafe { (*p).sz_alloc } &&
                    {
                            rc =
            ...
            unsafe { (*p).sz = i_ofst + i_amt as SqliteInt64 };
        }
        unsafe {
...
extern "C" fn memdb_file_control(p_file: *mut Sqlite3File, op: i32,
    ...
    unsafe {
        ...
        if op == 36 {
            ...
            unsafe { *(p_arg as *mut Sqlite3Int64) = i_limit };
            rc = 0;
        }
        memdb_leave(unsafe { &*p });
        return rc;
...
extern "C" fn memdb_fetch(p_file: *mut Sqlite3File, i_ofst: Sqlite3Int64,
    ...
    unsafe {
        let p: *mut MemStore = unsafe { (*(p_file as *mut MemFile)).p_store };
        memdb_enter(unsafe { &*p });
        if i_ofst + i_amt as Sqlite3Int64 > unsafe { (*p).sz } ||
...
extern "C" fn memdb_from_db_schema(db: *mut Sqlite3, z_schema_1: *const i8)
    ...
    if unsafe { (*p).base.p_methods } !=
            &raw const memdb_io_methods as *const Sqlite3IoMethods {
        ...
        loop {
            ...
            '__s3:
                ...
                match __state {
                    ...
                    18 => {
                        unsafe { *pi_size_1 = -1 as Sqlite3Int64 };
                        __state = 17;
                    }
                    19 => {
                    ...
                    29 => {
                        p_out =
                            unsafe {
                                    sqlite3_malloc64(unsafe { (*p_store).sz } as Sqlite3Uint64)
                                } as *mut u8;
                        __state = 30;
                    }
                    30 => {
                    ...
                    63 => {
                        p_to =
                            unsafe {
                                p_out.offset((sz_page as Sqlite3Int64 *
                                            (pgno - 1) as Sqlite3Int64) as isize)
                            };
                        __state = 64;
                    }
                    64 => {
...
pub extern "C" fn sqlite3_deserialize(db: *mut Sqlite3,
    ...
    sz_buf_1: Sqlite3Int64, m_flags_1: u32) -> i32 {
    unsafe {
        unsafe {
            let mut rc: i32 = 0;
            '__b4: loop {
