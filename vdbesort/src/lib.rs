...
extern "C" fn vdbe_sorter_map_file(p_task_1: &SortSubtask,
    ...
    unsafe {
        ...
        if (*p_file_1).i_eof <=
            ...
            let p_fd: *mut Sqlite3File = (*p_file_1).p_fd;
    ...
    if n_byte_1 <= n_avail {
        *pp_out_1 =
        ...
        if unsafe { (*p).n_alloc } < n_byte_1 {
            ...
            let mut n_new: Sqlite3Int64 =
                if 128 as Sqlite3Int64 >
                        2 as Sqlite3Int64 * unsafe { (*p).n_alloc } as Sqlite3Int64
                    {
                    128 as Sqlite3Int64
                } else {
                    2 as Sqlite3Int64 * unsafe { (*p).n_alloc } as Sqlite3Int64
                };
            while n_byte_1 as Sqlite3Int64 > n_new {
                n_new = n_new * 2 as Sqlite3Int64;
            }
            a_new =
                unsafe {
    ...
    unsafe {
        ...
        if !(unsafe { (*p_sorter).list.a_memory }).is_null() {
            ...
            if n_min > unsafe { (*p_sorter).n_memory } {
                ...
                let mut n_new: Sqlite3Int64 =
                    2 as Sqlite3Int64 *
                        unsafe { (*p_sorter).n_memory } as Sqlite3Int64;
                ...
                if n_new > unsafe { (*p_sorter).mx_pma_size } as i64 {
                    n_new = unsafe { (*p_sorter).mx_pma_size } as Sqlite3Int64;
                }
                if n_new < n_min as i64 { n_new = n_min as Sqlite3Int64; }
                a_new =
                    unsafe {
