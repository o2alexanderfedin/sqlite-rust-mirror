...
pub extern "C" fn sqlite3_mmap_warm(db: *mut sqlite3, z_db_1: *const i8)
    ...
    if rc == 0 {
        z_sql =
            unsafe {
                sqlite3_mprintf(c"PRAGMA %s%q%spage_size".as_ptr() as *mut i8
                        as *const i8,
                    if !(z_db_1).is_null() {
                        c"\'".as_ptr() as *mut i8
                    } else { c"".as_ptr() as *mut i8 },
        ...
        if z_sql == core::ptr::null_mut() {
            ...
            let mut p_pgsz: *mut sqlite3_stmt = core::ptr::null_mut();
            rc =
                unsafe {
                    sqlite3_prepare_v2(db, z_sql as *const i8, -1, &mut p_pgsz,
                        core::ptr::null_mut())
                };
            unsafe { sqlite3_free(z_sql as *mut ()) };
            if rc == 0 {
                if unsafe { sqlite3_step(p_pgsz) } == 100 {
                    pgsz = unsafe { sqlite3_column_int(p_pgsz, 0) };
                }
                rc = unsafe { sqlite3_finalize(p_pgsz) };
            }
            if rc == 0 && pgsz == 0 { rc = 1; }
        }
    }
    if rc == 0 {
        ...
        rc =
            unsafe {
                sqlite3_file_control(db, z_db_1, 7, &raw mut p_fd as *mut ())
            };
        if rc == 0 &&
                unsafe { (*unsafe { (*p_fd).p_methods }).i_version } as i32 >=
                    3 {
            let mut i_pg: sqlite3_int64 = 1 as sqlite3_int64;
            let p: *const sqlite3_io_methods = unsafe { (*p_fd).p_methods };
            loop {
                let mut p_map: *mut u8 = core::ptr::null_mut();
                rc =
                    unsafe {
                        (unsafe {
                                (*p).x_fetch.unwrap()
                            })(p_fd, pgsz as sqlite3_int64 * i_pg, pgsz,
                            &raw mut p_map as *mut *mut ())
                    };
                if rc != 0 || p_map == core::ptr::null_mut() { break; }
                n_total += unsafe { *p_map.offset(0 as isize) } as u32;
                n_total +=
                    unsafe { *p_map.offset((pgsz - 1) as isize) } as u32;
                rc =
                    unsafe {
                        (unsafe {
                                (*p).x_unfetch.unwrap()
                            })(p_fd, pgsz as sqlite3_int64 * i_pg, p_map as *mut ())
                    };
                if rc != 0 { break; }
            ...
            unsafe {
                sqlite3_log(0,
                    c"sqlite3_mmap_warm_cache: Warmed up %d pages of %s".as_ptr()
                            as *mut i8 as *const i8,
                    if i_pg == 1 as i64 { 0 as sqlite3_int64 } else { i_pg },
                    unsafe { sqlite3_db_filename(db, z_db_1) })
            };
        }
        rc2 =
            unsafe {
                sqlite3_exec(db, c"END".as_ptr() as *mut i8 as *const i8,
