...
extern "C" fn memjrnl_close(p_jfd_1: *mut sqlite3_file) -> i32 {
    let p: *const MemJournal =
        p_jfd_1 as *mut MemJournal as *const MemJournal;
    memjrnl_free_chunks(unsafe { (*p).p_first });
    return 0;
}
extern "C" fn memjrnl_read(p_jfd_1: *mut sqlite3_file, z_buf_1: *mut (),
    i_amt_1: i32, i_ofst_1: sqlite_int64) -> i32 {
    let p: *mut MemJournal = p_jfd_1 as *mut MemJournal;
    let mut z_out: *mut u8 = z_buf_1 as *mut u8;
    let mut n_read: i32 = i_amt_1;
    let mut i_chunk_offset: i32 = 0;
    let mut p_chunk: *mut FileChunk = core::ptr::null_mut();
    if i_amt_1 as sqlite_int64 + i_ofst_1 > unsafe { (*p).endpoint.i_offset }
        {
        return 10 | 2 << 8;
    }
    { let _ = 0; };
    if unsafe { (*p).readpoint.i_offset } != i_ofst_1 || i_ofst_1 == 0 as i64
        {
        let mut i_off: sqlite3_int64 = 0 as sqlite3_int64;
        {
            p_chunk = unsafe { (*p).p_first };
            '__b1: loop {
                if !(!(p_chunk).is_null() &&
                                i_off + unsafe { (*p).n_chunk_size } as sqlite3_int64 <=
                                    i_ofst_1) {
                    break '__b1;
                }
                '__c1: loop {
                ...
                p_chunk = unsafe { (*p_chunk).p_next };
            }
        }
    } else { p_chunk = unsafe { (*p).readpoint.p_chunk }; { let _ = 0; }; }
    i_chunk_offset =
        (i_ofst_1 % unsafe { (*p).n_chunk_size } as sqlite_int64) as i32;
    '__b2: loop {
        '__c2: loop {
            let i_space: i32 = unsafe { (*p).n_chunk_size } - i_chunk_offset;
            let n_copy: i32 =
                if n_read < unsafe { (*p).n_chunk_size } - i_chunk_offset {
                    n_read
                } else { (unsafe { (*p).n_chunk_size }) - i_chunk_offset };
            unsafe {
                memcpy(z_out as *mut (),
                    unsafe {
                            (unsafe { &raw mut (*p_chunk).z_chunk[0 as usize] } as
                                    *mut u8).offset(i_chunk_offset as isize)
        ...
        if !(n_read >= 0 &&
                            { p_chunk = unsafe { (*p_chunk).p_next }; p_chunk } !=
                                core::ptr::null_mut() && n_read > 0) {
            break '__b2;
        }
    }
    unsafe {
        (*p).readpoint.i_offset =
            if !(p_chunk).is_null() {
                i_ofst_1 + i_amt_1 as sqlite_int64
            } else { 0 as sqlite_int64 }
    };
...
extern "C" fn memjrnl_create_file(p: *mut MemJournal) -> i32 {
    let mut rc: i32 = 0;
    let p_real: *mut sqlite3_file = p as *mut sqlite3_file;
    let copy: MemJournal = unsafe { core::ptr::read(p) };
    unsafe {
        memset(p as *mut (), 0, core::mem::size_of::<MemJournal>() as u64)
    };
    rc =
        unsafe {
            sqlite3_os_open(copy.p_vfs, copy.z_journal, p_real, copy.flags,
                core::ptr::null_mut())
        };
    if rc == 0 {
        let mut n_chunk: i32 = copy.n_chunk_size;
        let mut i_off: i64 = 0 as i64;
        let mut p_iter: *mut FileChunk = core::ptr::null_mut();
        {
            p_iter = copy.p_first;
            '__b3: loop {
                if !(!(p_iter).is_null()) { break '__b3; }
                '__c3: loop {
                    if i_off + n_chunk as i64 > copy.endpoint.i_offset {
                        n_chunk = (copy.endpoint.i_offset - i_off) as i32;
                    }
                    rc =
                        unsafe {
                            sqlite3_os_write(p_real,
                                unsafe { &raw mut (*p_iter).z_chunk[0 as usize] } as *mut u8
                                    as *const (), n_chunk, i_off)
                        };
                    if rc != 0 { break '__b3; }
                    i_off += n_chunk as i64;
                    break '__c3;
                }
...
extern "C" fn memjrnl_truncate(p_jfd: *mut sqlite3_file, size: sqlite_int64)
    -> i32 {
    let p: *mut MemJournal = p_jfd as *mut MemJournal;
    { let _ = 0; };
    if size < unsafe { (*p).endpoint.i_offset } {
        let mut p_iter: *mut FileChunk = core::ptr::null_mut();
        if size == 0 as i64 {
            memjrnl_free_chunks(unsafe { (*p).p_first });
            unsafe { (*p).p_first = core::ptr::null_mut() };
        } else {
            let mut i_off: i64 = unsafe { (*p).n_chunk_size } as i64;
            {
                p_iter = unsafe { (*p).p_first };
                '__b4: loop {
                    if !(!(p_iter).is_null() && i_off < size) { break '__b4; }
                    '__c4: loop {
                        i_off += unsafe { (*p).n_chunk_size } as i64;
                        break '__c4;
                    }
                    p_iter = unsafe { (*p_iter).p_next };
                }
            ...
            if !(p_iter).is_null() {
                memjrnl_free_chunks(unsafe { (*p_iter).p_next });
                unsafe { (*p_iter).p_next = core::ptr::null_mut() };
            }
        }
        unsafe { (*p).endpoint.p_chunk = p_iter };
        unsafe { (*p).endpoint.i_offset = size };
        unsafe { (*p).readpoint.p_chunk = core::ptr::null_mut() };
        unsafe { (*p).readpoint.i_offset = 0 as sqlite3_int64 };
    }
    return 0;
}
extern "C" fn memjrnl_write(p_jfd_1: *mut sqlite3_file, z_buf_1: *const (),
    i_amt_1: i32, i_ofst_1: sqlite_int64) -> i32 {
    let p: *mut MemJournal = p_jfd_1 as *mut MemJournal;
    let mut n_write: i32 = i_amt_1;
    let mut z_write: *const u8 = z_buf_1 as *mut u8 as *const u8;
    if unsafe { (*p).n_spill } > 0 &&
            i_amt_1 as sqlite_int64 + i_ofst_1 >
                unsafe { (*p).n_spill } as i64 {
        let mut rc: i32 = memjrnl_create_file(p);
        if rc == 0 {
            rc =
                unsafe {
                    sqlite3_os_write(p_jfd_1, z_buf_1, i_amt_1, i_ofst_1)
                };
        }
        return rc;
    } else {
        ...
        if i_ofst_1 > 0 as i64 &&
            ...
            memjrnl_truncate(p_jfd_1, i_ofst_1);
        }
        if i_ofst_1 == 0 as i64 && !(unsafe { (*p).p_first }).is_null() {
            { let _ = 0; };
            unsafe {
                memcpy(unsafe {
                                &raw mut (*unsafe { (*p).p_first }).z_chunk[0 as usize]
                            } as *mut u8 as *mut (), z_buf_1, i_amt_1 as u64)
            };
        } else {
            while n_write > 0 {
                let mut p_chunk: *mut FileChunk =
                    unsafe { (*p).endpoint.p_chunk };
                let i_chunk_offset: i32 =
                    (unsafe { (*p).endpoint.i_offset } %
                            unsafe { (*p).n_chunk_size } as sqlite3_int64) as i32;
                let i_space: i32 =
                    if n_write < unsafe { (*p).n_chunk_size } - i_chunk_offset {
                        n_write
                    } else { (unsafe { (*p).n_chunk_size }) - i_chunk_offset };
                { let _ = 0; };
                if i_chunk_offset == 0 {
                    let p_new: *mut FileChunk =
                        unsafe {
                                sqlite3_malloc((core::mem::size_of::<FileChunk>() as u64 +
                                            (unsafe { (*p).n_chunk_size } - 8) as u64) as i32)
                            } as *mut FileChunk;
                    if (p_new).is_null() as i32 != 0 { return 10 | 12 << 8; }
                    unsafe { (*p_new).p_next = core::ptr::null_mut() };
                    if !(p_chunk).is_null() {
                        { let _ = 0; };
                        unsafe { (*p_chunk).p_next = p_new };
                    } else { { let _ = 0; }; unsafe { (*p).p_first = p_new }; }
                    p_chunk =
                        {
                            unsafe { (*p).endpoint.p_chunk = p_new };
                            unsafe { (*p).endpoint.p_chunk }
                        };
                }
                { let _ = 0; };
                unsafe {
                    memcpy(unsafe {
                                (unsafe { &raw mut (*p_chunk).z_chunk[0 as usize] } as
                                        *mut u8).offset(i_chunk_offset as isize)
                            } as *mut (), z_write as *const (), i_space as u64)
                };
                {
                    let __n = i_space;
                    let __p = &mut z_write;
                ...
                n_write -= i_space;
                unsafe { (*p).endpoint.i_offset += i_space as sqlite3_int64 };
            }
        }
    }
    return 0;
}
extern "C" fn memjrnl_sync(p_jfd_1: *mut sqlite3_file, flags: i32) -> i32 {
    { { let _ = p_jfd_1; }; { let _ = flags; } };
    return 0;
}
extern "C" fn memjrnl_file_size(p_jfd_1: *mut sqlite3_file,
    p_size_1: *mut sqlite_int64) -> i32 {
    let p: *const MemJournal =
        p_jfd_1 as *mut MemJournal as *const MemJournal;
    unsafe { *p_size_1 = unsafe { (*p).endpoint.i_offset } as sqlite_int64 };
    return 0;
}
static mem_journal_methods: sqlite3_io_methods =
    sqlite3_io_methods {
        i_version: 1,
        x_close: Some(memjrnl_close),
        x_read: Some(memjrnl_read),
        x_write: Some(memjrnl_write),
        x_truncate: Some(memjrnl_truncate),
        x_sync: Some(memjrnl_sync),
        x_file_size: Some(memjrnl_file_size),
        x_lock: None,
        x_unlock: None,
        x_check_reserved_lock: None,
        x_file_control: None,
        x_sector_size: None,
        x_device_characteristics: None,
        x_shm_map: None,
        x_shm_lock: None,
        x_shm_barrier: None,
        x_shm_unmap: None,
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_journal_open(p_vfs: *mut sqlite3_vfs,
    z_name: *const i8, p_jfd: *mut sqlite3_file, flags: i32, n_spill: i32)
    -> i32 {
    let p: *mut MemJournal = p_jfd as *mut MemJournal;
    { let _ = 0; };
    unsafe {
    ...
    if n_spill == 0 {
        return unsafe {
                sqlite3_os_open(p_vfs, z_name, p_jfd, flags,
                    core::ptr::null_mut())
            };
    }
    if n_spill > 0 {
        unsafe { (*p).n_chunk_size = n_spill };
    } else {
        unsafe {
            (*p).n_chunk_size =
                ((8 + 1024) as u64 - core::mem::size_of::<FileChunk>() as u64)
                    as i32
        };
        { let _ = 0; };
    }
    unsafe {
        (*p_jfd).p_methods =
            &raw const mem_journal_methods as *const sqlite3_io_methods
    };
    unsafe { (*p).n_spill = n_spill };
    unsafe { (*p).flags = flags };
    unsafe { (*p).z_journal = z_name };
    unsafe { (*p).p_vfs = p_vfs };
    return 0;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_journal_size(p_vfs: &sqlite3_vfs) -> i32 {
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_journal_is_in_memory(p: &sqlite3_file) -> i32 {
    return ((*p).p_methods ==
                &raw const mem_journal_methods as *const sqlite3_io_methods)
            as i32;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_mem_journal_open(p_jfd: *mut sqlite3_file) -> () {
