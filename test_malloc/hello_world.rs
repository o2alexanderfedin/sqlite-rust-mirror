...
extern "C" fn faultsim_step() -> i32 {
    unsafe {
        if (memfault.enable == 0) as i32 != 0 {
            {
                let __p = &mut memfault.n_ok_after;
                let __t = *__p;
                *__p += 1;
                __t
            };
        ...
        if memfault.i_countdown > 0 {
            {
                let __p = &mut memfault.i_countdown;
                let __t = *__p;
                *__p -= 1;
                __t
            };
            {
                let __p = &mut memfault.n_ok_before;
                let __t = *__p;
                *__p += 1;
                __t
            };
        ...
        if memfault.n_fail == 0 { sqlite3_first_fault(); }
        sqlite3_fault();
        { let __p = &mut memfault.n_fail; let __t = *__p; *__p += 1; __t };
        if memfault.is_benign_mode > 0 {
            {
                let __p = &mut memfault.n_benign;
                let __t = *__p;
                *__p += 1;
                __t
            };
        }
        { let __p = &mut memfault.n_repeat; let __t = *__p; *__p -= 1; __t };
        if memfault.n_repeat <= 0 { memfault.enable = 0 as u8; }
        return 1;
    }
}
extern "C" fn faultsim_malloc(n: i32) -> *mut () {
    unsafe {
        ...
        if (faultsim_step() == 0) as i32 != 0 {
            p = unsafe { memfault.m.x_malloc.unwrap()(n) };
        }
        return p;
    }
}
extern "C" fn faultsim_realloc(p_old_1: *mut (), n: i32) -> *mut () {
    unsafe {
        let mut p: *mut () = core::ptr::null_mut();
        if (faultsim_step() == 0) as i32 != 0 {
            p = unsafe { memfault.m.x_realloc.unwrap()(p_old_1, n) };
        }
        return p;
    }
}
...
extern "C" fn faultsim_begin_benign() -> () {
    unsafe {
        {
            let __p = &mut memfault.is_benign_mode;
            let __t = *__p;
            *__p += 1;
            __t
        };
...
extern "C" fn faultsim_install(mut install: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        install = if install != 0 { 1 } else { 0 };
        { let _ = 0; };
        if install == memfault.is_installed { return 1; }
        if install != 0 {
            rc =
                unsafe {
                    sqlite3_config(5,
                        &raw mut memfault.m as *mut sqlite3_mem_methods)
                };
            { let _ = 0; };
            if rc == 0 {
                let mut m: sqlite3_mem_methods = memfault.m;
                m.x_malloc = Some(faultsim_malloc);
                m.x_realloc = Some(faultsim_realloc);
                rc =
                    unsafe {
                        sqlite3_config(4, &raw mut m as *mut sqlite3_mem_methods)
                    };
            }
            unsafe {
                sqlite3_test_control(10,
                    unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn()
                                    -> ()>(faultsim_begin_benign as *const ())
                    },
                    unsafe {
                        core::mem::transmute::<*const (),
                                unsafe extern "C" fn()
                                    -> ()>(faultsim_end_benign as *const ())
                    })
            };
        } else {
            let mut m2: sqlite3_mem_methods = unsafe { core::mem::zeroed() };
            { let _ = 0; };
            unsafe {
                memset(&raw mut m2 as *mut (), 0,
                    core::mem::size_of::<sqlite3_mem_methods>() as u64)
            };
            unsafe {
                sqlite3_config(4, &raw mut m2 as *mut sqlite3_mem_methods)
            };
            unsafe {
                sqlite3_config(5, &raw mut m2 as *mut sqlite3_mem_methods)
            };
            { let _ = 0; };
            rc =
                unsafe {
            ...
            unsafe { sqlite3_test_control(10, 0 as *mut (), 0 as *mut ()) };
        }
        if rc == 0 { memfault.is_installed = 1; }
        return rc;
    }
}
