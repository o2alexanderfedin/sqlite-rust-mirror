...
struct sqlite3_io_methods {
    ...
    x_file_control: Option<unsafe extern "C" fn(*mut sqlite3_file, i32,
        *mut ()) -> i32>,
    x_sector_size: unsafe extern "C" fn(*mut sqlite3_file) -> i32,
...
static mut vfs_list: *mut sqlite3_vfs = core::ptr::null_mut();
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_vfs_find(z_vfs: *const i8) -> *mut sqlite3_vfs {
    unsafe {
        let mut p_vfs: *mut sqlite3_vfs = core::ptr::null_mut();
        let mut mutex: *mut sqlite3_mutex = core::ptr::null_mut();
        let rc: i32 = unsafe { sqlite3_initialize() };
        if rc != 0 { return core::ptr::null_mut(); }
        mutex = unsafe { sqlite3MutexAlloc(2) };
        unsafe { sqlite3_mutex_enter(mutex) };
        {
            p_vfs = vfs_list;
            '__b0: loop {
                if !(!(p_vfs).is_null()) { break '__b0; }
                '__c0: loop {
                    if z_vfs == core::ptr::null() { break '__b0; }
                    if unsafe { strcmp(z_vfs, unsafe { (*p_vfs).z_name }) } == 0
                        {
                        break '__b0;
                    }
                    break '__c0;
                }
                p_vfs = unsafe { (*p_vfs).p_next };
            }
        }
...
extern "C" fn vfs_unlink(p_vfs_1: *mut sqlite3_vfs) -> () {
    unsafe {
        { let _ = 0; };
        if p_vfs_1 == core::ptr::null_mut()
            {} else if vfs_list == p_vfs_1 {
            vfs_list = unsafe { (*p_vfs_1).p_next };
        } else if !(vfs_list).is_null() {
            let mut p: *mut sqlite3_vfs = vfs_list;
            while !(unsafe { (*p).p_next }).is_null() &&
                    unsafe { (*p).p_next } != p_vfs_1 {
                p = unsafe { (*p).p_next };
            }
            if unsafe { (*p).p_next } == p_vfs_1 {
                unsafe { (*p).p_next = unsafe { (*p_vfs_1).p_next } };
            }
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_vfs_register(p_vfs: *mut sqlite3_vfs,
    make_dflt: i32) -> i32 {
    unsafe {
        let mut mutex: *mut sqlite3_mutex = core::ptr::null_mut();
        let rc: i32 = unsafe { sqlite3_initialize() };
        ...
        unsafe { sqlite3_mutex_enter(mutex) };
        vfs_unlink(p_vfs);
        if make_dflt != 0 || vfs_list == core::ptr::null_mut() {
            unsafe { (*p_vfs).p_next = vfs_list };
            vfs_list = p_vfs;
        } else {
            unsafe { (*p_vfs).p_next = unsafe { (*vfs_list).p_next } };
            unsafe { (*vfs_list).p_next = p_vfs };
        }
        { let _ = 0; };
        unsafe { sqlite3_mutex_leave(mutex) };
        return 0;
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_vfs_unregister(p_vfs: *mut sqlite3_vfs) -> i32 {
    let mut mutex: *mut sqlite3_mutex = core::ptr::null_mut();
    let rc: i32 = unsafe { sqlite3_initialize() };
    if rc != 0 { return rc; }
    mutex = unsafe { sqlite3MutexAlloc(2) };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_close(p_id: *mut sqlite3_file) -> () {
    if !(unsafe { (*p_id).p_methods }).is_null() {
        unsafe {
            (unsafe {
                    (*unsafe { (*p_id).p_methods }).x_close.unwrap()
                })(p_id)
        };
        unsafe { (*p_id).p_methods = core::ptr::null() };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_read(id: *mut sqlite3_file, p_buf: *mut (),
    amt: i32, offset: i64) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_read.unwrap()
                })(id, p_buf, amt, offset)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_write(id: *mut sqlite3_file, p_buf: *const (),
    amt: i32, offset: i64) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_write.unwrap()
                })(id, p_buf, amt, offset)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_truncate(id: *mut sqlite3_file, size: i64)
    -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_truncate.unwrap()
                })(id, size)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_sync(id: *mut sqlite3_file, flags: i32) -> i32 {
    return if flags != 0 {
            unsafe {
                (unsafe {
                        (*unsafe { (*id).p_methods }).x_sync.unwrap()
                    })(id, flags)
            }
        } else { 0 };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_file_size(id: *mut sqlite3_file,
    p_size: *mut i64) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_file_size.unwrap()
                })(id, p_size)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_lock(id: *mut sqlite3_file, lock_type: i32)
    -> i32 {
    { let _ = 0; };
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_lock.unwrap()
                })(id, lock_type)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_unlock(id: *mut sqlite3_file, lock_type: i32)
    -> i32 {
    { let _ = 0; };
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_unlock.unwrap()
                })(id, lock_type)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_check_reserved_lock(id: *mut sqlite3_file,
    p_res_out: *mut i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_check_reserved_lock.unwrap()
                })(id, p_res_out)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_file_control(id: *mut sqlite3_file, op: i32,
    p_arg: *mut ()) -> i32 {
    if unsafe { (*id).p_methods } == core::ptr::null() { return 12; }
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_file_control.unwrap()
                })(id, op, p_arg)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_file_control_hint(id: *mut sqlite3_file, op: i32,
    p_arg: *mut ()) -> () {
    if !(unsafe { (*id).p_methods }).is_null() {
        {
            let _ =
                unsafe {
                    (unsafe {
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_sector_size(id: *mut sqlite3_file) -> i32 {
    let x_sector_size:
            Option<unsafe extern "C" fn(*mut sqlite3_file) -> i32> =
        Some(unsafe { (*unsafe { (*id).p_methods }).x_sector_size });
    return if x_sector_size.is_some() {
            unsafe { x_sector_size.unwrap()(id) }
        } else { 4096 };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_device_characteristics(id: *mut sqlite3_file)
    -> i32 {
    if unsafe { (*id).p_methods } == core::ptr::null() { return 0; }
    return unsafe {
            (unsafe {
                    (*unsafe {
                                        (*id).p_methods
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_shm_map(id: *mut sqlite3_file, i_page: i32,
    pgsz: i32, b_extend: i32, pp: *mut *mut ()) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_shm_map.unwrap()
                })(id, i_page, pgsz, b_extend, pp)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_shm_lock(id: *mut sqlite3_file, offset: i32,
    n: i32, flags: i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_shm_lock.unwrap()
                })(id, offset, n, flags)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_shm_barrier(id: *mut sqlite3_file) -> () {
    unsafe {
        (unsafe { (*unsafe { (*id).p_methods }).x_shm_barrier.unwrap() })(id)
    };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_shm_unmap(id: *mut sqlite3_file,
    delete_flag: i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_shm_unmap.unwrap()
                })(id, delete_flag)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_fetch(id: *mut sqlite3_file, i_off: i64,
    i_amt: i32, pp: *mut *mut ()) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_fetch.unwrap()
                })(id, i_off, i_amt, pp)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_unfetch(id: *mut sqlite3_file, i_off: i64,
    p: *mut ()) -> i32 {
    return unsafe {
            (unsafe {
                    (*unsafe { (*id).p_methods }).x_unfetch.unwrap()
                })(id, i_off, p)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_open(p_vfs: *mut sqlite3_vfs, z_path: *const i8,
    p_file: *mut sqlite3_file, flags: i32, p_flags_out: *mut i32) -> i32 {
    let mut rc: i32 = 0;
    { let _ = 0; };
    rc =
        unsafe {
            (unsafe {
                    (*p_vfs).x_open.unwrap()
                })(p_vfs, z_path, p_file, flags & 17334143, p_flags_out)
        };
    { let _ = 0; };
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_delete(p_vfs: *mut sqlite3_vfs,
    z_path: *const i8, dir_sync: i32) -> i32 {
    { let _ = 0; };
    return if unsafe { (*p_vfs).x_delete.is_some() } {
            unsafe {
                (unsafe {
                        (*p_vfs).x_delete.unwrap()
                    })(p_vfs, z_path, dir_sync)
            }
        } else { 0 };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_access(p_vfs: *mut sqlite3_vfs,
    z_path: *const i8, flags: i32, p_res_out: *mut i32) -> i32 {
    return unsafe {
            (unsafe {
                    (*p_vfs).x_access.unwrap()
                })(p_vfs, z_path, flags, p_res_out)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_full_pathname(p_vfs: *mut sqlite3_vfs,
    z_path: *const i8, n_path_out: i32, z_path_out: *mut i8) -> i32 {
    unsafe { *z_path_out.offset(0 as isize) = 0 as i8 };
    return unsafe {
            (unsafe {
                    (*p_vfs).x_full_pathname.unwrap()
                })(p_vfs, z_path, n_path_out, z_path_out)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_dl_open(p_vfs: *mut sqlite3_vfs,
    z_path: *const i8) -> *mut () {
    { let _ = 0; };
    { let _ = 0; };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_dl_error(p_vfs: *mut sqlite3_vfs, n_byte: i32,
    z_buf_out: *mut i8) -> () {
    unsafe {
        (unsafe { (*p_vfs).x_dl_error.unwrap() })(p_vfs, n_byte, z_buf_out)
    };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_dl_sym(p_vfs: *mut sqlite3_vfs, p_hdle: *mut (),
    z_sym: *const i8) -> unsafe extern "C" fn() -> () {
    return unsafe {
            (unsafe { (*p_vfs).x_dl_sym.unwrap() })(p_vfs, p_hdle, z_sym)
        };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_dl_close(p_vfs: *mut sqlite3_vfs,
    p_handle: *mut ()) -> () {
    unsafe { (unsafe { (*p_vfs).x_dl_close.unwrap() })(p_vfs, p_handle) };
}
#[repr(C)]
#[derive(Copy, Clone)]
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_randomness(p_vfs: *mut sqlite3_vfs,
    mut n_byte: i32, z_buf_out: *mut i8) -> i32 {
    unsafe {
        if sqlite3Config.i_prng_seed != 0 {
            unsafe { memset(z_buf_out as *mut (), 0, n_byte as u64) };
            if n_byte > core::mem::size_of::<u32>() as i32 {
                n_byte = core::mem::size_of::<u32>() as i32;
            }
            unsafe {
                memcpy(z_buf_out as *mut (),
                    &raw mut sqlite3Config.i_prng_seed as *const (),
                    n_byte as u64)
            };
            return 0;
        } else {
            return unsafe {
                    (unsafe {
                            (*p_vfs).x_randomness.unwrap()
                        })(p_vfs, n_byte, z_buf_out)
                };
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_sleep(p_vfs: *mut sqlite3_vfs, n_micro: i32)
    -> i32 {
    return unsafe { (unsafe { (*p_vfs).x_sleep.unwrap() })(p_vfs, n_micro) };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_get_last_error(p_vfs: *mut sqlite3_vfs) -> i32 {
    return if unsafe { (*p_vfs).x_get_last_error.is_some() } {
            unsafe {
                (unsafe {
                        (*p_vfs).x_get_last_error.unwrap()
                    })(p_vfs, 0, core::ptr::null_mut())
            }
        } else { 0 };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_current_time_int64(p_vfs: *mut sqlite3_vfs,
    p_time_out: *mut sqlite3_int64) -> i32 {
    let mut rc: i32 = 0;
    if unsafe { (*p_vfs).i_version } >= 2 &&
        ...
        rc =
            unsafe {
                (unsafe { (*p_vfs).x_current_time.unwrap() })(p_vfs, &mut r)
            };
        unsafe {
            *p_time_out = unsafe { sqlite3_real_to_i64(r * 86400000.0) }
        };
    }
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_open_malloc(p_vfs: *mut sqlite3_vfs,
    z_file: *const i8, pp_file: &mut *mut sqlite3_file, flags: i32,
    p_out_flags: *mut i32) -> i32 {
    let mut rc: i32 = 0;
    let mut p_file: *mut sqlite3_file = core::ptr::null_mut();
    p_file =
        unsafe { sqlite3_malloc_zero(unsafe { (*p_vfs).sz_os_file } as u64) }
            as *mut sqlite3_file;
    if !(p_file).is_null() {
        rc = sqlite3_os_open(p_vfs, z_file, p_file, flags, p_out_flags);
        if rc != 0 {
            unsafe { sqlite3_free(p_file as *mut ()) };
            *pp_file = core::ptr::null_mut();
        } else { *pp_file = p_file; }
    } else { *pp_file = core::ptr::null_mut(); rc = 7; }
    { let _ = 0; };
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_os_close_free(p_file: *mut sqlite3_file) -> () {
    { let _ = 0; };
