...
extern "C" fn get_and_bind_sql_statement(p_err_1: *mut Error,
    p_db_1: *mut Sqlite, mut ap: *const i8) -> *mut sqlite3_stmt {
...
extern "C" fn integrity_check_x(p_err_1: *mut Error, p_db_1: *mut Sqlite)
    ...
    if unsafe { (*p_err_1).rc } == 0 {
        ...
        if !(p_statement).is_null() {
            let p_stmt: *mut sqlite3_stmt = unsafe { (*p_statement).p_stmt };
            while 100 == unsafe { sqlite3_step(p_stmt) } {
                let z: *const i8 =
        ...
        rc =
            unsafe {
                pthread_create(unsafe { &mut (*p).tid },
                    0 as *mut () as *const pthread_attr_t, launch_thread_main,
                    p as *mut ())
            };
        if rc != 0 {
...
extern "C" fn current_time() -> f64 {
    unsafe {
        ...
        if unsafe { (*p_timelimit_vfs).i_version } >= 2 &&
            ...
            let mut tm: sqlite3_int64 = 0 as sqlite3_int64;
            unsafe {
                (unsafe {
...
extern "C" fn stress2(n_ms_1: i32) -> () {
    let a_task: [Stress2Task_N11Stress2Task; 13] =
        [Stress2Task_N11Stress2Task { x: Some(stress2_workload1) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload2) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload3) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload4) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload5) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload6) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload7) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload8) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload9) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload10) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload11) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload14) },
                Stress2Task_N11Stress2Task { x: Some(stress2_workload17) }];
    ...
    unsafe { sqlite3_enable_shared_cache(1) };
    {
        ...
        '__b51: loop {
            if !((i as u64) <
                            core::mem::size_of::<[Stress2Task_N11Stress2Task; 13]>() as
                                    u64 /
                                core::mem::size_of::<Stress2Task_N11Stress2Task>() as u64) {
                break '__b51;
            }
            '__c51: loop {
...
extern "C" fn __main_inner(mut argc: i32, mut argv: *const *mut i8)
    -> Result<(), i32> {
    unsafe {
        let a_test: [ThreadTest_N10ThreadTest; 15] =
            [ThreadTest_N10ThreadTest {
        ...
        loop {
            ...
            '__s55:
                ...
                match __state {
                    ...
                    24 => {
                        if i as u64 >=
                                core::mem::size_of::<[ThreadTest_N10ThreadTest; 15]>() as
                                        u64 /
                                    core::mem::size_of::<ThreadTest_N10ThreadTest>() as u64 {
                            __state = 29;
                        } else { __state = 14; }
                    }
                    25 => {
...
#[repr(C)]
#[derive(Copy, Clone)]
struct Stress2Task_N11Stress2Task {
    x: Option<unsafe extern "C" fn(*mut Error, *mut Sqlite, i32) -> ()>,
}
#[repr(C)]
#[derive(Copy, Clone)]
struct ThreadTest_N10ThreadTest {
...
static mut p_timelimit_vfs: *mut sqlite3_vfs = core::ptr::null_mut();
