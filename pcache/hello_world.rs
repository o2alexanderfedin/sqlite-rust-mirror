...
pub extern "C" fn sqlite3_pcache_initialize() -> i32 {
    unsafe {
        ...
        return unsafe {
                sqlite3Config.pcache2.x_init.unwrap()(sqlite3Config.pcache2.p_arg)
            };
    }
}
#[unsafe(no_mangle)]
...
pub extern "C" fn sqlite3_pcache_set_page_size(p_cache: *mut PCache,
    ...
    unsafe {
        { let _ = 0; };
        if unsafe { (*p_cache).sz_page } != 0 {
            let mut p_new: *mut sqlite3_pcache = core::ptr::null_mut();
            p_new =
                unsafe {
                    sqlite3Config.pcache2.x_create.unwrap()(sz_page,
                        (unsafe { (*p_cache).sz_extra } as u64 +
                                (core::mem::size_of::<PgHdr>() as u64 + 7 as u64 &
                                    !7 as u64)) as i32,
                        unsafe { (*p_cache).b_purgeable } as i32)
                };
            if p_new == core::ptr::null_mut() { return 7; }
            unsafe {
                sqlite3Config.pcache2.x_cachesize.unwrap()(p_new,
                    number_of_cache_pages(unsafe { &*p_cache }))
            };
            if !(unsafe { (*p_cache).p_cache }).is_null() {
                unsafe {
                    sqlite3Config.pcache2.x_destroy.unwrap()(unsafe {
                            (*p_cache).p_cache
                        })
                };
            }
            unsafe { (*p_cache).p_cache = p_new };
            unsafe { (*p_cache).sz_page = sz_page };
        }
        return 0;
    }
}
...
pub extern "C" fn sqlite3_pcache_open(sz_page: i32, sz_extra: i32,
    b_purgeable: i32,
    x_stress: Option<unsafe extern "C" fn(*mut (), *mut PgHdr) -> i32>,
    p_stress: *mut (), p: *mut PCache) -> i32 {
    unsafe { memset(p as *mut (), 0, core::mem::size_of::<PCache>() as u64) };
    unsafe { (*p).sz_page = 1 };
    ...
    unsafe { (*p).p_stress = p_stress };
    unsafe { (*p).sz_cache = 100 };
    unsafe { (*p).sz_spill = 1 };
...
pub extern "C" fn sqlite3_pcache_fetch(p_cache: &PCache, pgno: Pgno,
    create_flag: i32) -> *mut sqlite3_pcache_page {
    unsafe {
        let mut e_create: i32 = 0;
        let mut p_res: *mut sqlite3_pcache_page = core::ptr::null_mut();
        { let _ = 0; };
        { let _ = 0; };
        { let _ = 0; };
        { let _ = 0; };
        ...
        p_res =
            unsafe {
                sqlite3Config.pcache2.x_fetch.unwrap()((*p_cache).p_cache,
                    pgno, e_create)
            };
        return p_res;
    }
...
pub extern "C" fn sqlite3_pcache_pagecount(p_cache: &PCache) -> i32 {
    unsafe {
        { let _ = 0; };
        return unsafe {
                sqlite3Config.pcache2.x_pagecount.unwrap()((*p_cache).p_cache)
            };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_pcache_fetch_stress(p_cache: *mut PCache,
    pgno: Pgno, pp_page: &mut *mut sqlite3_pcache_page) -> i32 {
    unsafe {
        let mut p_pg: *mut PgHdr = core::ptr::null_mut();
        if unsafe { (*p_cache).e_create } as i32 == 2 { return 0; }
        if sqlite3_pcache_pagecount(unsafe { &*p_cache }) >
                unsafe { (*p_cache).sz_spill } {
            {
                p_pg = unsafe { (*p_cache).p_synced };
                '__b0: loop {
                    if !(!(p_pg).is_null() &&
                                    (unsafe { (*p_pg).n_ref } != 0 ||
                                        unsafe { (*p_pg).flags } as i32 & 8 != 0)) {
                        break '__b0;
                    }
                    '__c0: loop { break '__c0; }
                    p_pg = unsafe { (*p_pg).p_dirty_prev };
                }
            }
            unsafe { (*p_cache).p_synced = p_pg };
            if (p_pg).is_null() as i32 != 0 {
                {
                    p_pg = unsafe { (*p_cache).p_dirty_tail };
                    '__b1: loop {
                        if !(!(p_pg).is_null() && unsafe { (*p_pg).n_ref } != 0) {
                            break '__b1;
                        }
            ...
            if !(p_pg).is_null() {
                ...
                rc =
                    unsafe {
                        (unsafe {
                                (*p_cache).x_stress.unwrap()
                            })(unsafe { (*p_cache).p_stress }, p_pg)
                    };
        ...
        *pp_page =
            unsafe {
                sqlite3Config.pcache2.x_fetch.unwrap()(unsafe {
                        (*p_cache).p_cache
                    }, pgno, 2)
            };
...
extern "C" fn pcache_fetch_finish_with_init(p_cache_1: *mut PCache,
    pgno: Pgno, p_page_1: *mut sqlite3_pcache_page) -> *mut PgHdr {
    let mut p_pg_hdr: *mut PgHdr = core::ptr::null_mut();
    { let _ = 0; };
    p_pg_hdr = unsafe { (*p_page_1).p_extra } as *mut PgHdr;
    { let _ = 0; };
    unsafe {
        memset(unsafe { &raw mut (*p_pg_hdr).p_dirty } as *mut (), 0,
            core::mem::size_of::<PgHdr>() as u64 -
                core::mem::offset_of!(PgHdr, p_dirty) as u64)
    };
    unsafe { (*p_pg_hdr).p_page = p_page_1 };
    unsafe { (*p_pg_hdr).p_data = unsafe { (*p_page_1).p_buf } };
    unsafe {
        (*p_pg_hdr).p_extra =
            unsafe { &raw mut *p_pg_hdr.offset(1 as isize) } as *mut ()
    };
    unsafe { memset(unsafe { (*p_pg_hdr).p_extra }, 0, 8 as u64) };
    { let _ = 0; };
    unsafe { (*p_pg_hdr).p_cache = p_cache_1 };
    unsafe { (*p_pg_hdr).pgno = pgno };
...
pub extern "C" fn sqlite3_pcache_fetch_finish(p_cache: *mut PCache,
    pgno: Pgno, p_page: *mut sqlite3_pcache_page) -> *mut PgHdr {
    let mut p_pg_hdr: *mut PgHdr = core::ptr::null_mut();
    { let _ = 0; };
    p_pg_hdr = unsafe { (*p_page).p_extra } as *mut PgHdr;
    if (unsafe { (*p_pg_hdr).p_page }).is_null() as i32 != 0 {
        return pcache_fetch_finish_with_init(p_cache, pgno, p_page);
    }
    {
        let __p = unsafe { &mut (*p_cache).n_ref_sum };
        let __t = *__p;
        *__p += 1;
        __t
    };
    {
        let __p = unsafe { &mut (*p_pg_hdr).n_ref };
        let __t = *__p;
        *__p += 1;
        __t
    };
...
extern "C" fn pcache_unpin(p: &PgHdr) -> () {
    unsafe {
        if unsafe { (*(*p).p_cache).b_purgeable } != 0 {
            unsafe {
                sqlite3Config.pcache2.x_unpin.unwrap()(unsafe {
                        (*(*p).p_cache).p_cache
                    }, (*p).p_page, 0)
            };
        }
...
extern "C" fn pcache_manage_dirty_list(p_page_1: *mut PgHdr, add_remove_1: u8)
    ...
    if add_remove_1 as i32 & 1 != 0 {
        { let _ = 0; };
        { let _ = 0; };
        if unsafe { (*p).p_synced } == p_page_1 {
            unsafe { (*p).p_synced = unsafe { (*p_page_1).p_dirty_prev } };
        }
        if !(unsafe { (*p_page_1).p_dirty_next }).is_null() {
            unsafe {
                (*unsafe { (*p_page_1).p_dirty_next }).p_dirty_prev =
                    unsafe { (*p_page_1).p_dirty_prev }
            };
        } else {
            { let _ = 0; };
            unsafe {
                (*p).p_dirty_tail = unsafe { (*p_page_1).p_dirty_prev }
            };
        }
        if !(unsafe { (*p_page_1).p_dirty_prev }).is_null() {
            unsafe {
                (*unsafe { (*p_page_1).p_dirty_prev }).p_dirty_next =
    ...
    if add_remove_1 as i32 & 2 != 0 {
        unsafe { (*p_page_1).p_dirty_prev = core::ptr::null_mut() };
        unsafe { (*p_page_1).p_dirty_next = unsafe { (*p).p_dirty } };
        if !(unsafe { (*p_page_1).p_dirty_next }).is_null() {
            { let _ = 0; };
            unsafe {
                (*unsafe { (*p_page_1).p_dirty_next }).p_dirty_prev = p_page_1
            };
        } else {
            unsafe { (*p).p_dirty_tail = p_page_1 };
            if unsafe { (*p).b_purgeable } != 0 {
                { let _ = 0; };
                unsafe { (*p).e_create = 1 as u8 };
            }
        }
        unsafe { (*p).p_dirty = p_page_1 };
        if (unsafe { (*p).p_synced }).is_null() as i32 != 0 &&
                0 == unsafe { (*p_page_1).flags } as i32 & 8 {
            unsafe { (*p).p_synced = p_page_1 };
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_pcache_release(p: *mut PgHdr) -> () {
    { let _ = 0; };
    {
        let __p = unsafe { &mut (*unsafe { (*p).p_cache }).n_ref_sum };
        let __t = *__p;
        *__p -= 1;
        __t
    };
    if { let __p = unsafe { &mut (*p).n_ref }; *__p -= 1; *__p } == 0 as i64 {
        if unsafe { (*p).flags } as i32 & 1 != 0 {
            pcache_unpin(unsafe { &*p });
...
pub extern "C" fn sqlite3_pcache_drop(p: *mut PgHdr) -> () {
    unsafe {
        ...
        if unsafe { (*p).flags } as i32 & 2 != 0 {
            pcache_manage_dirty_list(p, 1 as u8);
        }
        {
            let __p = unsafe { &mut (*unsafe { (*p).p_cache }).n_ref_sum };
            let __t = *__p;
...
pub extern "C" fn sqlite3_pcache_make_dirty(p: *mut PgHdr) -> () {
    ...
    if unsafe { (*p).flags } as i32 & (1 | 16) != 0 {
        ...
        if unsafe { (*p).flags } as i32 & 1 != 0 {
            ...
            { let _ = 0; };
            pcache_manage_dirty_list(p, 2 as u8);
            { let _ = 0; };
        }
        { let _ = 0; };
    }
...
pub extern "C" fn sqlite3_pcache_clean_all(p_cache: &PCache) -> () {
    ...
    while { p = (*p_cache).p_dirty; p } != core::ptr::null_mut() {
...
pub extern "C" fn sqlite3_pcache_clear_writable(p_cache: &mut PCache) -> () {
    let mut p: *mut PgHdr = core::ptr::null_mut();
    {
        ...
        '__b3: loop {
            ...
            '__c3: loop {
                unsafe { (*p).flags &= !(8 | 4) as u16 };
                break '__c3;
            }
            p = unsafe { (*p).p_dirty_next };
        }
    }
...
pub extern "C" fn sqlite3_pcache_move(p: *mut PgHdr, new_pgno: Pgno) -> () {
    unsafe {
        let p_cache: *mut PCache = unsafe { (*p).p_cache };
        let mut p_other: *mut sqlite3_pcache_page = core::ptr::null_mut();
        { let _ = 0; };
        { let _ = 0; };
        { let _ = 0; };
        p_other =
        ...
        if !(p_other).is_null() {
            let p_x_page: *mut PgHdr =
                unsafe { (*p_other).p_extra } as *mut PgHdr;
            { let _ = 0; };
            {
                let __p = unsafe { &mut (*p_x_page).n_ref };
                let __t = *__p;
                *__p += 1;
                __t
            };
            ...
            sqlite3_pcache_drop(p_x_page);
        }
        unsafe {
            sqlite3Config.pcache2.x_rekey.unwrap()(unsafe {
                    (*p_cache).p_cache
                }, unsafe { (*p).p_page }, unsafe { (*p).pgno }, new_pgno)
        };
        unsafe { (*p).pgno = new_pgno };
        if unsafe { (*p).flags } as i32 & 2 != 0 &&
                unsafe { (*p).flags } as i32 & 8 != 0 {
            pcache_manage_dirty_list(p, 3 as u8);
            { let _ = 0; };
        }
    }
}
...
pub extern "C" fn sqlite3_pcache_truncate(p_cache: &PCache, mut pgno: Pgno)
    ...
    unsafe {
        if !((*p_cache).p_cache).is_null() {
            ...
            if pgno == 0 as u32 && (*p_cache).n_ref_sum != 0 {
                let mut p_page1: *const sqlite3_pcache_page =
                    core::ptr::null();
                p_page1 =
                    unsafe {
                        sqlite3Config.pcache2.x_fetch.unwrap()((*p_cache).p_cache,
            ...
            unsafe {
                sqlite3Config.pcache2.x_truncate.unwrap()((*p_cache).p_cache,
                    pgno + 1 as Pgno)
            };
        }
    }
...
extern "C" fn pcache_merge_dirty_list(mut p_a_1: *mut PgHdr,
    ...
    let mut p_tail: *mut PgHdr = core::ptr::null_mut();
    p_tail = &mut result;
    { let _ = 0; };
    {
        '__b5: loop {
            '__c5: loop {
                if unsafe { (*p_a_1).pgno } < unsafe { (*p_b_1).pgno } {
                    unsafe { (*p_tail).p_dirty = p_a_1 };
                    p_tail = p_a_1;
                    p_a_1 = unsafe { (*p_a_1).p_dirty };
                    if p_a_1 == core::ptr::null_mut() {
                        unsafe { (*p_tail).p_dirty = p_b_1 };
                        break '__b5;
                    }
                } else {
                    unsafe { (*p_tail).p_dirty = p_b_1 };
                    p_tail = p_b_1;
                    p_b_1 = unsafe { (*p_b_1).p_dirty };
                    if p_b_1 == core::ptr::null_mut() {
                        unsafe { (*p_tail).p_dirty = p_a_1 };
                        break '__b5;
                    }
                }
...
extern "C" fn pcache_sort_dirty_list(mut p_in_1: *mut PgHdr) -> *mut PgHdr {
    ...
    let mut i: i32 = 0;
    unsafe {
        memset(&raw mut a[0 as usize] as *mut *mut PgHdr as *mut (), 0,
            core::mem::size_of::<[*mut PgHdr; 32]>() as u64)
    };
    while !(p_in_1).is_null() {
        p = p_in_1;
        p_in_1 = unsafe { (*p).p_dirty };
        unsafe { (*p).p_dirty = core::ptr::null_mut() };
        {
            i = 0;
            '__b7: loop {
                if !(i < 32 - 1) { break '__b7; }
                '__c7: loop {
                    if a[i as usize] == core::ptr::null_mut() {
                        a[i as usize] = p;
                        break '__b7;
                    } else {
                        p = pcache_merge_dirty_list(a[i as usize], p);
    ...
    p = a[0 as usize];
    {
        ...
        '__b8: loop {
            if !(i < 32) { break '__b8; }
            '__c8: loop {
                if a[i as usize] == core::ptr::null_mut() { break '__c8; }
                p =
                    if !(p).is_null() {
                        pcache_merge_dirty_list(p, a[i as usize])
                    } else { a[i as usize] };
                break '__c8;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
...
pub extern "C" fn sqlite3_pcache_dirty_list(p_cache: &PCache) -> *mut PgHdr {
    let mut p: *mut PgHdr = core::ptr::null_mut();
    {
        ...
        '__b9: loop {
            ...
            '__c9: loop {
                unsafe { (*p).p_dirty = unsafe { (*p).p_dirty_next } };
                break '__c9;
            }
            p = unsafe { (*p).p_dirty_next };
        }
...
pub extern "C" fn sqlite3_pcache_close(p_cache: &PCache) -> () {
    unsafe {
        { let _ = 0; };
        unsafe {
            sqlite3Config.pcache2.x_destroy.unwrap()((*p_cache).p_cache)
        };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_pcache_clear_sync_flags(p_cache: &mut PCache)
    ...
    let mut p: *mut PgHdr = core::ptr::null_mut();
    {
        ...
        '__b10: loop {
            ...
            '__c10: loop { unsafe { (*p).flags &= !8 as u16 }; break '__c10; }
            p = unsafe { (*p).p_dirty_next };
        }
    }
    (*p_cache).p_synced = (*p_cache).p_dirty_tail;
...
pub extern "C" fn sqlite3_pcache_ref(p: &mut PgHdr) -> () {
    { let _ = 0; };
    { let _ = 0; };
    { let __p = &mut (*p).n_ref; let __t = *__p; *__p += 1; __t };
    {
        let __p = unsafe { &mut (*(*p).p_cache).n_ref_sum };
        let __t = *__p;
        *__p += 1;
        __t
    };
...
pub extern "C" fn sqlite3_pcache_set_cachesize(p_cache: *mut PCache,
    ...
    unsafe {
        { let _ = 0; };
        unsafe { (*p_cache).sz_cache = mx_page };
        unsafe {
            sqlite3Config.pcache2.x_cachesize.unwrap()(unsafe {
                    (*p_cache).p_cache
                }, number_of_cache_pages(unsafe { &*p_cache }))
        };
    }
...
pub extern "C" fn sqlite3_pcache_set_spillsize(p: *mut PCache,
    ...
    if mx_page != 0 {
        if mx_page < 0 {
            mx_page =
                (-1024 as i64 * mx_page as i64 /
                        (unsafe { (*p).sz_page } + unsafe { (*p).sz_extra }) as i64)
                    as i32;
        }
        unsafe { (*p).sz_spill = mx_page };
    }
    res = number_of_cache_pages(unsafe { &*p });
...
pub extern "C" fn sqlite3_pcache_shrink(p_cache: &PCache) -> () {
    unsafe {
        { let _ = 0; };
        unsafe {
            sqlite3Config.pcache2.x_shrink.unwrap()((*p_cache).p_cache)
        };
    }
}
#[unsafe(no_mangle)]
...
pub extern "C" fn sqlite3_p_cache_percent_dirty(p_cache: *mut PCache) -> i32 {
    ...
    let n_cache: i32 = number_of_cache_pages(unsafe { &*p_cache });
    {
        p_dirty = unsafe { (*p_cache).p_dirty };
        '__b11: loop {
            if !(!(p_dirty).is_null()) { break '__b11; }
            '__c11: loop {
                { let __p = &mut n_dirty; let __t = *__p; *__p += 1; __t };
                break '__c11;
            }
            p_dirty = unsafe { (*p_dirty).p_dirty_next };
        }
    }
    return if n_cache != 0 {
