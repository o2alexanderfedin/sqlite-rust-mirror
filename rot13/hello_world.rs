...
extern "C" fn rot13(mut c: u8) -> u8 {
    if c as i32 >= 'a' as i32 && c as i32 <= 'z' as i32 {
        c += 13 as u8;
        if c as i32 > 'z' as i32 { c -= 26 as u8; }
...
extern "C" fn rot13func(context: *mut sqlite3_context, argc: i32,
    argv: *mut *mut sqlite3_value) -> () {
    let mut z_in: *const u8 = core::ptr::null();
    let mut n_in: i32 = 0;
    let mut z_out: *mut u8 = core::ptr::null_mut();
    let mut z_to_free: *mut u8 = core::ptr::null_mut();
    let mut i: i32 = 0;
    let mut z_temp: [u8; 100] = [0; 100];
    if !(argc == 1) as i32 as i64 != 0 {
        unsafe {
            __assert_rtn(c"rot13func".as_ptr() as *const i8,
                c"rot13.c".as_ptr() as *mut i8 as *const i8, 53,
    ...
    if (n_in as u64) < core::mem::size_of::<[u8; 100]>() as u64 - 1 as u64 {
        z_out = &raw mut z_temp[0 as usize] as *mut u8;
    } else {
        z_out =
            {
                z_to_free =
    ...
    }
    {
        i = 0;
        '__b0: loop {
            if !(i < n_in) { break '__b0; }
            '__c0: loop {
                unsafe {
                    *z_out.offset(i as isize) =
                        rot13(unsafe { *z_in.offset(i as isize) })
                };
                break '__c0;
            }
    ...
    unsafe { *z_out.offset(i as isize) = 0 as u8 };
    unsafe {
        sqlite3_result_text(context, z_out as *mut i8 as *const i8, i,
            Some(unsafe {
...
extern "C" fn rot13_coll_func(not_used_1: *mut (), n_key1_1: i32,
    ...
    let z_b: *const i8 = p_key2_1 as *const i8;
    let mut i: i32 = 0;
    let mut x: i32 = 0;
    {
        i = 0;
        '__b1: loop {
            if !(i < n_key1_1 && i < n_key2_1) { break '__b1; }
            '__c1: loop {
                x =
                    rot13(unsafe { *z_a.offset(i as isize) } as u8) as i32 -
                        rot13(unsafe { *z_b.offset(i as isize) } as u8) as i32;
                if x != 0 { return x; }
                break '__c1;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
        ...
        rc =
            unsafe {
                sqlite3_create_collation(db,
                    c"rot13".as_ptr() as *mut i8 as *const i8, 1,
                    core::ptr::null_mut(), Some(rot13_coll_func))
            };
    }
    return rc;
}
