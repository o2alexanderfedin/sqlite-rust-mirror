...
struct _malloc_zone_t {
    reserved1: *mut (),
    reserved2: *mut (),
    size: Option<unsafe extern "C" fn(*mut _malloc_zone_t, *const ()) -> u64>,
    malloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64) -> *mut ()>,
    calloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64, u64)
        -> *mut ()>,
    valloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64) -> *mut ()>,
    free: Option<unsafe extern "C" fn(*mut _malloc_zone_t, *mut ()) -> ()>,
    realloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, *mut (), u64)
        -> *mut ()>,
    destroy: Option<unsafe extern "C" fn(*mut _malloc_zone_t) -> ()>,
    zone_name: *const i8,
    batch_malloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64,
        *mut *mut (), u32) -> u32>,
    batch_free: Option<unsafe extern "C" fn(*mut _malloc_zone_t, *mut *mut (),
        u32) -> ()>,
    introspect: *mut malloc_introspection_t,
    version: u32,
    memalign: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64, u64)
        -> *mut ()>,
    free_definite_size: Option<unsafe extern "C" fn(*mut _malloc_zone_t,
        *mut (), u64) -> ()>,
    pressure_relief: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64)
        -> u64>,
    claimed_address: Option<unsafe extern "C" fn(*mut _malloc_zone_t, *mut ())
        -> i32>,
    try_free_default: Option<unsafe extern "C" fn(*mut _malloc_zone_t,
        *mut ()) -> ()>,
    malloc_with_options: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64,
        u64, u64) -> *mut ()>,
    malloc_type_malloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64,
        u64) -> *mut ()>,
    malloc_type_calloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t, u64,
        u64, u64) -> *mut ()>,
    malloc_type_realloc: Option<unsafe extern "C" fn(*mut _malloc_zone_t,
        *mut (), u64, u64) -> *mut ()>,
    malloc_type_memalign: Option<unsafe extern "C" fn(*mut _malloc_zone_t,
        u64, u64, u64) -> *mut ()>,
    malloc_type_malloc_with_options: Option<unsafe extern "C" fn(*mut _malloc_zone_t,
        u64, u64, u64, u64) -> *mut ()>,
}
type malloc_zone_t = _malloc_zone_t;
#[repr(C)]
#[derive(Copy, Clone)]
struct sqlite3 {
...
extern "C" fn sqlite3_mem_malloc(n_byte_1: i32) -> *mut () {
    unsafe {
        ...
        p = unsafe { malloc_zone_malloc(_sqlite_zone_, n_byte_1 as u64) };
        if p == core::ptr::null_mut() {
            unsafe {
                sqlite3_log(7,
                    c"failed to allocate %u bytes of memory".as_ptr() as *mut i8
                        as *const i8, n_byte_1)
            };
        }
        return p;
    }
...
extern "C" fn sqlite3_mem_realloc(p_prior_1: *mut (), n_byte_1: i32)
    ...
    unsafe {
        let p: *mut () =
            unsafe {
                malloc_zone_realloc(_sqlite_zone_, p_prior_1, n_byte_1 as u64)
            };
        if p == core::ptr::null_mut() {
            unsafe {
                sqlite3_log(7,
                    c"failed memory resize %u to %u bytes".as_ptr() as *mut i8
                        as *const i8,
                    if !(_sqlite_zone_).is_null() {
                        unsafe {
                            (unsafe {
                                    (*_sqlite_zone_).size.unwrap()
                                })(_sqlite_zone_ as *mut _malloc_zone_t,
                                p_prior_1 as *const ())
                        }
                    } else { unsafe { malloc_size(p_prior_1 as *const ()) } },
                    n_byte_1)
            };
        }
        return p;
...
extern "C" fn sqlite3_mem_size(p_prior_1: *mut ()) -> i32 {
    unsafe {
        { let _ = 0; };
        return if !(_sqlite_zone_).is_null() {
                    unsafe {
                        (unsafe {
                                (*_sqlite_zone_).size.unwrap()
                            })(_sqlite_zone_ as *mut _malloc_zone_t,
                ...
                } else { unsafe { malloc_size(p_prior_1 as *const ()) } } as
                i32;
    }
}
...
extern "C" fn sqlite3_mem_init(not_used_1: *mut ()) -> i32 {
    unsafe {
        ...
        if cpu_count > 1 {
            ...
            _sqlite_zone_ =
                unsafe { malloc_create_zone(4096 as vm_size_t, 0 as u32) };
            unsafe {
...
pub extern "C" fn sqlite3_mem_set_default() -> () {
    unsafe {
        unsafe {
            sqlite3_config(4,
                &raw const default_methods as *const sqlite3_mem_methods)
        };
    }
}
static mut default_methods: sqlite3_mem_methods =
    sqlite3_mem_methods {
        x_malloc: Some(sqlite3_mem_malloc),
        x_free: Some(sqlite3_mem_free),
        x_realloc: Some(sqlite3_mem_realloc),
        x_size: Some(sqlite3_mem_size),
        x_roundup: Some(sqlite3_mem_roundup),
        x_init: Some(sqlite3_mem_init),
...
extern "C" {
    ...
    -> ();
    fn malloc_zone_malloc(zone: *mut malloc_zone_t, size: u64)
    -> *mut ();
    fn malloc_zone_free(zone: *mut malloc_zone_t, ptr: *mut ())
    -> ();
    fn malloc_zone_realloc(zone: *mut malloc_zone_t, ptr: *mut (), size: u64)
    -> *mut ();
    fn malloc_size(ptr: *const ())
    -> u64;
    fn sysctlbyname(_: *const i8, _: *mut (), oldlenp: *mut u64, _: *mut (),
    newlen: u64)
    -> i32;
    fn malloc_default_zone()
    -> *mut malloc_zone_t;
    fn malloc_create_zone(start_size: vm_size_t, flags: u32)
    -> *mut malloc_zone_t;
    fn malloc_set_zone_name(zone: *mut malloc_zone_t, name: *const i8)
    -> ();
