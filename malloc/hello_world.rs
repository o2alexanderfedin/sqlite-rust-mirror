...
struct Mem0Global {
    mutex: *mut sqlite3_mutex,
    alarm_threshold: sqlite3_int64,
    hard_limit: sqlite3_int64,
    nearly_full: i32,
}
static mut mem0: Mem0Global =
    Mem0Global {
        mutex: core::ptr::null_mut(),
        alarm_threshold: 0 as sqlite3_int64,
        hard_limit: 0 as sqlite3_int64,
        nearly_full: 0,
    };
#[unsafe(no_mangle)]
...
extern "C" fn sqlite3_malloc_alarm(n_byte_1: i32) -> () {
    unsafe {
        ...
        sqlite3_release_memory(n_byte_1);
...
extern "C" fn malloc_with_alarm(n: i32, pp: &mut *mut ()) -> () {
    unsafe {
        ...
        if mem0.alarm_threshold > 0 as i64 {
            let mut n_used: sqlite3_int64 =
                unsafe { sqlite3_status_value(0) };
            if n_used >= mem0.alarm_threshold - n_full as sqlite3_int64 {
                unsafe {
                    std::sync::atomic::AtomicI32::from_ptr(&raw mut mem0.nearly_full
                                as
                                *mut i32).store(1 as i32,
                        std::sync::atomic::Ordering::Relaxed)
                };
                sqlite3_malloc_alarm(n_full);
                if mem0.hard_limit != 0 {
                    n_used = unsafe { sqlite3_status_value(0) };
                    if n_used >= mem0.hard_limit - n_full as sqlite3_int64 {
        ...
        p = unsafe { sqlite3Config.m.x_malloc.unwrap()(n_full) };
        if !(p).is_null() {
            n_full = sqlite3_malloc_size(p as *const ());
            unsafe { sqlite3_status_up(0, n_full) };
            unsafe { sqlite3_status_up(9, 1) };
        }
        *pp = p;
    }
}
...
pub extern "C" fn sqlite3Malloc(n: u64) -> *mut () {
    unsafe {
        let mut p: *mut () = core::ptr::null_mut();
        if n == 0 as u64 || n > 2147483391 as u64 {
            p = core::ptr::null_mut();
        } else if sqlite3Config.b_memstat != 0 {
            unsafe { sqlite3_mutex_enter(mem0.mutex) };
            malloc_with_alarm(n as i32, &mut p);
            unsafe { sqlite3_mutex_leave(mem0.mutex) };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_malloc64(n: sqlite3_uint64) -> *mut () {
    if unsafe { sqlite3_initialize() } != 0 { return core::ptr::null_mut(); }
...
pub extern "C" fn sqlite3_free(p: *mut ()) -> () {
    unsafe {
        ...
        if sqlite3Config.b_memstat != 0 {
            unsafe { sqlite3_mutex_enter(mem0.mutex) };
            unsafe {
                sqlite3_status_down(0, sqlite3_malloc_size(p as *const ()))
            };
            unsafe { sqlite3_status_down(9, 1) };
            unsafe { sqlite3Config.m.x_free.unwrap()(p) };
            unsafe { sqlite3_mutex_leave(mem0.mutex) };
...
pub extern "C" fn sqlite3Realloc(p_old: *mut (), n_bytes: u64) -> *mut () {
    unsafe {
        ...
        if n_old == n_new {
            ...
            let mut n_used: sqlite3_int64 = 0 as sqlite3_int64;
            unsafe { sqlite3_mutex_enter(mem0.mutex) };
            unsafe { sqlite3_status_highwater(5, n_bytes as i32) };
            n_diff = n_new - n_old;
            if n_diff > 0 &&
                    { n_used = unsafe { sqlite3_status_value(0) }; n_used } >=
                        mem0.alarm_threshold - n_diff as sqlite3_int64 {
                sqlite3_malloc_alarm(n_diff);
                if mem0.hard_limit > 0 as i64 &&
                        n_used >= mem0.hard_limit - n_diff as sqlite3_int64 {
                    unsafe { sqlite3_mutex_leave(mem0.mutex) };
                    return core::ptr::null_mut();
                }
            }
            ...
            if !(p_new).is_null() {
                n_new = sqlite3_malloc_size(p_new as *const ());
                unsafe { sqlite3_status_up(0, n_new - n_old) };
            }
            unsafe { sqlite3_mutex_leave(mem0.mutex) };
        } else {
            p_new =
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_realloc64(p_old: *mut (), n: sqlite3_uint64)
    -> *mut () {
    if unsafe { sqlite3_initialize() } != 0 { return core::ptr::null_mut(); }
    return sqlite3Realloc(p_old, n);
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_msize(p: *mut ()) -> sqlite3_uint64 {
    unsafe {
        { let _ = 0; };
        { let _ = 0; };
        return if !(p).is_null() {
                    unsafe { sqlite3Config.m.x_size.unwrap()(p) }
                } else { 0 } as sqlite3_uint64;
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_memory_used() -> sqlite3_int64 {
    let mut res: sqlite3_int64 = 0 as sqlite3_int64;
    let mut mx: sqlite3_int64 = 0 as sqlite3_int64;
    unsafe { sqlite3_status64(0, &mut res, &mut mx, 0) };
    return res;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_memory_highwater(reset_flag: i32) -> sqlite3_int64 {
    let mut res: sqlite3_int64 = 0 as sqlite3_int64;
    let mut mx: sqlite3_int64 = 0 as sqlite3_int64;
...
pub extern "C" fn sqlite3_memory_alarm(x_callback:
    ...
    i_threshold: sqlite3_int64) -> i32 {
    {
        let _ =
            x_callback.unwrap_or(unsafe {
                    core::mem::transmute::<*const (),
                            unsafe extern "C" fn(*mut (), i64, i32)
                                -> ()>(0 as *const ())
                });
    };
    { let _ = p_arg; };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_soft_heap_limit64(mut n: sqlite3_int64)
    -> sqlite3_int64 {
    unsafe {
        let mut prior_limit: sqlite3_int64 = 0 as sqlite3_int64;
        let mut excess: sqlite3_int64 = 0 as sqlite3_int64;
        let mut n_used: sqlite3_int64 = 0 as sqlite3_int64;
        let rc: i32 = unsafe { sqlite3_initialize() };
        if rc != 0 { return -1 as sqlite3_int64; }
        unsafe { sqlite3_mutex_enter(mem0.mutex) };
        prior_limit = mem0.alarm_threshold;
        if n < 0 as i64 {
            unsafe { sqlite3_mutex_leave(mem0.mutex) };
            return prior_limit;
        }
        if mem0.hard_limit > 0 as i64 &&
                (n > mem0.hard_limit || n == 0 as i64) {
            n = mem0.hard_limit;
        }
        mem0.alarm_threshold = n;
        n_used = unsafe { sqlite3_status_value(0) };
        unsafe {
            std::sync::atomic::AtomicI32::from_ptr(&raw mut mem0.nearly_full
                        as
                        *mut i32).store((n > 0 as i64 && n <= n_used) as i32,
                std::sync::atomic::Ordering::Relaxed)
        };
        unsafe { sqlite3_mutex_leave(mem0.mutex) };
        excess = sqlite3_memory_used() - n;
        if excess > 0 as i64 {
            sqlite3_release_memory((excess & 2147483647 as sqlite3_int64) as
                    i32);
        }
        return prior_limit;
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_hard_heap_limit64(n: sqlite3_int64)
    -> sqlite3_int64 {
    unsafe {
        let mut prior_limit: sqlite3_int64 = 0 as sqlite3_int64;
        let rc: i32 = unsafe { sqlite3_initialize() };
        if rc != 0 { return -1 as sqlite3_int64; }
        unsafe { sqlite3_mutex_enter(mem0.mutex) };
        prior_limit = mem0.hard_limit;
        if n >= 0 as i64 {
            mem0.hard_limit = n;
            if n < mem0.alarm_threshold || mem0.alarm_threshold == 0 as i64 {
                mem0.alarm_threshold = n;
            }
        }
        unsafe { sqlite3_mutex_leave(mem0.mutex) };
...
pub extern "C" fn sqlite3_malloc_init() -> i32 {
    unsafe {
        ...
        mem0.mutex = unsafe { sqlite3MutexAlloc(3) };
        if sqlite3Config.p_page == core::ptr::null_mut() ||
                    sqlite3Config.sz_page < 512 || sqlite3Config.n_page <= 0 {
            sqlite3Config.p_page = core::ptr::null_mut();
            sqlite3Config.sz_page = 0;
        }
        rc =
            unsafe {
                sqlite3Config.m.x_init.unwrap()(sqlite3Config.m.p_app_data)
            };
        if rc != 0 {
            unsafe {
                memset(&raw mut mem0 as *mut (), 0,
                    core::mem::size_of::<Mem0Global>() as u64)
            };
        }
        return rc;
    }
...
pub extern "C" fn sqlite3_malloc_end() -> () {
    unsafe {
        if sqlite3Config.m.x_shutdown.is_some() {
            unsafe {
                sqlite3Config.m.x_shutdown.unwrap()(sqlite3Config.m.p_app_data)
            };
        }
        unsafe {
            memset(&raw mut mem0 as *mut (), 0,
...
pub extern "C" fn sqlite3_oom_fault(db: &mut sqlite3) -> *mut () {
    unsafe {
        if (*db).malloc_failed as i32 == 0 &&
                (*db).b_benign_malloc as i32 == 0 {
            (*db).malloc_failed = 1 as u8;
            if (*db).n_vdbe_exec > 0 {
                unsafe {
            ...
            {
                let __p = &mut (*db).lookaside.b_disable;
                let __t = *__p;
                *__p += 1;
                __t
            };
            (*db).lookaside.sz = 0 as u16;
            if !((*db).p_parse).is_null() {
                let mut p_parse: *mut Parse = core::ptr::null_mut();
                unsafe {
                    sqlite3_error_msg((*db).p_parse,
                        c"out of memory".as_ptr() as *mut i8 as *const i8)
                };
                unsafe { (*(*db).p_parse).rc = 7 };
                {
                    p_parse = unsafe { (*(*db).p_parse).p_outer_parse };
                    '__b0: loop {
                        if !(!(p_parse).is_null()) { break '__b0; }
                        '__c0: loop {
                            {
                        ...
                        p_parse = unsafe { (*p_parse).p_outer_parse };
                    }
                }
            }
        }
...
pub extern "C" fn sqlite3_db_malloc_raw_nn(db: *mut sqlite3, n: u64)
    -> *mut () {
    unsafe {
        let mut p_buf: *mut LookasideSlot = core::ptr::null_mut();
        { let _ = 0; };
        { let _ = 0; };
        { let _ = 0; };
        if n > unsafe { (*db).lookaside.sz } as u64 {
            if (unsafe { (*db).lookaside.b_disable } == 0) as i32 != 0 {
                {
                    let __p =
                        unsafe { &mut (*db).lookaside.an_stat[1 as usize] };
                    let __t = *__p;
                    *__p += 1;
                    __t
                };
            } else if unsafe { (*db).malloc_failed } != 0 {
                return core::ptr::null_mut();
            }
            return db_malloc_raw_finish(db, n);
        }
        if n <= 128 as u64 {
            if { p_buf = unsafe { (*db).lookaside.p_small_free }; p_buf } !=
                    core::ptr::null_mut() {
                unsafe {
                    (*db).lookaside.p_small_free = unsafe { (*p_buf).p_next }
                };
                {
                    let __p =
                        unsafe { &mut (*db).lookaside.an_stat[0 as usize] };
                    let __t = *__p;
                    *__p += 1;
                    __t
                };
                return p_buf as *mut ();
                        ...
                        p_buf = unsafe { (*db).lookaside.p_small_init };
                        p_buf
                    } != core::ptr::null_mut() {
                unsafe {
                    (*db).lookaside.p_small_init = unsafe { (*p_buf).p_next }
                };
                {
                    let __p =
                        unsafe { &mut (*db).lookaside.an_stat[0 as usize] };
        ...
        if { p_buf = unsafe { (*db).lookaside.p_free }; p_buf } !=
                core::ptr::null_mut() {
            unsafe { (*db).lookaside.p_free = unsafe { (*p_buf).p_next } };
            {
                let __p = unsafe { &mut (*db).lookaside.an_stat[0 as usize] };
                let __t = *__p;
                *__p += 1;
                __t
            };
            return p_buf as *mut ();
        } else if { p_buf = unsafe { (*db).lookaside.p_init }; p_buf } !=
                core::ptr::null_mut() {
            unsafe { (*db).lookaside.p_init = unsafe { (*p_buf).p_next } };
            {
                let __p = unsafe { &mut (*db).lookaside.an_stat[0 as usize] };
                let __t = *__p;
                *__p += 1;
            ...
            {
                let __p = unsafe { &mut (*db).lookaside.an_stat[2 as usize] };
                let __t = *__p;
                *__p += 1;
                __t
            };
...
pub extern "C" fn sqlite3_db_malloc_raw(db: *mut sqlite3, n: u64) -> *mut () {
    let mut p: *mut () = core::ptr::null_mut();
    if !(db).is_null() { return sqlite3_db_malloc_raw_nn(db, n); }
    p = sqlite3Malloc(n);
    return p;
}
#[unsafe(no_mangle)]
...
pub extern "C" fn sqlite3_db_str_dup(db: *mut sqlite3, z: *const i8)
    ...
    n = unsafe { strlen(z) } + 1 as u64;
    z_new = sqlite3_db_malloc_raw(db, n) as *mut i8;
    if !(z_new).is_null() {
        unsafe { memcpy(z_new as *mut (), z as *const (), n) };
    }
    return z_new;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_db_str_n_dup(db: *mut sqlite3, z: *const i8, n: u64)
    ...
    z_new =
        if !(z).is_null() {
                sqlite3_db_malloc_raw_nn(db, n + 1 as u64)
            } else { core::ptr::null_mut() } as *mut i8;
    if !(z_new).is_null() {
        unsafe { memcpy(z_new as *mut (), z as *const (), n as u64) };
        unsafe { *z_new.add(n as usize) = 0 as i8 };
    }
    return z_new;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_db_span_dup(db: *mut sqlite3,
    ...
    unsafe {
        ...
        n = unsafe { z_end.offset_from(z_start) } as i64 as i32;
        while unsafe {
                            *(sqlite3_ctype_map.as_ptr() as
                                        *const u8).add(unsafe { *z_start.offset((n - 1) as isize) }
                                            as u8 as usize)
                        } as i32 & 1 != 0 {
...
pub extern "C" fn sqlite3_db_malloc_size(db: *mut sqlite3, p: *const ())
    ...
    unsafe {
        { let _ = 0; };
        if !(db).is_null() {
            if (p as uptr) < unsafe { (*db).lookaside.p_true_end } as uptr {
                if p as uptr >= unsafe { (*db).lookaside.p_middle } as uptr {
                    { let _ = 0; };
                    return 128;
                }
                if p as uptr >= unsafe { (*db).lookaside.p_start } as uptr {
                    { let _ = 0; };
                    return unsafe { (*db).lookaside.sz_true } as i32;
                }
            }
        }
        return unsafe { sqlite3Config.m.x_size.unwrap()(p as *mut ()) };
...
pub extern "C" fn sqlite3_db_free_nn(db: *mut sqlite3, p: *mut ()) -> () {
    ...
    { let _ = 0; };
    if !(db).is_null() {
        if (p as uptr) < unsafe { (*db).lookaside.p_end } as uptr {
            if p as uptr >= unsafe { (*db).lookaside.p_middle } as uptr {
                let p_buf: *mut LookasideSlot = p as *mut LookasideSlot;
                { let _ = 0; };
                unsafe {
                    (*p_buf).p_next = unsafe { (*db).lookaside.p_small_free }
                };
                unsafe { (*db).lookaside.p_small_free = p_buf };
                return;
            }
            if p as uptr >= unsafe { (*db).lookaside.p_start } as uptr {
                let p_buf_1: *mut LookasideSlot = p as *mut LookasideSlot;
                { let _ = 0; };
                unsafe {
                    (*p_buf_1).p_next = unsafe { (*db).lookaside.p_free }
                };
                unsafe { (*db).lookaside.p_free = p_buf_1 };
                return;
            }
        }
...
extern "C" fn db_realloc_finish(db: *mut sqlite3, p: *mut (), n: u64)
    ...
    if unsafe { (*db).malloc_failed } as i32 == 0 {
        if is_lookaside(unsafe { &*db }, p as *const ()) != 0 {
            p_new = sqlite3_db_malloc_raw_nn(db, n);
            if !(p_new).is_null() {
                unsafe {
                    memcpy(p_new, p as *const (),
                        lookaside_malloc_size(unsafe { &*db }, p as *const ()) as
                            u64)
                };
                sqlite3_db_free(db, p);
            }
        } else {
            { let _ = 0; };
            { let _ = 0; };
            p_new = sqlite3Realloc(p, n);
            if (p_new).is_null() as i32 != 0 {
                sqlite3_oom_fault(unsafe { &mut *db });
            }
        }
    }
    return p_new;
...
pub extern "C" fn sqlite3_db_realloc(db: *mut sqlite3, p: *mut (), n: u64)
    -> *mut () {
    { let _ = 0; };
    if p == core::ptr::null_mut() { return sqlite3_db_malloc_raw_nn(db, n); }
    { let _ = 0; };
    if (p as uptr) < unsafe { (*db).lookaside.p_end } as uptr {
        if p as uptr >= unsafe { (*db).lookaside.p_middle } as uptr {
            if n <= 128 as u64 { return p; }
        } else if p as uptr >= unsafe { (*db).lookaside.p_start } as uptr {
            if n <= unsafe { (*db).lookaside.sz_true } as u64 { return p; }
        }
    }
...
pub extern "C" fn sqlite3_db_realloc_or_free(db: *mut sqlite3, p: *mut (),
    n: u64) -> *mut () {
    let mut p_new: *mut () = core::ptr::null_mut();
    p_new = sqlite3_db_realloc(db, p, n);
...
pub extern "C" fn sqlite3_db_nn_free_nn(db: *mut sqlite3, p: *mut ()) -> () {
    ...
    if (p as uptr) < unsafe { (*db).lookaside.p_end } as uptr {
        ...
        if p as uptr >= unsafe { (*db).lookaside.p_start } as uptr {
            let p_buf_1: *mut LookasideSlot = p as *mut LookasideSlot;
            { let _ = 0; };
            unsafe { (*p_buf_1).p_next = unsafe { (*db).lookaside.p_free } };
            unsafe { (*db).lookaside.p_free = p_buf_1 };
            return;
        }
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_malloc_mutex() -> *mut sqlite3_mutex {
    unsafe { return mem0.mutex; }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_set_string(pz: &mut *mut i8, db: *mut sqlite3,
    z_new: *const i8) -> () {
    let z: *mut i8 = sqlite3_db_str_dup(db, z_new);
        ...
        if (*db).malloc_failed != 0 && (*db).n_vdbe_exec == 0 {
            ...
            (*db).lookaside.sz =
                if (*db).lookaside.b_disable != 0 {
                        0
                    } else { (*db).lookaside.sz_true as i32 } as u16;
        }
    }
}
