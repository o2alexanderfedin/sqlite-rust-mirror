...
extern "C" fn compress_func(context: *mut sqlite3_context, argc: i32,
    ...
    p_in =
        unsafe { sqlite3_value_blob(unsafe { *argv.offset(0 as isize) }) } as
            *const u8;
    n_in =
        unsafe { sqlite3_value_bytes(unsafe { *argv.offset(0 as isize) }) } as
            u32;
    n_out = (13 as u32 + n_in + (n_in + 999 as u32) / 1000 as u32) as u64;
    p_out = unsafe { sqlite3_malloc64(n_out + 5 as u64) } as *mut u8;
    ...
    }
    {
        ...
        '__b0: loop {
            if !(i >= 0) { break '__b0; }
            '__c0: loop {
                x[i as usize] = (n_in >> 7 * (4 - i) & 127 as u32) as u8;
                break '__c0;
            }
    ...
    }
    {
        i = 0;
        '__b1: loop {
            if !(i < 4 && x[i as usize] as i32 == 0) { break '__b1; }
    ...
    }
    {
        ...
        '__b2: loop {
            if !(i <= 4) { break '__b2; }
            '__c2: loop {
                unsafe { *p_out.offset(j as isize) = x[i as usize] };
                break '__c2;
            }
            {
                { let __p = &mut i; let __t = *__p; *__p += 1; __t };
    ...
    unsafe { *p_out.offset((j - 1) as isize) |= 128 as u8 };
    rc =
        unsafe {
            compress(unsafe { &mut *p_out.offset(j as isize) }, &mut n_out,
                p_in, n_in as uLong)
        };
    if rc == 0 {
        unsafe {
            sqlite3_result_blob(context, p_out as *const (),
                (n_out + j as u64) as i32, Some(sqlite3_free))
        };
    } else { unsafe { sqlite3_free(p_out as *mut ()) }; }
}
extern "C" fn uncompress_func(context: *mut sqlite3_context, argc: i32,
    argv: *mut *mut sqlite3_value) -> () {
    let mut p_in: *const u8 = core::ptr::null();
    let mut p_out: *mut u8 = core::ptr::null_mut();
    let mut n_in: u32 = 0 as u32;
    ...
    n_out = 0 as u64;
    {
        i = 0 as u32;
        '__b3: loop {
            if !(i < n_in && i < 5 as u32) { break '__b3; }
            '__c3: loop {
                n_out =
                    n_out << 7 |
                        (unsafe { *p_in.add(i as usize) } as i32 & 127) as u64;
                if unsafe { *p_in.add(i as usize) } as i32 & 128 != 0 {
                    { let __p = &mut i; let __t = *__p; *__p += 1; __t };
                    break '__b3;
                }
                break '__c3;
    ...
    p_out = unsafe { sqlite3_malloc64(n_out + 1 as u64) } as *mut u8;
    if p_out == core::ptr::null_mut() {
        unsafe { sqlite3_result_error_nomem(context) };
        return;
    }
    rc =
        unsafe {
            uncompress(p_out, &mut n_out, unsafe { &*p_in.add(i as usize) },
                (n_in - i) as uLong)
        };
    if rc == 0 {
        unsafe {
            sqlite3_result_blob(context, p_out as *const (), n_out as i32,
                Some(sqlite3_free))
        };
    } else { unsafe { sqlite3_free(p_out as *mut ()) }; }
}
...
pub extern "C" fn sqlite3_compress_init(db: *mut sqlite3,
    ...
    if rc == 0 {
        rc =
            unsafe {
                sqlite3_create_function(db,
                    c"uncompress".as_ptr() as *mut i8 as *const i8, 1,
                    1 | 2097152 | 2048, core::ptr::null_mut(),
                    Some(uncompress_func), None, None)
            };
    }
    return rc;
}
