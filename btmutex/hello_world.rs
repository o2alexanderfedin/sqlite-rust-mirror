...
extern "C" fn unlock_btree_mutex(p: &mut Btree) -> () {
    ...
    { let _ = 0; };
    { let _ = 0; };
    unsafe { sqlite3_mutex_leave(unsafe { (*p_bt).mutex }) };
    (*p).locked = 0 as u8;
}
extern "C" fn lock_btree_mutex(p: &mut Btree) -> () {
    ...
    { let _ = 0; };
    { let _ = 0; };
    unsafe { sqlite3_mutex_enter(unsafe { (*(*p).p_bt).mutex }) };
    unsafe { (*(*p).p_bt).db = (*p).db };
    (*p).locked = 1 as u8;
}
extern "C" fn btree_lock_carefully(p: *mut Btree) -> () {
    ...
    if unsafe { sqlite3_mutex_try(unsafe { (*unsafe { (*p).p_bt }).mutex }) }
            == 0 {
        unsafe { (*unsafe { (*p).p_bt }).db = unsafe { (*p).db } };
        unsafe { (*p).locked = 1 as u8 };
        return;
    }
    {
        p_later = unsafe { (*p).p_next };
        '__b0: loop {
            if !(!(p_later).is_null()) { break '__b0; }
            '__c0: loop {
                { let _ = 0; };
                { let _ = 0; };
                { let _ = 0; };
                if unsafe { (*p_later).locked } != 0 {
                    unlock_btree_mutex(unsafe { &mut *p_later });
                }
                break '__c0;
            }
            p_later = unsafe { (*p_later).p_next };
        }
    }
    lock_btree_mutex(unsafe { &mut *p });
    {
        p_later = unsafe { (*p).p_next };
        '__b1: loop {
            if !(!(p_later).is_null()) { break '__b1; }
            '__c1: loop {
                if unsafe { (*p_later).want_to_lock } != 0 {
                    lock_btree_mutex(unsafe { &mut *p_later });
                }
                break '__c1;
            }
            p_later = unsafe { (*p_later).p_next };
...
pub extern "C" fn sqlite3_btree_enter(p: *mut Btree) -> () {
    ...
    {
        let __p = unsafe { &mut (*p).want_to_lock };
        let __t = *__p;
        *__p += 1;
        __t
    };
    if unsafe { (*p).locked } != 0 { return; }
...
extern "C" fn btree_enter_all(db: &mut sqlite3) -> () {
    ...
    { let _ = 0; };
    {
        i = 0;
        '__b2: loop {
            if !(i < (*db).n_db) { break '__b2; }
            '__c2: loop {
                p = unsafe { (*(*db).a_db.offset(i as isize)).p_bt };
                if !(p).is_null() && unsafe { (*p).sharable } != 0 {
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_btree_enter_all(db: *mut sqlite3) -> () {
    if unsafe { (*db).no_shared_cache } as i32 == 0 {
        btree_enter_all(unsafe { &mut *db });
    }
}
#[unsafe(no_mangle)]
...
pub extern "C" fn sqlite3_btree_leave(p: *mut Btree) -> () {
    ...
    if unsafe { (*p).sharable } != 0 {
        ...
        if unsafe { (*p).want_to_lock } == 0 {
            unlock_btree_mutex(unsafe { &mut *p });
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_btree_leave_cursor(p_cur_1: &BtCursor) -> () {
    sqlite3_btree_leave((*p_cur_1).p_btree);
}
extern "C" fn btree_leave_all(db: &sqlite3) -> () {
    let mut i: i32 = 0;
    let mut p: *mut Btree = core::ptr::null_mut();
    { let _ = 0; };
    {
        i = 0;
        '__b3: loop {
            if !(i < (*db).n_db) { break '__b3; }
            '__c3: loop {
                p = unsafe { (*(*db).a_db.offset(i as isize)).p_bt };
                if !(p).is_null() { sqlite3_btree_leave(p); }
                break '__c3;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
        }
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_btree_leave_all(db: *mut sqlite3) -> () {
    if unsafe { (*db).no_shared_cache } as i32 == 0 {
        btree_leave_all(unsafe { &*db });
    }
}
#[repr(C)]
#[derive(Copy, Clone)]
