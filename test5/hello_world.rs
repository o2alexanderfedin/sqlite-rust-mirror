...
extern "C" fn binarize(client_data_1: *mut (), interp: *mut Tcl_Interp,
    objc: i32, objv: *const *mut Tcl_Obj) -> i32 {
    let mut len: i32 = 0;
    let mut bytes: *mut i8 = core::ptr::null_mut();
    ...
    p_ret =
        unsafe {
            Tcl_NewByteArrayObj(bytes as *mut u8 as *const u8, len + 1)
        };
...
extern "C" fn test_value_overhead(client_data_1: *mut (),
    ...
    unsafe {
        let mut do_calls: i32 = 0;
        let mut repeat_count: i32 = 0;
        let mut i: i32 = 0;
        let mut val: Mem = unsafe { core::mem::zeroed() };
        if objc != 3 {
            unsafe {
                Tcl_AppendResult(interp,
                    ...
                    unsafe {
                        Tcl_GetStringFromObj(unsafe { *objv.offset(0 as isize) },
                            core::ptr::null_mut())
                    }, c" <repeat-count> <do-calls>".as_ptr() as *mut i8,
                    0 as *mut ())
            };
            return 1;
        }
        if unsafe {
                    Tcl_GetIntFromObj(interp,
                        unsafe { *objv.offset(1 as isize) }, &mut repeat_count)
                } != 0 {
            return 1;
        }
        if unsafe {
                    Tcl_GetIntFromObj(interp,
                        unsafe { *objv.offset(2 as isize) }, &mut do_calls)
                } != 0 {
            return 1;
        }
        val.flags = (2 | 512 | 8192) as u16;
        val.z = c"hello world".as_ptr() as *mut i8;
        val.enc = 1 as u8;
        {
            i = 0;
            '__b0: loop {
                if !(i < repeat_count) { break '__b0; }
                '__c0: loop {
                    if do_calls != 0 {
                        unsafe {
                            sqlite3_value_text(&raw mut val as *mut sqlite3_value)
                        };
                    }
                    break '__c0;
                }
...
extern "C" fn name_to_enc(interp: *mut Tcl_Interp, p_obj_1: *mut Tcl_Obj)
    -> u8 {
    let mut encnames: [EncName_N7EncName; 5] =
        [EncName_N7EncName {
                    z_name: c"UTF8".as_ptr() as *mut i8,
                    enc: 1 as u8,
                },
                EncName_N7EncName {
    ...
    let z: *mut i8 = unsafe { Tcl_GetString(p_obj_1) };
    {
        p_enc = &mut encnames[0 as usize];
        '__b1: loop {
            if !(!(unsafe { (*p_enc).z_name }).is_null()) { break '__b1; }
            '__c1: loop {
                if 0 ==
                        unsafe {
    ...
    if (unsafe { (*p_enc).enc } == 0) as i32 != 0 {
        unsafe {
            Tcl_AppendResult(interp,
                c"No such encoding: ".as_ptr() as *mut i8, z, 0 as *mut ())
        };
    }
    if unsafe { (*p_enc).enc } as i32 == 4 { return 2 as u8; }
    return unsafe { (*p_enc).enc };
}
extern "C" fn test_translate(client_data_1: *mut (), interp: *mut Tcl_Interp,
    objc: i32, objv: *const *mut Tcl_Obj) -> i32 {
    let mut enc_from: u8 = 0 as u8;
    let mut enc_to: u8 = 0 as u8;
    let mut p_val: *mut sqlite3_value = core::ptr::null_mut();
    let mut z: *mut i8 = core::ptr::null_mut();
    let mut len: i32 = 0;
    let mut x_del: Option<unsafe extern "C" fn(*mut ()) -> ()> = None;
    if objc != 4 && objc != 5 {
        unsafe {
            Tcl_AppendResult(interp,
                c"wrong # args: should be \"".as_ptr() as *mut i8,
                unsafe {
                    Tcl_GetStringFromObj(unsafe { *objv.offset(0 as isize) },
                        core::ptr::null_mut())
                }, c" <string/blob> <from enc> <to enc>".as_ptr() as *mut i8,
                0 as *mut ())
        };
        return 1;
    }
    if objc == 5 { x_del = Some(sqlite3_free); }
    enc_from = name_to_enc(interp, unsafe { *objv.offset(2 as isize) });
    if (enc_from == 0) as i32 != 0 { return 1; }
    enc_to = name_to_enc(interp, unsafe { *objv.offset(3 as isize) });
    if (enc_to == 0) as i32 != 0 { return 1; }
    p_val = unsafe { sqlite3_value_new(core::ptr::null_mut()) };
    if enc_from as i32 == 1 {
        z = unsafe { Tcl_GetString(unsafe { *objv.offset(1 as isize) }) };
        if objc == 5 {
            z =
                unsafe {
        ...
        unsafe {
            sqlite3_value_set_str(p_val, -1, z as *const (), enc_from, x_del)
        };
    } else {
        z =
            unsafe {
        ...
        if objc == 5 {
            let z_tmp: *const i8 = z as *const i8;
            z = unsafe { sqlite3_malloc64(len as sqlite3_uint64) } as *mut i8;
            unsafe { memcpy(z as *mut (), z_tmp as *const (), len as u64) };
        }
        unsafe {
            sqlite3_value_set_str(p_val, -1, z as *const (), enc_from, x_del)
        };
    }
    z = unsafe { sqlite3ValueText(p_val, enc_to) } as *mut i8;
    len =
        unsafe { sqlite3ValueBytes(p_val, enc_to) } +
            if enc_to as i32 == 1 { 1 } else { 2 };
    unsafe {
        Tcl_SetObjResult(interp,
            unsafe { Tcl_NewByteArrayObj(z as *mut u8 as *const u8, len) })
    };
    unsafe { sqlite3ValueFree(p_val) };
    return 0;
}
extern "C" fn test_translate_selftest(client_data_1: *mut (),
    interp: *const Tcl_Interp, objc: i32, objv: *const *mut Tcl_Obj) -> i32 {
