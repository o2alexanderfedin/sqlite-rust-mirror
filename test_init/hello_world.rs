...
extern "C" fn wr_mem_init(p_app_data_1: *mut ()) -> i32 {
    unsafe {
        ...
        if wrapped.mem_fail != 0 {
            ...
            rc =
                unsafe {
                    wrapped.mem.x_init.unwrap()(wrapped.mem.p_app_data)
                };
        }
        if rc == 0 { wrapped.mem_init = 1; }
        return rc;
    }
}
...
extern "C" fn wr_mutex_init() -> i32 {
    unsafe {
        ...
        if wrapped.mutex_fail != 0 {
            rc = 1;
        } else { rc = unsafe { wrapped.mutex.x_mutex_init.unwrap()() }; }
        if rc == 0 { wrapped.mutex_init = 1; }
        return rc;
    }
}
...
extern "C" fn wr_mutex_alloc(e: i32) -> *mut sqlite3_mutex {
    unsafe { return unsafe { wrapped.mutex.x_mutex_alloc.unwrap()(e) }; }
}
extern "C" fn wr_mutex_free(p: *mut sqlite3_mutex) -> () {
    unsafe { unsafe { wrapped.mutex.x_mutex_free.unwrap()(p) }; }
}
extern "C" fn wr_mutex_enter(p: *mut sqlite3_mutex) -> () {
    unsafe { unsafe { wrapped.mutex.x_mutex_enter.unwrap()(p) }; }
}
extern "C" fn wr_mutex_try(p: *mut sqlite3_mutex) -> i32 {
    unsafe { return unsafe { wrapped.mutex.x_mutex_try.unwrap()(p) }; }
}
extern "C" fn wr_mutex_leave(p: *mut sqlite3_mutex) -> () {
    unsafe { unsafe { wrapped.mutex.x_mutex_leave.unwrap()(p) }; }
}
extern "C" fn wr_mutex_held(p: *mut sqlite3_mutex) -> i32 {
    unsafe { return unsafe { wrapped.mutex.x_mutex_held.unwrap()(p) }; }
}
extern "C" fn wr_mutex_notheld(p: *mut sqlite3_mutex) -> i32 {
...
extern "C" fn wr_p_cache_init(p_arg_1: *mut ()) -> i32 {
    unsafe {
        ...
        if wrapped.pcache_fail != 0 {
            ...
            rc =
                unsafe {
                    wrapped.pcache.x_init.unwrap()(wrapped.pcache.p_arg)
                };
        }
        if rc == 0 { wrapped.pcache_init = 1; }
        return rc;
    }
}
...
extern "C" fn wr_p_cache_cachesize(p: *mut sqlite3_pcache, n: i32) -> () {
    unsafe { unsafe { wrapped.pcache.x_cachesize.unwrap()(p, n) }; }
}
extern "C" fn wr_p_cache_pagecount(p: *mut sqlite3_pcache) -> i32 {
    unsafe { return unsafe { wrapped.pcache.x_pagecount.unwrap()(p) }; }
}
extern "C" fn wr_p_cache_fetch(p: *mut sqlite3_pcache, a: u32, b: i32)
    -> *mut sqlite3_pcache_page {
    unsafe { return unsafe { wrapped.pcache.x_fetch.unwrap()(p, a, b) }; }
}
extern "C" fn wr_p_cache_unpin(p: *mut sqlite3_pcache,
    a: *mut sqlite3_pcache_page, b: i32) -> () {
    unsafe { unsafe { wrapped.pcache.x_unpin.unwrap()(p, a, b) }; }
}
extern "C" fn wr_p_cache_rekey(p: *mut sqlite3_pcache,
    a: *mut sqlite3_pcache_page, b: u32, c: u32) -> () {
    unsafe { unsafe { wrapped.pcache.x_rekey.unwrap()(p, a, b, c) }; }
}
extern "C" fn wr_p_cache_truncate(p: *mut sqlite3_pcache, a: u32) -> () {
    unsafe { unsafe { wrapped.pcache.x_truncate.unwrap()(p, a) }; }
}
extern "C" fn wr_p_cache_destroy(p: *mut sqlite3_pcache) -> () {
    unsafe { unsafe { wrapped.pcache.x_destroy.unwrap()(p) }; }
}
extern "C" fn install_init_wrappers() -> () {
    unsafe {
        let mut mutexmethods: sqlite3_mutex_methods =
            sqlite3_mutex_methods {
                x_mutex_init: Some(wr_mutex_init),
                x_mutex_end: Some(wr_mutex_end),
                x_mutex_alloc: Some(wr_mutex_alloc),
                x_mutex_free: Some(wr_mutex_free),
                x_mutex_enter: Some(wr_mutex_enter),
                x_mutex_try: Some(wr_mutex_try),
                x_mutex_leave: Some(wr_mutex_leave),
                x_mutex_held: Some(wr_mutex_held),
                x_mutex_notheld: Some(wr_mutex_notheld),
            };
        let mut pcachemethods: sqlite3_pcache_methods2 =
            sqlite3_pcache_methods2 {
                i_version: 1,
                p_arg: core::ptr::null_mut(),
                x_init: Some(wr_p_cache_init),
                x_shutdown: Some(wr_p_cache_shutdown),
                x_create: Some(wr_p_cache_create),
                x_cachesize: Some(wr_p_cache_cachesize),
                x_pagecount: Some(wr_p_cache_pagecount),
                x_fetch: Some(wr_p_cache_fetch),
                x_unpin: Some(wr_p_cache_unpin),
                x_rekey: Some(wr_p_cache_rekey),
                x_truncate: Some(wr_p_cache_truncate),
                x_destroy: Some(wr_p_cache_destroy),
                x_shrink: None,
            };
        let mut memmethods: sqlite3_mem_methods =
            sqlite3_mem_methods {
                x_malloc: Some(wr_mem_malloc),
                x_free: Some(wr_mem_free),
                x_realloc: Some(wr_mem_realloc),
                x_size: Some(wr_mem_size),
                x_roundup: Some(wr_mem_roundup),
                x_init: Some(wr_mem_init),
                x_shutdown: Some(wr_mem_shutdown),
                p_app_data: core::ptr::null_mut(),
            };
        unsafe {
            memset(&raw mut wrapped as *mut (), 0,
                core::mem::size_of::<Wrapped>() as u64)
        };
        unsafe { sqlite3_shutdown() };
        unsafe {
            sqlite3_config(11,
                &raw mut wrapped.mutex as *mut sqlite3_mutex_methods)
        };
        unsafe {
            sqlite3_config(5,
                &raw mut wrapped.mem as *mut sqlite3_mem_methods)
        };
        unsafe {
            sqlite3_config(19,
                &raw mut wrapped.pcache as *mut sqlite3_pcache_methods2)
        };
        unsafe {
            sqlite3_config(10,
                &raw mut mutexmethods as *mut sqlite3_mutex_methods)
        };
        unsafe {
            sqlite3_config(4, &raw mut memmethods as *mut sqlite3_mem_methods)
        };
        unsafe {
            sqlite3_config(18,
                &raw mut pcachemethods as *mut sqlite3_pcache_methods2)
        };
    }
}
extern "C" fn init_wrapper_install(client_data_1: ClientData,
    ...
    unsafe {
        let mut i: i32 = 0;
        install_init_wrappers();
        {
            i = 1;
            '__b0: loop {
                if !(i < objc) { break '__b0; }
                '__c0: loop {
                    let z: *mut i8 =
                        unsafe {
                            Tcl_GetString(unsafe { *objv.offset(i as isize) })
                        };
                    if unsafe {
                                strcmp(z as *const i8,
                                    c"mem".as_ptr() as *mut i8 as *const i8)
                            } == 0 {
                        wrapped.mem_fail = 1;
                    } else if unsafe {
                                strcmp(z as *const i8,
                        ...
                        wrapped.mutex_fail = 1;
                                ...
                                strcmp(z as *const i8,
                                    c"pcache".as_ptr() as *mut i8 as *const i8)
                            } == 0 {
                        wrapped.pcache_fail = 1;
                    } else {
                        unsafe {
                            Tcl_AppendResult(interp,
                                c"Unknown argument: \"".as_ptr() as *mut i8, z,
    ...
    unsafe {
        ...
        if wrapped.mutex_init != 0 {
            unsafe {
                Tcl_ListObjAppendElement(interp, p_ret,
                    unsafe {
                        Tcl_NewStringObj(c"mutex".as_ptr() as *mut i8 as *const i8,
                            -1)
                    })
            };
        }
        if wrapped.mem_init != 0 {
            unsafe {
                Tcl_ListObjAppendElement(interp, p_ret,
                    unsafe {
                        Tcl_NewStringObj(c"mem".as_ptr() as *mut i8 as *const i8,
                            -1)
                    })
            };
        }
        if wrapped.pcache_init != 0 {
            unsafe {
                Tcl_ListObjAppendElement(interp, p_ret,
                    unsafe {
                        Tcl_NewStringObj(c"pcache".as_ptr() as *mut i8 as *const i8,
                            -1)
                    })
            };
        }
