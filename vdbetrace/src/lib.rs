...
extern "C" fn find_next_host_parameter(mut z_sql_1: *const i8,
    ...
    let mut n_total: i64 = 0 as i64;
    let mut n: i64 = 0 as i64;
    *pn_token_1 = 0 as i64;
    while unsafe { *z_sql_1.offset(0 as isize) } != 0 {
        n =
            unsafe {
                sqlite3_get_token(z_sql_1 as *mut u8 as *const u8,
                    &mut token_type)
            };
        { let _ = 0; };
        if token_type == 157 { *pn_token_1 = n; break; }
        n_total += n;
        {
            let __n = n;
            let __p = &mut z_sql_1;
...
pub extern "C" fn sqlite3_vdbe_expand_sql(p: *mut Vdbe,
    ...
    unsafe {
        ...
        if unsafe { (*db).n_vdbe_exec } > 1 {
            while unsafe { *z_raw_sql_1 } != 0 {
                let z_start: *const i8 = z_raw_sql_1;
                while unsafe {
                                    *{
                                            let __p = &mut z_raw_sql_1;
                                            let __t = *__p;
                                            *__p = unsafe { (*__p).offset(1) };
                                            __t
                                        }
                                } as i32 != '\n' as i32 && unsafe { *z_raw_sql_1 } != 0 {}
                unsafe {
                ...
                { let _ = 0; };
                unsafe {
                    sqlite3_str_append(&raw mut out as *mut Sqlite3Str, z_start,
                        unsafe { z_raw_sql_1.offset_from(z_start) } as i64 as i32)
                };
            }
        } else if unsafe { (*p).n_var } as i32 == 0 {
            unsafe {
                sqlite3_str_append(&raw mut out as *mut Sqlite3Str,
                    z_raw_sql_1, unsafe { sqlite3_strlen30(z_raw_sql_1) })
            };
        } else {
            while unsafe { *z_raw_sql_1.offset(0 as isize) } != 0 {
                n = find_next_host_parameter(z_raw_sql_1, &mut n_token);
                { let _ = 0; };
                unsafe {
                    sqlite3_str_append(&raw mut out as *mut Sqlite3Str,
                        z_raw_sql_1, n as i32)
                };
                {
                    let __n = n;
                    let __p = &mut z_raw_sql_1;
                ...
                { let _ = 0; };
                if n_token == 0 as i64 { break; }
                if unsafe { *z_raw_sql_1.offset(0 as isize) } as i32 ==
                        '?' as i32 {
                    if n_token > 1 as i64 {
                        { let _ = 0; };
                        unsafe {
                            sqlite3_get_int32(unsafe {
                                    &*z_raw_sql_1.offset(1 as isize)
                                }, &mut idx)
                        };
                    } else { idx = next_index; }
                } else {
                    ...
                    idx =
                        unsafe {
                            sqlite3_vdbe_parameter_index(p, z_raw_sql_1, n_token as i32)
                        };
                    { let _ = 0; };
                }
                {
                ...
                next_index =
                    if idx + 1 > next_index { idx + 1 } else { next_index };
                { let _ = 0; };
                p_var =
                    unsafe { unsafe { (*p).a_var.offset((idx - 1) as isize) } };
                if unsafe { (*p_var).flags } as i32 & 1 != 0 {
                    unsafe {
                        sqlite3_str_append(&raw mut out as *mut Sqlite3Str,
                            c"NULL".as_ptr() as *mut i8 as *const i8, 4)
                    };
                } else if unsafe { (*p_var).flags } as i32 & (4 | 32) != 0 {
                    unsafe {
                        sqlite3_str_appendf(&raw mut out as *mut Sqlite3Str,
                            c"%lld".as_ptr() as *mut i8 as *const i8,
                            unsafe { (*p_var).u.i })
                    };
                } else if unsafe { (*p_var).flags } as i32 & 8 != 0 {
                    unsafe {
                        sqlite3_str_appendf(&raw mut out as *mut Sqlite3Str,
                            c"%!.15g".as_ptr() as *mut i8 as *const i8,
                            unsafe { (*p_var).u.r })
                    };
                    ...
                    if enc as i32 != 1 {
                        unsafe {
                            memset(&raw mut utf8 as *mut (), 0,
                                core::mem::size_of::<Mem>() as u64)
                        };
                        utf8.db = db;
                        unsafe {
                            sqlite3_vdbe_mem_set_str(&mut utf8,
                                unsafe { (*p_var).z } as *const i8,
                                unsafe { (*p_var).n } as i64, enc, None)
                        };
                        if 7 ==
                                unsafe { sqlite3_vdbe_change_encoding(&mut utf8, 1) } {
                            out.acc_error = 7 as u8;
                            out.n_alloc = 0 as u32;
                        }
                        p_var = &mut utf8;
                    }
                    n_out = unsafe { (*p_var).n };
                    unsafe {
                        sqlite3_str_appendf(&raw mut out as *mut Sqlite3Str,
                            c"\'%.*q\'".as_ptr() as *mut i8 as *const i8, n_out,
                            unsafe { (*p_var).z })
                    };
                    if enc as i32 != 1 {
                        unsafe { sqlite3_vdbe_mem_release(&mut utf8) };
                    }
                } else if unsafe { (*p_var).flags } as i32 & 1024 != 0 {
                    unsafe {
                        sqlite3_str_appendf(&raw mut out as *mut Sqlite3Str,
                            c"zeroblob(%d)".as_ptr() as *mut i8 as *const i8,
                            unsafe { (*p_var).u.n_zero })
                    };
                } else {
                    ...
                    n_out_1 = unsafe { (*p_var).n };
                    {
                        i = 0;
                        '__b4: loop {
                            if !(i < n_out_1) { break '__b4; }
                            '__c4: loop {
                                unsafe {
                                    sqlite3_str_appendf(&raw mut out as *mut Sqlite3Str,
                                        c"%02x".as_ptr() as *mut i8 as *const i8,
                                        unsafe { *unsafe { (*p_var).z.offset(i as isize) } } as i32
                                            & 255)
                                };
                                break '__c4;
                            }
        ...
        if out.acc_error != 0 {
            unsafe { sqlite3_str_reset(&raw mut out as *mut Sqlite3Str) };
        }
        return unsafe { sqlite3_str_accum_finish(&mut out) };
    }
}
