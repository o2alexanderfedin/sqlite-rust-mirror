...
extern "C" fn ptr_to_text(p: *mut ()) -> *mut i8 {
    unsafe {
        unsafe {
            sqlite3_snprintf((core::mem::size_of::<[i8; 100]>() as u64 -
                        1 as u64) as i32, &raw mut buf[0 as usize] as *mut i8,
                c"%p".as_ptr() as *mut i8 as *const i8, p)
        };
        return &raw mut buf[0 as usize] as *mut i8;
    }
}
extern "C" fn blob_handle_from_obj(interp: *mut Tcl_Interp,
    p_obj_1: *mut Tcl_Obj, pp_blob_1: &mut *mut sqlite3_blob) -> i32 {
    let mut z: *const i8 = core::ptr::null();
    let mut n: i32 = 0;
    z = unsafe { Tcl_GetStringFromObj(p_obj_1, &mut n) };
    if n == 0 {
        *pp_blob_1 = core::ptr::null_mut();
    } else if n > 9 &&
            0 ==
                unsafe {
                    memcmp(c"incrblob_".as_ptr() as *mut i8 as *const (),
                        z as *const (), 9 as u64)
                } {
        let mut not_used: i32 = 0;
        let mut channel: Tcl_Channel = core::ptr::null_mut();
        let mut instance_data: ClientData = core::ptr::null_mut();
        channel =
            unsafe { Tcl_GetChannel(interp, z as *const i8, &mut not_used) };
        if (channel).is_null() as i32 != 0 { return 1; }
        unsafe { Tcl_Flush(channel) };
        unsafe { Tcl_Seek(channel, 0 as Tcl_WideInt, 0) };
        instance_data = unsafe { Tcl_GetChannelInstanceData(channel) };
        *pp_blob_1 = unsafe { *(instance_data as *mut *mut sqlite3_blob) };
    } else {
        *pp_blob_1 =
            unsafe { sqlite3_test_text_to_ptr(z as *const i8) } as
                *mut sqlite3_blob;
    }
    return 0;
}
extern "C" fn blob_string_from_obj(p_obj_1: *mut Tcl_Obj) -> *mut i8 {
    let mut n: i32 = 0;
...
extern "C" fn test_blob_open(client_data_1: ClientData,
    ...
    let mut i_rowid: Tcl_WideInt = 0 as Tcl_WideInt;
    let mut flags: i32 = 0;
    let mut z_varname: *const i8 = core::ptr::null();
    let mut n_varname: i32 = 0;
    let mut p_blob: *mut sqlite3_blob = &raw mut flags as *mut sqlite3_blob;
    let mut rc: i32 = 0;
    if objc != 8 {
        let z_usage: *const i8 =
            c"DB DATABASE TABLE COLUMN ROWID FLAGS VARNAME".as_ptr() as
                    *mut i8 as *const i8;
        unsafe { Tcl_WrongNumArgs(interp, 1, objv, z_usage) };
        return 1;
    }
    if unsafe {
    ...
    z_db =
        unsafe { Tcl_GetString(unsafe { *objv.offset(2 as isize) }) } as
            *const i8;
    z_table =
        blob_string_from_obj(unsafe { *objv.offset(3 as isize) }) as
            *const i8;
    z_column =
        unsafe { Tcl_GetString(unsafe { *objv.offset(4 as isize) }) } as
            *const i8;
    if unsafe {
                Tcl_GetWideIntFromObj(interp,
                    unsafe { *objv.offset(5 as isize) }, &mut i_rowid)
            } != 0 {
        return 1;
    }
    if unsafe {
                Tcl_GetIntFromObj(interp, unsafe { *objv.offset(6 as isize) },
                    &mut flags)
            } != 0 {
        return 1;
    }
    z_varname =
        unsafe {
                Tcl_GetStringFromObj(unsafe { *objv.offset(7 as isize) },
                    &mut n_varname)
            } as *const i8;
    if n_varname > 0 {
        rc =
            unsafe {
                sqlite3_blob_open(db, z_db, z_table, z_column, i_rowid, flags,
                    &mut p_blob)
            };
        unsafe {
            Tcl_SetVar(interp, z_varname,
                ptr_to_text(p_blob as *mut ()) as *const i8, 0)
        };
    } else {
        rc =
            unsafe {
    ...
    if objc != 2 {
        unsafe {
            Tcl_WrongNumArgs(interp, 1, objv,
                c"HANDLE".as_ptr() as *mut i8 as *const i8)
        };
        return 1;
    }
    if blob_handle_from_obj(interp, unsafe { *objv.offset(1 as isize) },
                &mut p_blob) != 0 {
        return 1;
    }
    rc = unsafe { sqlite3_blob_close(p_blob) };
    if rc != 0 {
        unsafe {
            Tcl_SetResult(interp, unsafe { sqlite3_err_name(rc) } as *mut i8,
                unsafe {
...
extern "C" fn test_blob_bytes(client_data_1: ClientData,
    ...
    n_byte = unsafe { sqlite3_blob_bytes(p_blob) };
...
extern "C" fn test_blob_read(client_data_1: ClientData,
    ...
    if objc != 4 {
        unsafe {
            Tcl_WrongNumArgs(interp, 1, objv,
                c"CHANNEL OFFSET N".as_ptr() as *mut i8 as *const i8)
        };
        return 1;
    }
    if blob_handle_from_obj(interp, unsafe { *objv.offset(1 as isize) },
    ...
    if 0 !=
                unsafe {
                    Tcl_GetIntFromObj(interp,
                        unsafe { *objv.offset(2 as isize) }, &mut i_offset)
                } ||
            0 !=
                unsafe {
                    Tcl_GetIntFromObj(interp,
                        unsafe { *objv.offset(3 as isize) }, &mut n_byte)
                } {
        return 1;
    }
    if n_byte > 0 {
        z_buf = unsafe { Tcl_AttemptAlloc(n_byte as u32) } as *mut u8;
        if z_buf == core::ptr::null_mut() {
            unsafe {
                Tcl_AppendResult(interp,
                    c"out of memory in /Volumes/ProjectsSSD/Projects/transpilers/cpp-to-rust/Testing/corpuses/sqlite/src/test_blob.c".as_ptr()
                        as *mut i8, 0 as *mut ())
            };
            return 1;
        }
    ...
    rc =
        unsafe {
            sqlite3_blob_read(p_blob, z_buf as *mut (), n_byte, i_offset)
        };
    if rc == 0 {
        unsafe {
            Tcl_SetObjResult(interp,
                unsafe { Tcl_NewByteArrayObj(z_buf as *const u8, n_byte) })
        };
    } else {
        unsafe {
            Tcl_SetResult(interp, unsafe { sqlite3_err_name(rc) } as *mut i8,
...
extern "C" fn test_blob_write(client_data_1: ClientData,
    ...
    let mut i_offset: i32 = 0;
    let mut rc: i32 = 0;
    let mut z_buf: *const u8 = core::ptr::null();
    let mut n_buf: i32 = 0;
    let mut n: i32 = 0;
    if objc != 4 && objc != 5 {
        unsafe {
            Tcl_WrongNumArgs(interp, 1, objv,
                c"HANDLE OFFSET DATA ?NDATA?".as_ptr() as *mut i8 as
                    *const i8)
        };
        return 1;
    }
    ...
    z_buf =
        unsafe {
            Tcl_GetByteArrayFromObj(unsafe { *objv.offset(3 as isize) },
                &mut n_buf)
        };
    n = (n_buf & 2147483647) as i32;
    if objc == 5 &&
            unsafe {
                    Tcl_GetIntFromObj(interp,
                        unsafe { *objv.offset(4 as isize) }, &mut n)
                } != 0 {
        return 1;
    }
    rc =
        unsafe {
            sqlite3_blob_write(p_blob, z_buf as *const (), n, i_offset)
        };
    if rc != 0 {
        unsafe {
            Tcl_SetResult(interp, unsafe { sqlite3_err_name(rc) } as *mut i8,
...
extern "C" {
    ...
    -> i32;
    fn sqlite3_test_text_to_ptr(z: *const i8)
    -> *mut ();
    fn Tcl_GetStringFromObj(objPtr: *mut Tcl_Obj, lengthPtr: *mut i32)
    -> *mut i8;
    fn memcmp(__s1: *const (), __s2: *const (), __n: u64)
    -> i32;
    fn Tcl_GetChannel(interp: *mut Tcl_Interp, chanName: *const i8,
    modePtr: *mut i32)
    -> Tcl_Channel;
    fn Tcl_Flush(chan: Tcl_Channel)
    -> i32;
    fn Tcl_Seek(chan: Tcl_Channel, offset: Tcl_WideInt, mode: i32)
    -> Tcl_WideInt;
    fn Tcl_GetChannelInstanceData(chan: Tcl_Channel)
    -> ClientData;
