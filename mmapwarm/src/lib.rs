...
pub extern "C" fn sqlite3_mmap_warm(db: *mut Sqlite3, z_db_1: *const i8)
    ...
    if rc == 0 {
        ...
        if z_sql == core::ptr::null_mut() {
            ...
            let mut p_pgsz: *mut Sqlite3Stmt = core::ptr::null_mut();
            rc =
                unsafe {
    ...
    if rc == 0 {
        ...
        if rc == 0 &&
                unsafe { (*unsafe { (*p_fd).p_methods }).i_version } as i32 >=
                    3 {
            let mut i_pg: Sqlite3Int64 = 1 as Sqlite3Int64;
            let p: *const Sqlite3IoMethods = unsafe { (*p_fd).p_methods };
            loop {
                ...
                rc =
                    unsafe {
                        (unsafe {
                                (*p).x_fetch.unwrap()
                            })(p_fd, pgsz as Sqlite3Int64 * i_pg, pgsz,
                ...
                rc =
                    unsafe {
                        (unsafe {
                                (*p).x_unfetch.unwrap()
                            })(p_fd, pgsz as Sqlite3Int64 * i_pg, p_map as *mut ())
                    };
                if rc != 0 { break; }
            ...
            unsafe {
                sqlite3_log(0,
                    c"sqlite3_mmap_warm_cache: Warmed up %d pages of %s".as_ptr()
                            as *mut i8 as *const i8,
                    if i_pg == 1 as i64 { 0 as Sqlite3Int64 } else { i_pg },
