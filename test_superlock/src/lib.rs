...
struct SuperlockBusy {
    x_busy: Option<unsafe extern "C" fn(*mut (), i32) -> i32>,
    p_busy_arg: *mut (),
    n_busy: i32,
}
#[repr(C)]
...
extern "C" fn superlock_busy_handler(p_ctx_1: *mut (), unused_1: i32) -> i32 {
    ...
    return unsafe {
            (unsafe {
                    (*p_busy).x_busy.unwrap()
                })(unsafe { (*p_busy).p_busy_arg },
                {
                    let __p = unsafe { &mut (*p_busy).n_busy };
                    let __t = *__p;
                    *__p += 1;
                    __t
                })
...
extern "C" fn superlock_is_wal(p_lock_1: &mut Superlock) -> i32 {
    ...
    if 100 == unsafe { sqlite3_step(p_stmt) } {
        ...
        if !(z_mode).is_null() && unsafe { strlen(z_mode) } == 3 as u64 &&
                unsafe {
                        sqlite3_strnicmp(c"wal".as_ptr() as *mut i8 as *const i8,
                            z_mode, 3)
                    } == 0 {
            (*p_lock_1).b_wal = 1;
        }
...
extern "C" fn superlock_shm_lock(fd: *mut Sqlite3File, idx: i32,
    ...
    let mut rc: i32 = 0;
    let x_shm_lock:
            Option<unsafe extern "C" fn(*mut Sqlite3File, i32, i32, i32)
                -> i32> =
        Some(unsafe { (*unsafe { (*fd).p_methods }).x_shm_lock });
    '__b0: loop {
        '__c0: loop {
            rc = unsafe { x_shm_lock.unwrap()(fd, idx, n_byte_1, 2 | 8) };
            break '__c0;
        }
        if !(rc == 5 && superlock_busy_handler(p_busy_1 as *mut (), 0) != 0) {
            break '__b0;
        }
    }
    return rc;
        ...
        rc =
            unsafe {
                sqlite3_file_control(unsafe { (*p).db },
                    c"main".as_ptr() as *mut i8 as *const i8, 7,
                    &raw mut fd as *mut ())
            };
        if rc == 0 {
            unsafe {
                (unsafe {
                        (*unsafe { (*fd).p_methods }).x_shm_lock
                    })(fd, 2, 1, flags)
            };
            unsafe {
                (unsafe {
...
pub extern "C" fn sqlite3demo_superlock(z_path_1: *const i8,
    ...
    p_busy_arg_1: *mut (), pp_lock_1: &mut *mut ()) -> i32 {
    let mut busy: SuperlockBusy =
        SuperlockBusy {
            x_busy: None,
    ...
    if rc == 0 {
        ...
        unsafe {
            sqlite3_busy_handler(unsafe { (*p_lock).db },
                Some(superlock_busy_handler), &raw mut busy as *mut ())
        };
        rc =
            unsafe {
                sqlite3_exec(unsafe { (*p_lock).db },
                    c"BEGIN EXCLUSIVE".as_ptr() as *mut i8 as *const i8, None,
                    core::ptr::null_mut(), core::ptr::null_mut())
            };
    }
    if rc == 0 {
        if 0 == { rc = superlock_is_wal(unsafe { &mut *p_lock }); rc } &&
                unsafe { (*p_lock).b_wal } != 0 {
            rc =
                unsafe {
                    sqlite3_exec(unsafe { (*p_lock).db },
            ...
            if rc == 0 {
                rc = superlock_wal_lock(unsafe { (*p_lock).db }, &mut busy);
            }
        }
    }
    if rc != 0 {
        sqlite3demo_superunlock(p_lock as *mut ());
        *pp_lock_1 = core::ptr::null_mut();
