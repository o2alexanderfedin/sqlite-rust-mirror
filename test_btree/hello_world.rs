...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_btree_shared_cache_report(client_data_1: *mut (),
    interp: *mut Tcl_Interp, objc: i32, objv: *const *mut Tcl_Obj) -> i32 {
    unsafe {
        let mut p_bt: *const BtShared = core::ptr::null();
        let p_ret: *mut Tcl_Obj = unsafe { Tcl_NewObj() };
        {
            p_bt = sqlite3_shared_cache_list;
            '__b0: loop {
                if !(!(p_bt).is_null()) { break '__b0; }
                '__c0: loop {
                    let z_file: *const i8 =
                        unsafe {
                    ...
                    unsafe {
                        Tcl_ListObjAppendElement(interp, p_ret,
                            unsafe { Tcl_NewStringObj(z_file, -1) })
                    };
                    unsafe {
                        Tcl_ListObjAppendElement(interp, p_ret,
                            unsafe { Tcl_NewIntObj(unsafe { (*p_bt).n_ref }) })
                    };
                    break '__c0;
                }
                p_bt = unsafe { (*p_bt).p_next };
            }
        }
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_btree_cursor_list(p: *const Btree) -> () {}
extern "C" {
    ...
    -> ();
    static mut sqlite3_shared_cache_list: *mut BtShared;
}
#[repr(C)]
#[derive(Copy, Clone)]
struct CCurHint {
