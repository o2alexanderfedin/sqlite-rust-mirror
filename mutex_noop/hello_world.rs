...
extern "C" fn noop_mutex_alloc(id: i32) -> *mut sqlite3_mutex {
    { let _ = id; };
    return 8 as *mut sqlite3_mutex;
}
extern "C" fn noop_mutex_free(p: *mut sqlite3_mutex) -> () {
    { let _ = p; };
    return;
}
extern "C" fn noop_mutex_enter(p: *mut sqlite3_mutex) -> () {
    { let _ = p; };
    return;
}
extern "C" fn noop_mutex_try(p: *mut sqlite3_mutex) -> i32 {
    { let _ = p; };
    return 0;
}
extern "C" fn noop_mutex_leave(p: *mut sqlite3_mutex) -> () {
    { let _ = p; };
    return;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_noop_mutex() -> *const sqlite3_mutex_methods {
    return &s_mutex;
}
static s_mutex: sqlite3_mutex_methods =
    sqlite3_mutex_methods {
        x_mutex_init: Some(noop_mutex_init),
        x_mutex_end: Some(noop_mutex_end),
        x_mutex_alloc: Some(noop_mutex_alloc),
        x_mutex_free: Some(noop_mutex_free),
        x_mutex_enter: Some(noop_mutex_enter),
        x_mutex_try: Some(noop_mutex_try),
        x_mutex_leave: Some(noop_mutex_leave),
        x_mutex_held: None,
