...
extern "C" fn wholenumber_connect(db: *mut sqlite3, p_aux_1: *mut (),
    ...
    unsafe {
        sqlite3_declare_vtab(db,
            c"CREATE TABLE x(value)".as_ptr() as *mut i8 as *const i8)
    };
...
extern "C" fn wholenumber_disconnect(p_vtab_1: *mut sqlite3_vtab) -> i32 {
    unsafe { sqlite3_free(p_vtab_1 as *mut ()) };
    return 0;
}
extern "C" fn wholenumber_open(p: *mut sqlite3_vtab,
    pp_cursor_1: *mut *mut sqlite3_vtab_cursor) -> i32 {
    let mut p_cur: *mut wholenumber_cursor = core::ptr::null_mut();
    p_cur =
        unsafe {
                sqlite3_malloc64(core::mem::size_of::<wholenumber_cursor>() as
                        sqlite3_uint64)
            } as *mut wholenumber_cursor;
    if p_cur == core::ptr::null_mut() { return 7; }
    unsafe {
        memset(p_cur as *mut (), 0,
            core::mem::size_of::<wholenumber_cursor>() as u64)
    };
    unsafe { *pp_cursor_1 = unsafe { &mut (*p_cur).base } };
    return 0;
}
extern "C" fn wholenumber_close(cur: *mut sqlite3_vtab_cursor) -> i32 {
    unsafe { sqlite3_free(cur as *mut ()) };
    return 0;
}
extern "C" fn wholenumber_next(cur: *mut sqlite3_vtab_cursor) -> i32 {
    let p_cur: *mut wholenumber_cursor = cur as *mut wholenumber_cursor;
    {
        let __p = unsafe { &mut (*p_cur).i_value };
        let __t = *__p;
        *__p += 1;
        __t
    };
...
extern "C" fn wholenumber_column(cur: *mut sqlite3_vtab_cursor,
    ctx: *mut sqlite3_context, i: i32) -> i32 {
    let p_cur: *const wholenumber_cursor =
        cur as *mut wholenumber_cursor as *const wholenumber_cursor;
    unsafe { sqlite3_result_int64(ctx, unsafe { (*p_cur).i_value }) };
    return 0;
}
extern "C" fn wholenumber_rowid(cur: *mut sqlite3_vtab_cursor,
...
extern "C" fn wholenumber_eof(cur: *mut sqlite3_vtab_cursor) -> i32 {
    let p_cur: *const wholenumber_cursor =
        cur as *mut wholenumber_cursor as *const wholenumber_cursor;
    return (unsafe { (*p_cur).i_value } > unsafe { (*p_cur).mx_value } ||
                unsafe { (*p_cur).i_value } == 0 as i64) as i32;
}
extern "C" fn wholenumber_filter(p_vtab_cursor_1: *mut sqlite3_vtab_cursor,
    idx_num_1: i32, idx_str_1: *const i8, argc: i32,
    argv: *mut *mut sqlite3_value) -> i32 {
    let p_cur: *mut wholenumber_cursor =
        p_vtab_cursor_1 as *mut wholenumber_cursor;
    let mut v: sqlite3_int64 = 0 as sqlite3_int64;
    let mut i: i32 = 0;
    unsafe { (*p_cur).i_value = 1 as sqlite3_int64 };
    unsafe { (*p_cur).mx_value = 4294967295u32 as sqlite3_int64 };
    if idx_num_1 & 3 != 0 {
        v =
            unsafe {
                sqlite3_value_int64(unsafe { *argv.offset(0 as isize) })
            };
        if v <= unsafe { (*p_cur).mx_value } && idx_num_1 & 1 != 0 {
            { let __p = &mut v; let __t = *__p; *__p += 1; __t };
        }
        if v > unsafe { (*p_cur).i_value } &&
                v <= unsafe { (*p_cur).mx_value } {
            unsafe { (*p_cur).i_value = v };
        }
    ...
    if idx_num_1 & 12 != 0 {
        ...
        if v > 0 as i64 && idx_num_1 >> 2 & 1 != 0 {
            { let __p = &mut v; let __t = *__p; *__p -= 1; __t };
        }
        if v >= unsafe { (*p_cur).i_value } &&
                v < unsafe { (*p_cur).mx_value } {
            unsafe { (*p_cur).mx_value = v };
        }
    }
    return 0;
}
extern "C" fn wholenumber_best_index(tab: *mut sqlite3_vtab,
    p_idx_info_1: *mut sqlite3_index_info) -> i32 {
    let mut i: i32 = 0;
    let mut idx_num: i32 = 0;
    let mut argv_idx: i32 = 1;
    let mut lt_idx: i32 = -1;
    let mut gt_idx: i32 = -1;
            ...
            *const sqlite3_index_constraint;
    {
        ...
        '__b0: loop {
            ...
            '__c0: loop {
                ...
                if idx_num & 3 == 0 &&
                        unsafe { (*p_constraint).op } as i32 == 4 {
                    idx_num |= 1;
                    lt_idx = i;
                }
                if idx_num & 3 == 0 &&
                        unsafe { (*p_constraint).op } as i32 == 32 {
                    idx_num |= 2;
                    lt_idx = i;
                }
                if idx_num & 12 == 0 &&
    ...
    if lt_idx >= 0 {
        unsafe {
            (*unsafe {
                            (*p_idx_info_1).a_constraint_usage.offset(lt_idx as isize)
                        }).argv_index =
                { let __p = &mut argv_idx; let __t = *__p; *__p += 1; __t }
        };
        unsafe {
            (*unsafe {
                            (*p_idx_info_1).a_constraint_usage.offset(lt_idx as isize)
    ...
    if gt_idx >= 0 {
        unsafe {
            (*unsafe {
                            (*p_idx_info_1).a_constraint_usage.offset(gt_idx as isize)
                        }).argv_index = argv_idx
        };
        unsafe {
            (*unsafe {
    ...
    if idx_num & 12 == 0 {
        unsafe { (*p_idx_info_1).estimated_cost = 1e99 };
    } else if idx_num & 3 == 0 {
        unsafe { (*p_idx_info_1).estimated_cost = 5 as f64 };
    } else { unsafe { (*p_idx_info_1).estimated_cost = 1 as f64 }; }
    return 0;
}
static mut wholenumber_module: sqlite3_module =
    sqlite3_module {
        i_version: 0,
        x_create: Some(wholenumber_connect),
        x_connect: Some(wholenumber_connect),
        x_best_index: Some(wholenumber_best_index),
        x_disconnect: Some(wholenumber_disconnect),
        x_destroy: Some(wholenumber_disconnect),
        x_open: Some(wholenumber_open),
        x_close: Some(wholenumber_close),
        x_filter: Some(wholenumber_filter),
        x_next: Some(wholenumber_next),
        x_eof: Some(wholenumber_eof),
        x_column: Some(wholenumber_column),
        x_rowid: Some(wholenumber_rowid),
        x_update: None,
        x_begin: None,
        x_sync: None,
        x_commit: None,
        ...
        rc =
            unsafe {
                sqlite3_create_module(db,
                    c"wholenumber".as_ptr() as *mut i8 as *const i8,
                    &raw mut wholenumber_module as *const sqlite3_module,
                    core::ptr::null_mut())
            };
        return rc;
    }
