...
pub extern "C" fn sqlite3_p_cache_buffer_setup(mut p_buf: *mut (),
    ...
    unsafe {
        if pcache1_g.is_init != 0 {
            let mut p: *mut PgFreeslot = core::ptr::null_mut();
            if p_buf == core::ptr::null_mut() { sz = { n = 0; n }; }
            if n == 0 { sz = 0; }
            sz = sz & !7;
            pcache1_g.sz_slot = sz;
            pcache1_g.n_slot =
                { pcache1_g.n_free_slot = n; pcache1_g.n_free_slot };
            pcache1_g.n_reserve = if n > 90 { 10 } else { n / 10 + 1 };
            pcache1_g.p_start = p_buf;
            pcache1_g.p_free = core::ptr::null_mut();
            unsafe {
                std::sync::atomic::AtomicI32::from_ptr(&raw mut pcache1_g.b_under_pressure
                            as
                            *mut i32).store(0 as i32,
                    std::sync::atomic::Ordering::Relaxed)
            };
            while { let __p = &mut n; let __t = *__p; *__p -= 1; __t } != 0 {
                p = p_buf as *mut PgFreeslot;
                unsafe { (*p).p_next = pcache1_g.p_free };
                pcache1_g.p_free = p;
                p_buf =
                    unsafe { &raw mut *(p_buf as *mut i8).offset(sz as isize) }
                        as *mut ();
            }
            pcache1_g.p_end = p_buf;
        }
    }
}
#[repr(C)]
...
extern "C" fn pcache1_init(not_used_1: *mut ()) -> i32 {
    unsafe {
        ...
        if pcache1_g.separate_cache != 0 && sqlite3Config.n_page != 0 &&
                sqlite3Config.p_page == core::ptr::null_mut() {
            pcache1_g.n_init_page = sqlite3Config.n_page;
        } else { pcache1_g.n_init_page = 0; }
        pcache1_g.grp.mx_pinned = 10 as u32;
        pcache1_g.is_init = 1;
        return 0;
    }
}
...
extern "C" fn pcache1_resize_hash(p: &mut PCache1) -> () {
    ...
    { let _ = 0; };
    n_new = 2 as u64 * (*p).n_hash as u64;
    if n_new < 256 as u64 { n_new = 256 as u64; }
    { let _ = 0; };
    if (*p).n_hash != 0 { unsafe { sqlite3_begin_benign_malloc() }; }
    ap_new =
        unsafe {
                sqlite3_malloc_zero(core::mem::size_of::<*mut PgHdr1>() as u64
                        * n_new)
            } as *mut *mut PgHdr1;
    if (*p).n_hash != 0 { unsafe { sqlite3_end_benign_malloc() }; }
    { let _ = 0; };
    if !(ap_new).is_null() {
        {
            i = 0 as u32;
            '__b1: loop {
                if !(i < (*p).n_hash) { break '__b1; }
                '__c1: loop {
                    let mut p_page: *mut PgHdr1 = core::ptr::null_mut();
                    let mut p_next: *mut PgHdr1 =
                        unsafe { *(*p).ap_hash.add(i as usize) };
                    while { p_page = p_next; p_page } != core::ptr::null_mut() {
                        let h: u32 =
                            (unsafe { (*p_page).i_key } as u64 % n_new) as u32;
                        p_next = unsafe { (*p_page).p_next };
                        unsafe {
                            (*p_page).p_next = unsafe { *ap_new.add(h as usize) }
                        };
                        unsafe { *ap_new.add(h as usize) = p_page };
                    }
                    break '__c1;
                }
        ...
        unsafe { sqlite3_free((*p).ap_hash as *mut ()) };
        (*p).ap_hash = ap_new;
        (*p).n_hash = n_new as u32;
    }
}
extern "C" fn pcache1_pin_page(p_page_1: *mut PgHdr1) -> *mut PgHdr1 {
    ...
    { let _ = 0; };
    unsafe {
        (*unsafe { (*p_page_1).p_lru_prev }).p_lru_next =
            unsafe { (*p_page_1).p_lru_next }
    };
    unsafe {
        (*unsafe { (*p_page_1).p_lru_next }).p_lru_prev =
    ...
    {
        let __p =
            unsafe { &mut (*unsafe { (*p_page_1).p_cache }).n_recyclable };
        let __t = *__p;
        *__p -= 1;
        __t
    };
...
extern "C" fn pcache1_free(p: *mut ()) -> () {
    unsafe {
        ...
        if p as uptr >= pcache1_g.p_start as uptr &&
                (p as uptr) < pcache1_g.p_end as uptr {
            let mut p_slot: *mut PgFreeslot = core::ptr::null_mut();
            unsafe { sqlite3_mutex_enter(pcache1_g.mutex) };
            unsafe { sqlite3_status_down(1, 1) };
            p_slot = p as *mut PgFreeslot;
            unsafe { (*p_slot).p_next = pcache1_g.p_free };
            pcache1_g.p_free = p_slot;
            {
                let __p = &mut pcache1_g.n_free_slot;
                let __t = *__p;
                *__p += 1;
                __t
            };
            unsafe {
                std::sync::atomic::AtomicI32::from_ptr(&raw mut pcache1_g.b_under_pressure
                            as
                            *mut i32).store((pcache1_g.n_free_slot <
                            pcache1_g.n_reserve) as i32,
                    std::sync::atomic::Ordering::Relaxed)
            };
            { let _ = 0; };
            ...
            { let _ = 0; };
            {
                let mut n_freed: i32 = 0;
                n_freed = unsafe { sqlite3_malloc_size(p as *const ()) };
                unsafe { sqlite3_mutex_enter(pcache1_g.mutex) };
                unsafe { sqlite3_status_down(2, n_freed) };
                unsafe { sqlite3_mutex_leave(pcache1_g.mutex) };
            }
...
extern "C" fn pcache1_free_page(p: *mut PgHdr1) -> () {
    ...
    { let _ = 0; };
    p_cache = unsafe { (*p).p_cache };
    { let _ = 0; };
    if unsafe { (*p).is_bulk_local } != 0 {
        unsafe { (*p).p_next = unsafe { (*p_cache).p_free } };
        unsafe { (*p_cache).p_free = p };
    } else { pcache1_free(unsafe { (*p).page.p_buf }); }
    {
        let __p = unsafe { &mut *unsafe { (*p_cache).pn_purgeable } };
        let __t = *__p;
        *__p -= 1;
        __t
    };
...
extern "C" fn pcache1_truncate_unsafe(p_cache_1: &mut PCache1, i_limit_1: u32)
    ...
    } else { h = (*p_cache_1).n_hash / 2 as u32; i_stop = h - 1 as u32; }
    {
        '__b3: loop {
            '__c3: loop {
                let mut pp: *mut *mut PgHdr1 = core::ptr::null_mut();
                let mut p_page: *mut PgHdr1 = core::ptr::null_mut();
                { let _ = 0; };
                pp = unsafe { (*p_cache_1).ap_hash.add(h as usize) };
                while { p_page = unsafe { *pp }; p_page } !=
                        core::ptr::null_mut() {
                    if unsafe { (*p_page).i_key } >= i_limit_1 {
                        {
                            let __p = &mut (*p_cache_1).n_page;
                            let __t = *__p;
                            *__p -= 1;
                            __t
                        };
                        unsafe { *pp = unsafe { (*p_page).p_next } };
                        if unsafe { (*p_page).p_lru_next } != core::ptr::null_mut()
                            {
                            pcache1_pin_page(p_page);
                        }
                        pcache1_free_page(p_page);
                    } else { pp = unsafe { &mut (*p_page).p_next }; }
                }
                if h == i_stop { break '__b3; }
                h = (h + 1 as u32) % (*p_cache_1).n_hash;
                break '__c3;
            }
        }
    }
...
extern "C" fn pcache1_remove_from_hash(p_page_1: *mut PgHdr1,
    free_flag_1: i32) -> () {
    let mut h: u32 = 0 as u32;
    let p_cache: *mut PCache1 = unsafe { (*p_page_1).p_cache };
    let mut pp: *mut *mut PgHdr1 = core::ptr::null_mut();
    { let _ = 0; };
    h = unsafe { (*p_page_1).i_key } % unsafe { (*p_cache).n_hash };
    {
        pp = unsafe { unsafe { (*p_cache).ap_hash.add(h as usize) } };
        '__b5: loop {
            if !(unsafe { *pp } != p_page_1) { break '__b5; }
    ...
    {
        let __p = unsafe { &mut (*p_cache).n_page };
        let __t = *__p;
        *__p -= 1;
        __t
    };
...
extern "C" fn pcache1_enforce_max_page(p_cache_1: &mut PCache1) -> () {
    ...
    { let _ = 0; };
    while unsafe { (*p_group).n_purgeable } > unsafe { (*p_group).n_max_page }
            &&
            unsafe {
                        (*{ p = unsafe { (*p_group).lru.p_lru_prev }; p }).is_anchor
                    } as i32 == 0 {
        { let _ = 0; };
        { let _ = 0; };
        pcache1_pin_page(p);
    ...
    if (*p_cache_1).n_page == 0 as u32 && !((*p_cache_1).p_bulk).is_null() {
        unsafe { sqlite3_free((*p_cache_1).p_bulk) };
        (*p_cache_1).p_bulk =
            {
                    (*p_cache_1).p_free = core::ptr::null_mut();
                    (*p_cache_1).p_free
                } as *mut ();
    }
}
extern "C" fn pcache1_destroy(p: *mut sqlite3_pcache) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    let p_group: *mut PGroup = unsafe { (*p_cache).p_group };
    { let _ = 0; };
    { let _ = 0; };
    if unsafe { (*p_cache).n_page } != 0 {
        pcache1_truncate_unsafe(unsafe { &mut *p_cache }, 0 as u32);
    }
    { let _ = 0; };
    unsafe { (*p_group).n_max_page -= unsafe { (*p_cache).n_max } };
    { let _ = 0; };
    unsafe { (*p_group).n_min_page -= unsafe { (*p_cache).n_min } };
    unsafe {
        (*p_group).mx_pinned =
            unsafe { (*p_group).n_max_page } + 10 as u32 -
                unsafe { (*p_group).n_min_page }
    };
    pcache1_enforce_max_page(unsafe { &mut *p_cache });
    { let _ = 0; };
    unsafe { sqlite3_free(unsafe { (*p_cache).p_bulk }) };
    unsafe { sqlite3_free(unsafe { (*p_cache).ap_hash } as *mut ()) };
    unsafe { sqlite3_free(p_cache as *mut ()) };
}
extern "C" fn pcache1_create(sz_page_1: i32, sz_extra_1: i32,
    b_purgeable_1: i32) -> *mut sqlite3_pcache {
    unsafe {
        let mut p_cache: *mut PCache1 = core::ptr::null_mut();
        ...
        { let _ = 0; };
        sz =
            (core::mem::size_of::<PCache1>() as u64 +
                    core::mem::size_of::<PGroup>() as u64 *
                        pcache1_g.separate_cache as u64) as i64;
        p_cache = unsafe { sqlite3_malloc_zero(sz as u64) } as *mut PCache1;
        if !(p_cache).is_null() {
            if pcache1_g.separate_cache != 0 {
                p_group =
                    unsafe { &raw mut *p_cache.offset(1 as isize) } as
                        *mut PGroup;
                unsafe { (*p_group).mx_pinned = 10 as u32 };
            } else { p_group = &mut pcache1_g.grp; }
            { let _ = 0; };
            if unsafe { (*p_group).lru.is_anchor } as i32 == 0 {
                unsafe { (*p_group).lru.is_anchor = 1 as u16 };
                unsafe {
                    (*p_group).lru.p_lru_prev =
                        {
                            unsafe {
                                (*p_group).lru.p_lru_next = unsafe { &mut (*p_group).lru }
                            };
                            unsafe { (*p_group).lru.p_lru_next }
                        }
                };
            }
            unsafe { (*p_cache).p_group = p_group };
            unsafe { (*p_cache).sz_page = sz_page_1 };
            unsafe { (*p_cache).sz_extra = sz_extra_1 };
            unsafe {
                (*p_cache).sz_alloc =
                    ((sz_page_1 + sz_extra_1) as u64 +
                            (core::mem::size_of::<PgHdr1>() as u64 + 7 as u64 &
                                !7 as u64)) as i32
            };
            unsafe {
                (*p_cache).b_purgeable =
                    if b_purgeable_1 != 0 { 1 } else { 0 }
            };
            pcache1_resize_hash(unsafe { &mut *p_cache });
            if b_purgeable_1 != 0 {
                unsafe { (*p_cache).n_min = 10 as u32 };
                unsafe {
                    (*p_group).n_min_page += unsafe { (*p_cache).n_min }
                };
                unsafe {
                    (*p_group).mx_pinned =
                        unsafe { (*p_group).n_max_page } + 10 as u32 -
                ...
                unsafe {
                    (*p_cache).pn_purgeable =
                        unsafe { &mut (*p_group).n_purgeable }
                };
            } else {
                unsafe {
                    (*p_cache).pn_purgeable =
                        unsafe { &mut (*p_cache).n_purgeable_dummy }
                };
            }
            { let _ = 0; };
            if unsafe { (*p_cache).n_hash } == 0 as u32 {
                pcache1_destroy(p_cache as *mut sqlite3_pcache);
                p_cache = core::ptr::null_mut();
            }
        }
        return p_cache as *mut sqlite3_pcache;
    }
}
extern "C" fn pcache1_cachesize(p: *mut sqlite3_pcache, n_max_1: i32) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    ...
    if unsafe { (*p_cache).b_purgeable } != 0 {
        ...
        n = n_max_1 as u32;
        if n >
                2147418112 as u32 - unsafe { (*p_group).n_max_page } +
                    unsafe { (*p_cache).n_max } {
            n =
                2147418112 as u32 - unsafe { (*p_group).n_max_page } +
                    unsafe { (*p_cache).n_max };
        }
        unsafe { (*p_group).n_max_page += n - unsafe { (*p_cache).n_max } };
        unsafe {
            (*p_group).mx_pinned =
                unsafe { (*p_group).n_max_page } + 10 as u32 -
                    unsafe { (*p_group).n_min_page }
        };
        unsafe { (*p_cache).n_max = n };
        unsafe {
            (*p_cache).n90pct =
                unsafe { (*p_cache).n_max } * 9 as u32 / 10 as u32
        };
        pcache1_enforce_max_page(unsafe { &mut *p_cache });
        { let _ = 0; };
    }
}
extern "C" fn pcache1_pagecount(p: *mut sqlite3_pcache) -> i32 {
    let mut n: i32 = 0;
    let p_cache: *const PCache1 = p as *mut PCache1 as *const PCache1;
    { let _ = 0; };
    n = unsafe { (*p_cache).n_page } as i32;
    { let _ = 0; };
    return n;
}
extern "C" fn pcache1_under_memory_pressure(p_cache_1: &PCache1) -> i32 {
    unsafe {
        if pcache1_g.n_slot != 0 &&
                (*p_cache_1).sz_page + (*p_cache_1).sz_extra <=
                    pcache1_g.sz_slot {
            return unsafe {
                    std::sync::atomic::AtomicI32::from_ptr(&raw mut pcache1_g.b_under_pressure
                                as *mut i32).load(std::sync::atomic::Ordering::Relaxed)
                };
        } else { return unsafe { sqlite3_heap_nearly_full() }; }
    }
}
extern "C" fn pcache1_init_bulk(p_cache_1: &mut PCache1) -> i32 {
    unsafe {
        let mut sz_bulk: i64 = 0 as i64;
        let mut z_bulk: *mut i8 = core::ptr::null_mut();
        if pcache1_g.n_init_page == 0 { return 0; }
        if (*p_cache_1).n_max < 3 as u32 { return 0; }
        unsafe { sqlite3_begin_benign_malloc() };
        if pcache1_g.n_init_page > 0 {
            sz_bulk =
                (*p_cache_1).sz_alloc as i64 * pcache1_g.n_init_page as i64;
        } else { sz_bulk = -1024 as i64 * pcache1_g.n_init_page as i64; }
        if sz_bulk > (*p_cache_1).sz_alloc as i64 * (*p_cache_1).n_max as i64
            {
            sz_bulk =
                (*p_cache_1).sz_alloc as i64 * (*p_cache_1).n_max as i64;
        }
        if sz_bulk >= (*p_cache_1).sz_alloc as i64 {
            z_bulk =
                {
                        (*p_cache_1).p_bulk =
                            unsafe { sqlite3Malloc(sz_bulk as u64) };
                        (*p_cache_1).p_bulk
                    } as *mut i8;
            unsafe { sqlite3_end_benign_malloc() };
            if !(z_bulk).is_null() {
                let mut n_bulk: i32 =
                    unsafe { sqlite3_malloc_size(z_bulk as *const ()) } /
                        (*p_cache_1).sz_alloc;
                '__b7: loop {
                    '__c7: loop {
                        let p_x: *mut PgHdr1 =
                            unsafe {
                                    &raw mut *z_bulk.offset((*p_cache_1).sz_page as isize)
                                } as *mut PgHdr1;
                        unsafe { (*p_x).page.p_buf = z_bulk as *mut () };
                        unsafe {
                            (*p_x).page.p_extra =
                                unsafe {
                                        (p_x as
                                                *mut u8).add((core::mem::size_of::<PgHdr1>() as u64 +
                                                        7 as u64 & !7 as u64) as usize)
                                    } as *mut ()
                        };
                        { let _ = 0; };
                        unsafe { (*p_x).is_bulk_local = 1 as u16 };
                    ...
                    if !({ let __p = &mut n_bulk; *__p -= 1; *__p } != 0) {
                        break '__b7;
                    }
                }
            }
        }
        return ((*p_cache_1).p_free != core::ptr::null_mut()) as i32;
    }
}
extern "C" fn pcache1_alloc(n_byte_1: i32) -> *mut () {
    unsafe {
        ...
        { let _ = 0; };
        if n_byte_1 <= pcache1_g.sz_slot {
            unsafe { sqlite3_mutex_enter(pcache1_g.mutex) };
            p = pcache1_g.p_free as *mut PgHdr1 as *mut ();
            if !(p).is_null() {
                pcache1_g.p_free = unsafe { (*pcache1_g.p_free).p_next };
                {
                    let __p = &mut pcache1_g.n_free_slot;
                    let __t = *__p;
                    *__p -= 1;
                ...
                { let _ = 0; };
                unsafe { sqlite3_status_highwater(7, n_byte_1) };
                unsafe { sqlite3_status_up(1, 1) };
            }
            unsafe { sqlite3_mutex_leave(pcache1_g.mutex) };
        }
...
extern "C" fn pcache1_alloc_page(p_cache_1: *mut PCache1,
    ...
    let mut p: *mut PgHdr1 = core::ptr::null_mut();
    let mut p_pg: *mut () = core::ptr::null_mut();
    { let _ = 0; };
    if !(unsafe { (*p_cache_1).p_free }).is_null() ||
            unsafe { (*p_cache_1).n_page } == 0 as u32 &&
                pcache1_init_bulk(unsafe { &mut *p_cache_1 }) != 0 {
        { let _ = 0; };
        p = unsafe { (*p_cache_1).p_free };
        unsafe { (*p_cache_1).p_free = unsafe { (*p).p_next } };
        unsafe { (*p).p_next = core::ptr::null_mut() };
    } else {
        if benign_malloc_1 != 0 { unsafe { sqlite3_begin_benign_malloc() }; }
        p_pg = pcache1_alloc(unsafe { (*p_cache_1).sz_alloc });
        if benign_malloc_1 != 0 { unsafe { sqlite3_end_benign_malloc() }; }
        if p_pg == core::ptr::null_mut() { return core::ptr::null_mut(); }
        p =
            unsafe {
                    &raw mut *(p_pg as
                                    *mut u8).offset(unsafe { (*p_cache_1).sz_page } as isize)
                } as *mut PgHdr1;
        unsafe { (*p).page.p_buf = p_pg };
        unsafe {
            (*p).page.p_extra =
                unsafe {
                        (p as
        ...
        { let _ = 0; };
        unsafe { (*p).is_bulk_local = 0 as u16 };
        unsafe { (*p).is_anchor = 0 as u16 };
        unsafe { (*p).p_lru_prev = core::ptr::null_mut() };
    }
    {
        let __p = unsafe { &mut *unsafe { (*p_cache_1).pn_purgeable } };
        let __t = *__p;
        *__p += 1;
        __t
    };
...
extern "C" fn pcache1_fetch_stage2(p_cache_1: *mut PCache1, i_key_1: u32,
    ...
    { let _ = 0; };
    if create_flag_1 == 1 &&
            (n_pinned >= unsafe { (*p_group).mx_pinned } ||
                    n_pinned >= unsafe { (*p_cache_1).n90pct } ||
                pcache1_under_memory_pressure(unsafe { &*p_cache_1 }) != 0 &&
                    unsafe { (*p_cache_1).n_recyclable } < n_pinned) {
        return core::ptr::null_mut();
    }
    if unsafe { (*p_cache_1).n_page } >= unsafe { (*p_cache_1).n_hash } {
        pcache1_resize_hash(unsafe { &mut *p_cache_1 });
    }
    { let _ = 0; };
    if unsafe { (*p_cache_1).b_purgeable } != 0 &&
                (unsafe { (*unsafe { (*p_group).lru.p_lru_prev }).is_anchor }
                            == 0) as i32 != 0 &&
            (unsafe { (*p_cache_1).n_page } + 1 as u32 >=
                    unsafe { (*p_cache_1).n_max } ||
                pcache1_under_memory_pressure(unsafe { &*p_cache_1 }) != 0) {
        let mut p_other: *const PCache1 = core::ptr::null();
        p_page = unsafe { (*p_group).lru.p_lru_prev };
        { let _ = 0; };
        pcache1_remove_from_hash(p_page, 0);
        pcache1_pin_page(p_page);
        p_other = unsafe { (*p_page).p_cache };
        if unsafe { (*p_other).sz_alloc } != unsafe { (*p_cache_1).sz_alloc }
            {
            pcache1_free_page(p_page);
            p_page = core::ptr::null_mut();
        } else {
            unsafe {
                (*p_group).n_purgeable -=
                    (unsafe { (*p_other).b_purgeable } -
                            unsafe { (*p_cache_1).b_purgeable }) as u32
            };
        }
    }
    if (p_page).is_null() as i32 != 0 {
        p_page = pcache1_alloc_page(p_cache_1, (create_flag_1 == 1) as i32);
    }
    if !(p_page).is_null() {
        let h: u32 = i_key_1 % unsafe { (*p_cache_1).n_hash };
        {
            let __p = unsafe { &mut (*p_cache_1).n_page };
            let __t = *__p;
            *__p += 1;
            __t
        };
        unsafe { (*p_page).i_key = i_key_1 };
        unsafe {
            (*p_page).p_next =
                unsafe { *unsafe { (*p_cache_1).ap_hash.add(h as usize) } }
        };
        ...
        unsafe { *unsafe { (*p_cache_1).ap_hash.add(h as usize) } = p_page };
        if i_key_1 > unsafe { (*p_cache_1).i_max_key } {
            unsafe { (*p_cache_1).i_max_key = i_key_1 };
        }
    }
    return p_page;
}
extern "C" fn pcache1_fetch_no_mutex(p: *mut sqlite3_pcache, i_key_1: u32,
    create_flag_1: i32) -> *mut PgHdr1 {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    let mut p_page: *mut PgHdr1 = core::ptr::null_mut();
    p_page =
...
extern "C" fn pcache1_fetch(p: *mut sqlite3_pcache, i_key_1: u32,
    create_flag_1: i32) -> *mut sqlite3_pcache_page {
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    ...
    { let _ = 0; };
    {
        return pcache1_fetch_no_mutex(p, i_key_1, create_flag_1) as
                *mut sqlite3_pcache_page;
    }
}
extern "C" fn pcache1_unpin(p: *mut sqlite3_pcache,
    p_pg_1: *mut sqlite3_pcache_page, reuse_unlikely_1: i32) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    let p_page: *mut PgHdr1 = p_pg_1 as *mut PgHdr1;
    let p_group: *mut PGroup = unsafe { (*p_cache).p_group };
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    ...
    if reuse_unlikely_1 != 0 ||
        ...
        unsafe {
            (*{
                                let __v = unsafe { *pp_first };
                                unsafe { (*p_page).p_lru_next = __v };
                                __v
                            }).p_lru_prev = p_page
        };
        unsafe { *pp_first = p_page };
        {
            let __p = unsafe { &mut (*p_cache).n_recyclable };
            let __t = *__p;
            *__p += 1;
            __t
        };
...
extern "C" fn pcache1_rekey(p: *mut sqlite3_pcache,
    p_pg_1: *mut sqlite3_pcache_page, i_old_1: u32, i_new_1: u32) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    let p_page: *mut PgHdr1 = p_pg_1 as *mut PgHdr1;
    let mut pp: *mut *mut PgHdr1 = core::ptr::null_mut();
    let mut h_old: u32 = 0 as u32;
    let mut h_new: u32 = 0 as u32;
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    { let _ = 0; };
    ...
    unsafe {
        (*p_page).p_next =
            unsafe { *unsafe { (*p_cache).ap_hash.add(h_new as usize) } }
    };
    unsafe { *unsafe { (*p_cache).ap_hash.add(h_new as usize) } = p_page };
    if i_new_1 > unsafe { (*p_cache).i_max_key } {
        unsafe { (*p_cache).i_max_key = i_new_1 };
    }
    { let _ = 0; };
}
extern "C" fn pcache1_truncate(p: *mut sqlite3_pcache, i_limit_1: u32) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    { let _ = 0; };
    if i_limit_1 <= unsafe { (*p_cache).i_max_key } {
        pcache1_truncate_unsafe(unsafe { &mut *p_cache }, i_limit_1);
        unsafe { (*p_cache).i_max_key = i_limit_1 - 1 as u32 };
    }
    { let _ = 0; };
}
extern "C" fn pcache1_shrink(p: *mut sqlite3_pcache) -> () {
    let p_cache: *mut PCache1 = p as *mut PCache1;
    if unsafe { (*p_cache).b_purgeable } != 0 {
        let p_group: *mut PGroup = unsafe { (*p_cache).p_group };
        let mut saved_max_page: u32 = 0 as u32;
        { let _ = 0; };
        saved_max_page = unsafe { (*p_group).n_max_page };
        unsafe { (*p_group).n_max_page = 0 as u32 };
        pcache1_enforce_max_page(unsafe { &mut *p_cache });
        unsafe { (*p_group).n_max_page = saved_max_page };
        { let _ = 0; };
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_p_cache_set_default() -> () {
    unsafe {
        unsafe {
            sqlite3_config(18,
                &raw const default_methods as *const sqlite3_pcache_methods2)
        };
    }
}
#[unsafe(no_mangle)]
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_pcache1_mutex() -> *mut sqlite3_mutex {
    unsafe { return pcache1_g.mutex; }
}
static mut default_methods: sqlite3_pcache_methods2 =
