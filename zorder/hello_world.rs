...
extern "C" fn zorder_func(context: *mut sqlite3_context, argc: i32,
    argv: *mut *mut sqlite3_value) -> () {
    let mut z: sqlite3_int64 = 0 as sqlite3_int64;
    let mut x: [i64; 24] = [0; 24];
    let mut i: i32 = 0;
    let mut j: i32 = 0;
    z = 0 as sqlite3_int64;
    if argc < 2 || argc > 24 {
        unsafe {
            sqlite3_result_error(context,
                c"zorder() needs between 2 and 24 arguments4".as_ptr() as
    ...
    }
    {
        ...
        '__b1: loop {
            ...
            '__c1: loop {
                j = i % argc;
                z |= (x[j as usize] & 1 as sqlite3_int64) << i;
                x[j as usize] >>= 1 as sqlite3_int64;
                break '__c1;
            }
            { let __p = &mut i; let __t = *__p; *__p += 1; __t };
        }
    }
    unsafe { sqlite3_result_int64(context, z) };
    {
        i = 0;
        '__b2: loop {
            ...
            '__c2: loop {
                if x[i as usize] != 0 {
                    let mut z: *mut i8 =
                        unsafe {
                            sqlite3_mprintf(c"the %r argument to zorder() (%lld) is too large for a 64-bit %d-dimensional Morton code".as_ptr()
                    ...
                    unsafe {
                        sqlite3_result_error(context, z as *const i8, -1)
                    };
...
extern "C" fn unzorder_func(context: *mut sqlite3_context, argc: i32,
    argv: *mut *mut sqlite3_value) -> () {
    let mut z: sqlite3_int64 = 0 as sqlite3_int64;
    ...
    let mut j: i32 = 0;
    let mut k: i32 = 0;
    z = unsafe { sqlite3_value_int64(unsafe { *argv.offset(0 as isize) }) };
    n = unsafe { sqlite3_value_int64(unsafe { *argv.offset(1 as isize) }) };
    if n < 2 as i64 || n > 24 as i64 {
        unsafe {
            sqlite3_result_error(context,
                c"N argument to unzorder(Z,N,K) should be between 2 and 24".as_ptr()
                        as *mut i8 as *const i8, -1)
        };
        return;
    }
    i = unsafe { sqlite3_value_int64(unsafe { *argv.offset(2 as isize) }) };
    if i < 0 as i64 || i >= n {
        unsafe {
            sqlite3_result_error(context,
                c"K argument to unzorder(Z,N,K) should be between 0 and N-1".as_ptr()
                        as *mut i8 as *const i8, -1)
        };
        return;
    }
    x = 0 as sqlite3_int64;
    {
        { k = 0; j = i as i32 };
        '__b3: loop {
            if !(j < 63) { break '__b3; }
            '__c3: loop {
                x |= (z >> j & 1 as sqlite3_int64) << k;
                break '__c3;
            }
            {
                j += n as i32;
    ...
    unsafe { sqlite3_result_int64(context, x) };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_zorder_init(db: *mut sqlite3,
    ...
    if rc == 0 {
        rc =
            unsafe {
                sqlite3_create_function(db,
                    c"unzorder".as_ptr() as *mut i8 as *const i8, 3, 1,
                    core::ptr::null_mut(), Some(unzorder_func), None, None)
            };
    }
    return rc;
}
