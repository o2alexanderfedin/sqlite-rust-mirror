...
struct sqlite3_backup {
    p_dest_db: *mut sqlite3,
    z_dest_db: *mut i8,
    p_dest: *mut Btree,
    i_dest_schema: u32,
    b_dest_locked: i32,
    i_next: Pgno,
    p_src_db: *mut sqlite3,
    p_src: *mut Btree,
    rc: i32,
    n_remaining: Pgno,
    n_pagecount: Pgno,
    is_attached: i32,
    p_next: *mut sqlite3_backup,
}
extern "C" fn find_btree(p_error_db_1: *mut sqlite3, p_db_1: *mut sqlite3,
    z_db_1: *const i8) -> *mut Btree {
    let mut i: i32 = 0;
    { let _ = 0; };
    i = unsafe { sqlite3_find_db_name(p_db_1, z_db_1) };
    if i == 1 {
        let mut s_parse: Parse = unsafe { core::mem::zeroed() };
        let mut rc: i32 = 0;
        unsafe { sqlite3_parse_object_init(&mut s_parse, p_db_1) };
        if unsafe { sqlite3_open_temp_database(&mut s_parse) } != 0 {
            unsafe {
                sqlite3_error_with_msg(p_error_db_1, s_parse.rc,
                    c"%s".as_ptr() as *mut i8 as *const i8, s_parse.z_err_msg)
            };
            rc = 1;
        }
        unsafe {
            sqlite3_db_free(p_error_db_1, s_parse.z_err_msg as *mut ())
        };
    ...
    if i < 0 {
        unsafe {
            sqlite3_error_with_msg(p_error_db_1, 1,
    ...
    if unsafe { sqlite3_btree_txn_state(p) } != 0 {
        unsafe {
            sqlite3_error_with_msg(db, 1,
                c"destination database is in use".as_ptr() as *mut i8 as
                    *const i8)
        };
        return 1;
    }
...
pub extern "C" fn sqlite3_backup_init(p_dest_db_1: *mut sqlite3,
    ...
    -> *mut sqlite3_backup {
    let mut p: *mut sqlite3_backup = core::ptr::null_mut();
    ...
    if p_src_db_1 == p_dest_db_1 {
        unsafe {
            sqlite3_error_with_msg(p_dest_db_1, 1,
                c"source and destination must be distinct".as_ptr() as *mut i8
                    as *const i8)
        };
        p = core::ptr::null_mut();
    } else {
        ...
        p =
            unsafe {
                    sqlite3_malloc_zero(core::mem::size_of::<sqlite3_backup>()
                                    as u64 + n_dest as u64 + 1 as u64)
        ...
        if (p).is_null() as i32 != 0 {
            ...
            unsafe {
                memcpy(unsafe { (*p).z_dest_db } as *mut (),
                    z_dest_db_1 as *const (), n_dest as u64)
            };
        }
    }
    if !(p).is_null() {
        ...
        unsafe {
            (*p).p_src = find_btree(p_dest_db_1, p_src_db_1, z_src_db_1)
        };
        unsafe { (*p).p_dest_db = p_dest_db_1 };
        unsafe { (*p).p_src_db = p_src_db_1 };
        unsafe { (*p).i_next = 1 as Pgno };
        unsafe { (*p).is_attached = 0 };
    ...
    if !(p).is_null() {
        {
            let __p = unsafe { &mut (*unsafe { (*p).p_src }).n_backup };
            let __t = *__p;
            *__p += 1;
            __t
        };
    }
    unsafe { sqlite3_mutex_leave(unsafe { (*p_dest_db_1).mutex }) };
...
extern "C" fn set_dest_pgsz(p_dest_1: *mut Btree, p_src_1: *mut Btree)
    -> i32 {
    return unsafe {
            sqlite3_btree_set_page_size(p_dest_1,
                unsafe { sqlite3_btree_get_page_size(p_src_1) }, 0, 0)
        };
}
extern "C" fn backup_one_page(p: &sqlite3_backup, i_src_pg_1: Pgno,
    z_src_data_1: *const u8, b_update_1: i32) -> i32 {
    unsafe {
        let p_dest_pager: *mut Pager =
            unsafe { sqlite3_btree_pager((*p).p_dest) };
        let n_src_pgsz: i32 =
            unsafe { sqlite3_btree_get_page_size((*p).p_src) } as i32;
        let n_dest_pgsz: i32 =
            unsafe { sqlite3_btree_get_page_size((*p).p_dest) };
        let n_copy: i32 =
            if (n_src_pgsz as i32) < n_dest_pgsz {
                    n_src_pgsz
                } else { n_dest_pgsz } as i32;
        let i_end: i64 = (i_src_pg_1 as i64 * n_src_pgsz as i64) as i64;
        let mut rc: i32 = 0;
        ...
        { let _ = 0; };
        {
            ...
            '__b0: loop {
                if !(rc == 0 && i_off < i_end) { break '__b0; }
                '__c0: loop {
                    let mut p_dest_pg: *mut DbPage = core::ptr::null_mut();
                    let i_dest: Pgno =
                        (i_off / n_dest_pgsz as i64) as Pgno + 1 as Pgno;
                    if i_dest ==
                            (sqlite3_pending_byte as u32 /
                                        unsafe { (*unsafe { (*(*p).p_dest).p_bt }).page_size } +
                                    1 as u32) as Pgno {
                        break '__c0;
                    }
                    if 0 ==
                                {
                                    rc =
                                        unsafe {
                                            sqlite3_pager_get(p_dest_pager, i_dest, &mut p_dest_pg, 0)
                                        };
                                    rc
                                } &&
                            0 == { rc = unsafe { sqlite3_pager_write(p_dest_pg) }; rc }
                        {
                        let z_in: *const u8 =
                            unsafe {
                                &*z_src_data_1.offset((i_off % n_src_pgsz as i64) as isize)
                            };
                        let z_dest_data: *mut u8 =
                            unsafe { sqlite3_pager_get_data(p_dest_pg) } as *mut u8;
                        let z_out: *mut u8 =
                            unsafe {
                        ...
                        unsafe {
                            *(unsafe { sqlite3_pager_get_extra(p_dest_pg) } as
                                            *mut u8).offset(0 as isize) = 0 as u8
                        };
                        if i_off == 0 as i64 && b_update_1 == 0 {
                            unsafe {
                                sqlite3_put4byte(unsafe { &mut *z_out.offset(28 as isize) },
                                    unsafe { sqlite3_btree_last_page((*p).p_src) })
                            };
                        }
                    }
                    unsafe { sqlite3_pager_unref(p_dest_pg) };
                    break '__c0;
                }
...
extern "C" fn attach_backup_object(p: *mut sqlite3_backup) -> () {
    let mut pp: *mut *mut sqlite3_backup = core::ptr::null_mut();
    { let _ = 0; };
    pp =
        unsafe {
            sqlite3_pager_backup_ptr(unsafe {
                    sqlite3_btree_pager(unsafe { (*p).p_src })
                })
        };
    unsafe { (*p).p_next = unsafe { *pp } };
    unsafe { *pp = p };
    unsafe { (*p).is_attached = 1 };
}
extern "C" fn backup_truncate_file(p_file_1: *mut sqlite3_file, i_size_1: i64)
    -> i32 {
    let mut i_current: i64 = 0 as i64;
    let mut rc: i32 =
        unsafe { sqlite3_os_file_size(p_file_1, &mut i_current) };
    if rc == 0 && i_current > i_size_1 {
        rc = unsafe { sqlite3_os_truncate(p_file_1, i_size_1) };
    }
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_step(p: *mut sqlite3_backup, n_page_1: i32)
    -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        ...
        unsafe { sqlite3_btree_enter(unsafe { (*p).p_src }) };
        if !(unsafe { (*p).p_dest_db }).is_null() {
            unsafe {
                sqlite3_mutex_enter(unsafe {
                        (*unsafe { (*p).p_dest_db }).mutex
                    })
            };
        }
        rc = unsafe { (*p).rc };
        if (is_fatal_error(rc) == 0) as i32 != 0 {
            ...
            if !(unsafe { (*p).p_dest_db }).is_null() &&
                    unsafe {
                                (*unsafe { (*unsafe { (*p).p_src }).p_bt }).in_transaction
                            } as i32 == 2 {
                rc = 5;
            } else { rc = 0; }
            if rc == 0 &&
                    0 ==
                        unsafe { sqlite3_btree_txn_state(unsafe { (*p).p_src }) } {
                rc =
                    unsafe {
                        sqlite3_btree_begin_trans(unsafe { (*p).p_src }, 0,
                            core::ptr::null_mut())
                    };
                b_close_trans = 1;
            }
            if { p_dest = unsafe { (*p).p_dest }; p_dest } ==
                    core::ptr::null_mut() {
                p_dest =
                    find_btree(unsafe { (*p).p_dest_db },
                        unsafe { (*p).p_dest_db },
                        unsafe { (*p).z_dest_db } as *const i8);
            }
            if p_dest == core::ptr::null_mut() {
                rc = 1;
            } else { p_dest_pager = unsafe { sqlite3_btree_pager(p_dest) }; }
            if unsafe { (*p).b_dest_locked } == 0 && rc == 0 &&
                    set_dest_pgsz(p_dest, unsafe { (*p).p_src }) == 7 {
                rc = 7;
            }
            if 0 == rc && unsafe { (*p).b_dest_locked } == 0 &&
                    0 ==
                        {
                            rc =
                                unsafe {
                                    sqlite3_btree_begin_trans(p_dest, 2,
                                        unsafe { &raw mut (*p).i_dest_schema } as *mut i32)
                                };
                            rc
                        } {
                unsafe { (*p).b_dest_locked = 1 };
                unsafe { (*p).p_dest = p_dest };
            }
            if rc == 0 {
                pgsz_src =
                    unsafe {
                        sqlite3_btree_get_page_size(unsafe { (*p).p_src })
                    };
                pgsz_dest =
                    unsafe {
                        sqlite3_btree_get_page_size(unsafe { (*p).p_dest })
                    };
                dest_mode =
                    unsafe {
                        sqlite3_pager_get_journal_mode(unsafe {
                                sqlite3_btree_pager(unsafe { (*p).p_dest })
                            })
                    };
                if (dest_mode == 5 ||
                            unsafe { sqlite3_pager_is_memdb(p_dest_pager) } != 0) &&
                        pgsz_src != pgsz_dest {
                    rc = 8;
                }
            }
            ...
            { let _ = 0; };
            {
                ii = 0;
                '__b1: loop {
                    if !((n_page_1 < 0 || ii < n_page_1) &&
                                        unsafe { (*p).i_next } <= n_src_page as Pgno &&
                                    (rc == 0) as i32 != 0) {
                        break '__b1;
                    }
                    '__c1: loop {
                        let i_src_pg: Pgno = unsafe { (*p).i_next } as Pgno;
                        if i_src_pg as Pgno !=
                                (sqlite3_pending_byte as u32 /
                                            unsafe {
                            ...
                            if rc == 0 {
                                rc =
                                    backup_one_page(unsafe { &*p }, i_src_pg,
                                        unsafe { sqlite3_pager_get_data(p_src_pg) } as *const u8,
                                        0);
                                unsafe { sqlite3_pager_unref(p_src_pg) };
                            }
                        }
                        {
                            let __p = unsafe { &mut (*p).i_next };
                            let __t = *__p;
                            *__p += 1;
                            __t
                        };
            ...
            if rc == 0 {
                unsafe { (*p).n_pagecount = n_src_page as Pgno };
                unsafe {
                    (*p).n_remaining =
                        (n_src_page + 1) as Pgno - unsafe { (*p).i_next }
                };
                if unsafe { (*p).i_next } > n_src_page as Pgno {
                    rc = 101;
                } else if (unsafe { (*p).is_attached } == 0) as i32 != 0 {
            ...
            if rc == 101 {
                if n_src_page == 0 {
                    rc =
                        unsafe { sqlite3_btree_new_db(unsafe { (*p).p_dest }) };
                    n_src_page = 1;
                }
                if rc == 0 || rc == 101 {
                    rc =
                        unsafe {
                            sqlite3_btree_update_meta(unsafe { (*p).p_dest }, 1,
                                unsafe { (*p).i_dest_schema } + 1 as u32)
                        };
                }
                if rc == 0 {
                    if !(unsafe { (*p).p_dest_db }).is_null() {
                        unsafe {
                            sqlite3_reset_all_schemas_of_connection(unsafe {
                                    (*p).p_dest_db
                                })
                        };
                    }
                    if dest_mode == 5 {
                        rc =
                            unsafe {
                                sqlite3_btree_set_version(unsafe { (*p).p_dest }, 2)
                            };
                    }
                }
                if rc == 0 {
                    ...
                    if pgsz_src < pgsz_dest {
                        let ratio: i32 = pgsz_dest / pgsz_src;
                        n_dest_truncate = (n_src_page + ratio - 1) / ratio;
                        if n_dest_truncate ==
                                (sqlite3_pending_byte as u32 /
                                                unsafe {
                                                    (*unsafe { (*unsafe { (*p).p_dest }).p_bt }).page_size
                                                } + 1 as u32) as Pgno as i32 {
                            {
                                let __p = &mut n_dest_truncate;
                                let __t = *__p;
                                *__p -= 1;
                                __t
                            };
                    ...
                    if pgsz_src < pgsz_dest {
                        ...
                        let p_file: *mut sqlite3_file =
                            unsafe { sqlite3_pager_file(p_dest_pager) };
                        let mut i_pg: Pgno = 0 as Pgno;
                        ...
                        };
                        {
                            ...
                            '__b2: loop {
                                if !(rc == 0 && i_pg <= n_dst_page as Pgno) { break '__b2; }
                                '__c2: loop {
                                    if i_pg !=
                                            (sqlite3_pending_byte as u32 /
                                                        unsafe {
                                        ...
                                        let mut p_pg: *mut DbPage = core::ptr::null_mut();
                                        rc =
                                            unsafe {
                                                sqlite3_pager_get(p_dest_pager, i_pg, &mut p_pg, 0)
                                            };
                                        if rc == 0 {
                                            rc = unsafe { sqlite3_pager_write(p_pg) };
                                            unsafe { sqlite3_pager_unref(p_pg) };
                                        }
                                    }
                                    break '__c2;
                                }
                        ...
                        if rc == 0 {
                            rc =
                                unsafe {
                                    sqlite3_pager_commit_phase_one(p_dest_pager,
                                        core::ptr::null(), 1)
                                };
                        }
                        i_end =
                            ...
                            } else { i_size };
                        {
                            ...
                            '__b3: loop {
                                if !(rc == 0 && i_off < i_end) { break '__b3; }
                                '__c3: loop {
                                    let mut p_src_pg_1: *mut PgHdr = core::ptr::null_mut();
                                    let i_src_pg_1: Pgno =
                                        (i_off / pgsz_src as i64 + 1 as i64) as Pgno as Pgno;
                                    rc =
                                        unsafe {
                                            sqlite3_pager_get(p_src_pager, i_src_pg_1,
                                                &raw mut p_src_pg_1 as *mut *mut DbPage, 0)
                                        };
                                    if rc == 0 {
                                        let z_data: *const u8 =
                                            unsafe { sqlite3_pager_get_data(p_src_pg_1 as *mut DbPage) }
                                                as *const u8;
                                        rc =
                                            unsafe {
                                                sqlite3_os_write(p_file, z_data as *const (), pgsz_src,
                                                    i_off)
                                            };
                                    }
                                    unsafe { sqlite3_pager_unref(p_src_pg_1 as *mut DbPage) };
                                    break '__c3;
                                }
                                i_off += pgsz_src as i64;
                            }
                        }
                        if rc == 0 { rc = backup_truncate_file(p_file, i_size); }
                        if rc == 0 {
                            rc =
                                unsafe {
                                    sqlite3_pager_sync(p_dest_pager, core::ptr::null())
                                };
                        }
                    } else {
                        unsafe {
                            sqlite3_pager_truncate_image(p_dest_pager,
                                n_dest_truncate as Pgno)
                        };
                        rc =
                            unsafe {
                                sqlite3_pager_commit_phase_one(p_dest_pager,
                            ...
                            0 ==
                                {
                                    rc =
                                        unsafe {
                                            sqlite3_btree_commit_phase_two(unsafe { (*p).p_dest }, 0)
                                        };
                                    rc
                                } {
                        rc = 101;
            ...
            if b_close_trans != 0 {
                unsafe {
                    sqlite3_btree_commit_phase_one(unsafe { (*p).p_src },
                        core::ptr::null())
                };
                unsafe {
                    sqlite3_btree_commit_phase_two(unsafe { (*p).p_src }, 0)
                };
                { let _ = 0; };
            }
            if rc == 10 | 12 << 8 { rc = 7; }
        ...
        unsafe {
            sqlite3_mutex_leave(unsafe { (*unsafe { (*p).p_src_db }).mutex })
        };
        return rc;
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_finish(p: *mut sqlite3_backup) -> i32 {
    let mut pp: *mut *mut sqlite3_backup = core::ptr::null_mut();
    let mut p_src_db: *mut sqlite3 = core::ptr::null_mut();
    let mut rc: i32 = 0;
    if p == core::ptr::null_mut() { return 0; }
    p_src_db = unsafe { (*p).p_src_db };
    unsafe { sqlite3_mutex_enter(unsafe { (*p_src_db).mutex }) };
    unsafe { sqlite3_btree_enter(unsafe { (*p).p_src }) };
    if !(unsafe { (*p).p_dest_db }).is_null() {
        unsafe {
            sqlite3_mutex_enter(unsafe { (*unsafe { (*p).p_dest_db }).mutex })
        };
    }
    if !(unsafe { (*p).p_dest_db }).is_null() {
        {
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_remaining(p: &sqlite3_backup) -> i32 {
    return (*p).n_remaining as i32;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_pagecount(p: &sqlite3_backup) -> i32 {
...
pub extern "C" fn sqlite3_btree_copy_file(p_to_1: *mut Btree,
    ...
    '__b5: loop {
        '__c5: loop {
            ...
            p_fd =
                unsafe {
                    sqlite3_pager_file(unsafe { sqlite3_btree_pager(p_to_1) })
                };
            if !(unsafe { (*p_fd).p_methods }).is_null() {
                let mut n_byte: i64 =
                    unsafe { sqlite3_btree_get_page_size(p_from_1) } as i64 *
                        unsafe { sqlite3_btree_last_page(p_from_1) } as i64;
                rc =
                    unsafe {
                        sqlite3_os_file_control(p_fd, 11,
                            &raw mut n_byte as *mut ())
                    };
                if rc == 12 { rc = 0; }
                if rc != 0 { break '__b5; }
            }
            unsafe {
                memset(&raw mut b as *mut (), 0,
                    core::mem::size_of::<sqlite3_backup>() as u64)
            };
            b.p_src_db = unsafe { (*p_from_1).db };
            b.p_src = p_from_1;
            b.p_dest = p_to_1;
            b.i_next = 1 as Pgno;
            ...
            if rc == 0 {
                unsafe {
                    (*unsafe { (*p_to_1).p_bt }).bts_flags &= !2 as u16
                };
            } else {
                unsafe {
                    sqlite3_pager_clear_cache(unsafe {
                            sqlite3_btree_pager(b.p_dest)
                        })
                };
            }
            { let _ = 0; };
...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_restart(p_backup_1: *mut sqlite3_backup)
    -> () {
    let mut p: *mut sqlite3_backup = core::ptr::null_mut();
    {
        p = p_backup_1;
        '__b6: loop {
            if !(!(p).is_null()) { break '__b6; }
            '__c6: loop {
                { let _ = 0; };
                unsafe { (*p).i_next = 1 as Pgno };
                break '__c6;
...
extern "C" fn backup_update(mut p: *mut sqlite3_backup, i_page_1: Pgno,
    a_data_1: *const u8) -> () {
    { let _ = 0; };
    '__b7: loop {
        '__c7: loop {
            ...
            if (is_fatal_error(unsafe { (*p).rc }) == 0) as i32 != 0 &&
                ...
                rc = backup_one_page(unsafe { &*p }, i_page_1, a_data_1, 1);
                unsafe {
                    sqlite3_mutex_leave(unsafe {
                            (*unsafe { (*p).p_dest_db }).mutex
                        })
                ...
                { let _ = 0; };
                if rc != 0 { unsafe { (*p).rc = rc }; }
            }
            break '__c7;
        }
        if !({ p = unsafe { (*p).p_next }; p } != core::ptr::null_mut()) {
            break '__b7;
        }
    }
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_backup_update(p_backup_1: *mut sqlite3_backup,
    i_page_1: Pgno, a_data_1: *const u8) -> () {
    if !(p_backup_1).is_null() {
