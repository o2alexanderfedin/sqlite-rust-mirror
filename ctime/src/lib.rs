...
static mut sqlite3az_compile_opt: [*const i8; 40] =
    [c"ATOMIC_INTRINSICS=1".as_ptr() as *const i8,
            c"COMPILER=clang-22.1.4".as_ptr() as *const i8,
            c"DEFAULT_AUTOVACUUM".as_ptr() as *const i8,
            c"DEFAULT_CACHE_SIZE=-2000".as_ptr() as *const i8,
            c"DEFAULT_FILE_FORMAT=4".as_ptr() as *const i8,
            c"DEFAULT_JOURNAL_SIZE_LIMIT=-1".as_ptr() as *const i8,
            c"DEFAULT_MMAP_SIZE=0".as_ptr() as *const i8,
            c"DEFAULT_PAGE_SIZE=4096".as_ptr() as *const i8,
            c"DEFAULT_PCACHE_INITSZ=20".as_ptr() as *const i8,
            c"DEFAULT_RECURSIVE_TRIGGERS".as_ptr() as *const i8,
            c"DEFAULT_SECTOR_SIZE=4096".as_ptr() as *const i8,
            c"DEFAULT_SYNCHRONOUS=2".as_ptr() as *const i8,
            c"DEFAULT_WAL_AUTOCHECKPOINT=1000".as_ptr() as *const i8,
            c"DEFAULT_WAL_SYNCHRONOUS=2".as_ptr() as *const i8,
            c"DEFAULT_WORKER_THREADS=0".as_ptr() as *const i8,
            c"DIRECT_OVERFLOW_READ".as_ptr() as *const i8,
            c"ENABLE_MATH_FUNCTIONS".as_ptr() as *const i8,
            c"ENABLE_PERCENTILE".as_ptr() as *const i8,
            c"HAVE_ISNAN".as_ptr() as *const i8,
            c"MALLOC_SOFT_LIMIT=1024".as_ptr() as *const i8,
            c"MAX_ATTACHED=10".as_ptr() as *const i8,
            c"MAX_COLUMN=2000".as_ptr() as *const i8,
            c"MAX_COMPOUND_SELECT=500".as_ptr() as *const i8,
            c"MAX_DEFAULT_PAGE_SIZE=8192".as_ptr() as *const i8,
            c"MAX_EXPR_DEPTH=1000".as_ptr() as *const i8,
            c"MAX_FUNCTION_ARG=1000".as_ptr() as *const i8,
            c"MAX_LENGTH=1000000000".as_ptr() as *const i8,
            c"MAX_LIKE_PATTERN_LENGTH=50000".as_ptr() as *const i8,
            c"MAX_MMAP_SIZE=0x7fff0000".as_ptr() as *const i8,
            c"MAX_PAGE_COUNT=0xfffffffe".as_ptr() as *const i8,
            c"MAX_PAGE_SIZE=65536".as_ptr() as *const i8,
            c"MAX_SQL_LENGTH=1000000000".as_ptr() as *const i8,
            c"MAX_TRIGGER_DEPTH=1000".as_ptr() as *const i8,
            c"MAX_VARIABLE_NUMBER=32766".as_ptr() as *const i8,
            c"MAX_VDBE_OP=250000000".as_ptr() as *const i8,
            c"MAX_WORKER_THREADS=8".as_ptr() as *const i8,
            c"MUTEX_PTHREADS".as_ptr() as *const i8,
            c"SYSTEM_MALLOC".as_ptr() as *const i8,
            c"TEMP_STORE=1".as_ptr() as *const i8,
            c"THREADSAFE=1".as_ptr() as *const i8];

#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_compile_options(pn_opt: &mut i32)
    -> *mut *const i8 {
    unsafe {
        *pn_opt =
            (core::mem::size_of::<[*const i8; 40]>() as u64 /
                    core::mem::size_of::<*const i8>() as u64) as i32;
        return &raw const sqlite3az_compile_opt[0 as usize] as *mut *const i8;
    }
}
