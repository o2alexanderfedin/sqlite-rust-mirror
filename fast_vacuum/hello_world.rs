...
extern "C" fn vacuum_finalize(p_stmt_1: *mut sqlite3_stmt) -> () {
    unsafe {
...
extern "C" fn exec_sql(db: *mut sqlite3, z_sql_1: *const i8) -> () {
    unsafe {
        ...
        if 0 !=
                unsafe {
                    sqlite3_prepare(db, z_sql_1, -1, &mut p_stmt,
                        core::ptr::null_mut())
                } {
            unsafe {
                fprintf(__stderrp,
...
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    unsafe {
        ...
        rc = unsafe { sqlite3_open(z_db_to_vacuum, &mut db) };
        if rc != 0 {
            unsafe {
                fprintf(__stderrp,
                    c"%s: %s\n".as_ptr() as *mut i8 as *const i8,
                    z_db_to_vacuum, unsafe { sqlite3_errstr(rc) })
            };
            return Err(1);
        }
        unsafe {
            sqlite3_randomness(core::mem::size_of::<sqlite3_uint64>() as i32,
                &raw mut r as *mut ())
        };
        z_temp_db =
            unsafe {
                sqlite3_mprintf(c"%s-vacuum-%016llx".as_ptr() as *mut i8 as
        ...
        exec_exec_sql(db,
            c"SELECT \'CREATE INDEX vacuum_db.\' || substr(sql,14)  FROM sqlite_schema WHERE sql LIKE \'CREATE INDEX %\'".as_ptr()
                    as *mut i8 as *const i8);
        exec_exec_sql(db,
        ...
        unsafe { rename(z_db_to_vacuum, z_backup_db as *const i8) };
        unsafe {
            printf(c"-- rename \"%s\" to \"%s\"\n".as_ptr() as *mut i8 as
