    ...
    unsafe {
        ...
        if install != 0 {
            rc =
                unsafe {
                    sqlite3_config(5,
                        &raw mut memfault.m as *mut Sqlite3MemMethods)
                };
            { let _ = 0; };
            if rc == 0 {
                let mut m: Sqlite3MemMethods = memfault.m;
                m.x_malloc = Some(faultsim_malloc);
                m.x_realloc = Some(faultsim_realloc);
                rc =
                    unsafe {
                        sqlite3_config(4, &raw mut m as *mut Sqlite3MemMethods)
                    };
            }
            unsafe {
            ...
            unsafe {
                sqlite3_config(4, &raw mut m2 as *mut Sqlite3MemMethods)
            };
            unsafe {
                sqlite3_config(5, &raw mut m2 as *mut Sqlite3MemMethods)
            };
            { let _ = 0; };
            rc =
                unsafe {
