...
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    unsafe {
        ...
        let mut m_debug: u32 = 0 as u32;
        {
            i = 1;
            '__b0: loop {
                if !(i < argc) { break '__b0; }
                '__c0: loop {
                    let mut z_filename: *const i8 =
                        unsafe { *argv.offset(i as isize) } as *const i8;
                    if unsafe { *z_filename.offset(0 as isize) } as i32 ==
                            '-' as i32 {
                        if unsafe { *z_filename.offset(1 as isize) } as i32 ==
                                '-' as i32 {
                            {
                        ...
                        if unsafe {
                                    strcmp(z_filename,
                                        c"-show-errors".as_ptr() as *mut i8 as *const i8)
                                } == 0 {
                            m_debug |= 4 as u32;
                            unsafe { ossfuzz_set_debug_flags(m_debug) };
                        } else if unsafe {
                                    strcmp(z_filename,
                                        c"-show-max-delay".as_ptr() as *mut i8 as *const i8)
                                } == 0 {
                            m_debug |= 2 as u32;
                            unsafe { ossfuzz_set_debug_flags(m_debug) };
                        } else if unsafe {
                                    strcmp(z_filename,
                                        c"-sql-trace".as_ptr() as *mut i8 as *const i8)
                                } == 0 {
                            m_debug |= 1 as u32;
                            unsafe { ossfuzz_set_debug_flags(m_debug) };
                        } else {
                            ...
                            unsafe {
                                printf(c"should be one of: --show-errors --show-max-delay --sql-trace\n".as_ptr()
                    ...
                    in_ =
                        unsafe {
                            fopen(z_filename, c"rb".as_ptr() as *mut i8 as *const i8)
                        };
                    ...
                    sz = unsafe { ftell(in_) } as u64;
                    unsafe { rewind(in_) };
                    z_buf = unsafe { realloc(z_buf as *mut (), sz) } as *mut u8;
                    if z_buf == core::ptr::null_mut() {
                        eprintln!("cannot malloc() for {} bytes", sz as i32 as i32);
                        unsafe { exit(1) };
                    }
                    if unsafe { fread(z_buf as *mut (), sz, 1 as u64, in_) } !=
                            1 as u64 {
                        unsafe {
                            fprintf(__stderrp,
                                c"cannot read %d bytes from \"%s\"\n".as_ptr() as *mut i8 as
                                    *const i8, sz as i32, z_filename)
                        };
                        { let __p = &mut n_err; let __t = *__p; *__p += 1; __t };
                    } else {
                        unsafe {
                        ...
                        unsafe { fflush(__stdoutp) };
                        {
                            let _ =
                                unsafe {
                                    llvm_fuzzer_test_one_input(z_buf as *const u8, sz)
                                };
                        };
                        if m_debug != 0 {
                            unsafe {
                                printf(c"%s: ".as_ptr() as *mut i8 as *const i8, z_filename)
                            };
                        }
                        unsafe { printf(c"ok\n".as_ptr() as *mut i8 as *const i8) };
                    }
...
extern "C" {
    ...
    -> i32;
    fn llvm_fuzzer_test_one_input(data: *const u8, size: u64)
    -> i32;
    fn ossfuzz_set_debug_flags(_: u32)
    -> ();
