...
#[unsafe(no_mangle)]
pub extern "C" fn log_est_from_integer(mut x: sqlite3_uint64) -> LogEst {
    unsafe {
        ...
        if x < 8 as u64 {
            ...
            while x < 8 as u64 {
                y -= 10 as LogEst;
                x <<= 1 as sqlite3_uint64;
            }
        } else {
            while x > 255 as u64 {
                y += 40 as LogEst;
                x >>= 4 as sqlite3_uint64;
            }
            while x > 15 as u64 {
                y += 10 as LogEst;
                x >>= 1 as sqlite3_uint64;
        ...
        return (a_1[(x & 7 as sqlite3_uint64) as usize] as i32 + y as i32 -
                    10) as LogEst;
    }
}
extern "C" fn log_est_to_int(mut x: LogEst) -> sqlite3_uint64 {
    let mut n: sqlite3_uint64 = 0 as sqlite3_uint64;
    if (x as i32) < 10 { return 1 as sqlite3_uint64; }
    n = (x as i32 % 10) as sqlite3_uint64;
    x /= 10 as LogEst;
    if n >= 5 as u64 {
        n -= 2 as sqlite3_uint64;
    } else if n >= 1 as u64 { n -= 1 as sqlite3_uint64; }
    if x as i32 > 60 {
        return ((4294967295u32 as sqlite3_uint64) << 32) +
                4294967295u32 as sqlite3_uint64;
    }
    if x as i32 >= 3 { return (n + 8 as sqlite3_uint64) << x as i32 - 3; }
    return n + 8 as sqlite3_uint64 >> 3 - x as i32;
}
extern "C" fn log_est_from_double(mut x: f64) -> LogEst {
    let mut a: sqlite3_uint64 = 0 as sqlite3_uint64;
    let mut e: LogEst = 0 as LogEst;
    if !(core::mem::size_of::<f64>() as u64 == 8 as u64 &&
                            core::mem::size_of::<sqlite3_uint64>() as u64 == 8 as u64)
                    as i32 as i64 != 0 {
        unsafe {
            __assert_rtn(c"logEstFromDouble".as_ptr() as *const i8,
                c"logest.c".as_ptr() as *mut i8 as *const i8, 85,
                c"sizeof(x)==8 && sizeof(a)==8".as_ptr() as *mut i8 as
                    *const i8)
        }
    } else { { let _ = 0; } };
    if x <= 0.0 { return -32768 as LogEst; }
    if x < 0.01 { return -log_est_from_double(1.0 / x) as LogEst; }
    if x < 1.0 {
        return (log_est_from_double(100.0 * x) as i32 - 66) as LogEst;
    }
    if x < 1024.0 {
        return (log_est_from_integer((1024.0 * x) as sqlite3_uint64) as i32 -
                    100) as LogEst;
    }
    if x <= 2000000000.0 { return log_est_from_integer(x as sqlite3_uint64); }
    unsafe {
        memcpy(&raw mut a as *mut (), &raw mut x as *const (), 8 as u64)
    };
    e = ((a >> 52) - 1022 as sqlite3_uint64) as LogEst;
...
pub extern "C" fn is_float(mut z: *const i8) -> i32 {
    ...
    while { c = unsafe { *z.offset(0 as isize) } as i8; c } as i32 >=
                                    '0' as i32 && c as i32 <= '9' as i32 ||
                            c as i32 == '.' as i32 || c as i32 == 'E' as i32 ||
                    c as i32 == 'e' as i32 || c as i32 == '+' as i32 ||
            c as i32 == '-' as i32 {
        {
            let __p = &mut z;
            let __t = *__p;
...
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    let mut a: [i16; 100] = [0; 100];
    {
        ...
        '__b5: loop {
            ...
            '__c5: loop {
                ...
                if unsafe { strcmp(z, c"+".as_ptr() as *mut i8 as *const i8) }
                        == 0 {
                    if n >= 2 {
                        a[(n - 2) as usize] =
                            log_est_add(a[(n - 2) as usize], a[(n - 1) as usize]);
                    ...
                    if n >= 2 {
                        a[(n - 2) as usize] =
                            log_est_multiply(a[(n - 2) as usize], a[(n - 1) as usize]);
                        { let __p = &mut n; let __t = *__p; *__p -= 1; __t };
                    }
                } else if unsafe {
                            strcmp(z, c"dup".as_ptr() as *mut i8 as *const i8)
                        } == 0 {
                    if n > 0 {
                        a[n as usize] = a[(n - 1) as usize];
                        { let __p = &mut n; let __t = *__p; *__p += 1; __t };
                    }
                } else if unsafe {
                            strcmp(z, c"log".as_ptr() as *mut i8 as *const i8)
                        } == 0 {
                    if n > 0 {
                        a[(n - 1) as usize] =
                            (log_est_from_integer(a[(n - 1) as usize] as sqlite3_uint64)
                                        as i32 - 33) as LogEst;
                    }
                } else if unsafe {
                            strcmp(z, c"nlogn".as_ptr() as *mut i8 as *const i8)
                        } == 0 {
                    if n > 0 {
                        a[(n - 1) as usize] +=
                            (log_est_from_integer(a[(n - 1) as usize] as sqlite3_uint64)
                            ...
                            strcmp(z, c"inv".as_ptr() as *mut i8 as *const i8)
                        } == 0 {
                    if n > 0 {
                        a[(n - 1) as usize] = -a[(n - 1) as usize] as LogEst;
                    }
                } else if unsafe { *z.offset(0 as isize) } as i32 ==
                        '^' as i32 {
                    a[{ let __p = &mut n; let __t = *__p; *__p += 1; __t } as
                                usize] =
                        unsafe { atoi(unsafe { z.offset(1 as isize) }) } as LogEst;
                } else if is_integer(z) != 0 {
                    a[{ let __p = &mut n; let __t = *__p; *__p += 1; __t } as
                                usize] =
                        log_est_from_integer(unsafe { atoll(z) } as sqlite3_uint64);
                } else if is_float(z) != 0 &&
                        unsafe { *z.offset(0 as isize) } as i32 != '-' as i32 {
                    a[{ let __p = &mut n; let __t = *__p; *__p += 1; __t } as
                                usize] = log_est_from_double(unsafe { atof(z) });
                } else {
    ...
    }
    {
        ...
        '__b6: loop {
            if !(i >= 0) { break '__b6; }
            '__c6: loop {
                if (a[i as usize] as i32) < -40 {
                    unsafe {
                        printf(c"%5d (%f)\n".as_ptr() as *mut i8 as *const i8,
                            a[i as usize] as i32,
                            1.0 / log_est_to_int(-a[i as usize] as LogEst) as f64)
                    };
                } else if (a[i as usize] as i32) < 10 {
                    unsafe {
                        printf(c"%5d (%f)\n".as_ptr() as *mut i8 as *const i8,
                            a[i as usize] as i32,
                            log_est_to_int((a[i as usize] as i32 + 100) as LogEst) as
                                    f64 / 1024.0)
                    };
                } else if a[i as usize] as i32 > 100 {
                    unsafe {
                        printf(c"%5d (%lld)\n".as_ptr() as *mut i8 as *const i8,
                            a[i as usize] as i32, log_est_to_int(a[i as usize]))
                    };
                } else {
                    let x: sqlite3_uint64 =
                        log_est_to_int((a[i as usize] as i32 + 100) as LogEst) *
                                100 as sqlite3_uint64 / 1024 as sqlite3_uint64;
                    unsafe {
                        printf(c"%5d (%lld.%02lld)\n".as_ptr() as *mut i8 as
                                *const i8, a[i as usize] as i32, x / 100 as sqlite3_uint64,
                            x % 100 as sqlite3_uint64)
                    };
                }
                break '__c6;
            }
