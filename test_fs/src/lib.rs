...
extern "C" fn fs_connect(db: *mut Sqlite3, p_aux_1: *mut (), argc: i32,
    argv: *const *const i8, pp_vtab_1: *mut *mut Sqlite3Vtab,
    pz_err_1: *mut *mut i8) -> i32 {
    let mut p_vtab: *mut FsVtab = core::ptr::null_mut();
    ...
    n_byte =
        (core::mem::size_of::<FsVtab>() as u64 +
                            unsafe { strlen(z_tbl) } as i32 as u64 + 1 as u64 +
                    unsafe { strlen(z_db) } as i32 as u64 + 1 as u64) as i32;
    p_vtab = unsafe { sqlite3_malloc_zero(n_byte as u64) } as *mut FsVtab;
...
extern "C" fn fs_open(p_v_tab_1: *mut Sqlite3Vtab,
    ...
    p_cur =
        unsafe {
                sqlite3_malloc_zero(core::mem::size_of::<FsCursor>() as u64)
            } as *mut FsCursor;
    unsafe { *pp_cursor_1 = unsafe { &mut (*p_cur).base } };
    return 0;
}
...
extern "C" fn fs_filter(p_vtab_cursor_1: *mut Sqlite3VtabCursor,
    ...
    unsafe {
        let mut rc: i32 = 0;
        let p_cur: *mut FsCursor = p_vtab_cursor_1 as *mut FsCursor;
        let p: *const FsVtab =
            unsafe { (*p_vtab_cursor_1).p_vtab } as *mut FsVtab as
                *const FsVtab;
        { let _ = 0; };
        if idx_num_1 == 1 {
            let z_stmt: *mut i8 =
...
extern "C" fn fs_column(cur: *mut Sqlite3VtabCursor, ctx: *mut Sqlite3Context,
    ...
    if i == 0 {
        ...
        if sbuf.st_size >= unsafe { (*p_cur).n_alloc } as i64 {
            let mut n_new: Sqlite3Int64 = sbuf.st_size * 2 as OffT;
            let mut z_new: *mut i8 = core::ptr::null_mut();
            if n_new < 1024 as i64 { n_new = 1024 as Sqlite3Int64; }
            z_new =
                unsafe {
        ...
        unsafe { close(fd) };
        if n as OffT != sbuf.st_size { return 1; }
    ...
    unsafe {
        ...
        unsafe {
            sqlite3_create_module(db, c"fs".as_ptr() as *mut i8 as *const i8,
                &raw mut fs_module as *const Sqlite3Module, interp as *mut ())
        };
        unsafe {
        ...
        unsafe {
            sqlite3_create_module(db,
                ...
                &raw mut fstree_module as *const Sqlite3Module,
                core::ptr::null_mut())
        };
        return 0;
    }
