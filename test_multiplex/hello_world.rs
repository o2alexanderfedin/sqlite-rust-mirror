...
static mut g_multiplex: anon_s0 = unsafe { core::mem::zeroed() };
#[repr(C)]
#[derive(Copy, Clone)]
...
extern "C" fn multiplex_sub_filename(p_group_1: &mut multiplexGroup,
    ...
    if !((*p_group_1).z_name).is_null() &&
        ...
        z = unsafe { sqlite3_malloc64((n + 5) as sqlite3_uint64) } as *mut i8;
...
extern "C" fn multiplex_sub_open(p_group_1: *mut multiplexGroup,
    ...
    unsafe {
        ...
        let p_orig_vfs: *mut sqlite3_vfs = g_multiplex.p_orig_vfs;
...
extern "C" fn multiplex_sub_size(p_group_1: *mut multiplexGroup,
    i_chunk_1: i32, rc: *mut i32) -> sqlite3_int64 {
    let mut p_sub: *mut sqlite3_file = core::ptr::null_mut();
    let mut sz: sqlite3_int64 = 0 as sqlite3_int64;
    if unsafe { *rc } != 0 { return 0 as sqlite3_int64; }
    p_sub =
        multiplex_sub_open(p_group_1, i_chunk_1, unsafe { &mut *rc },
            0 as *mut () as *mut i32, 0);
    if p_sub == core::ptr::null_mut() { return 0 as sqlite3_int64; }
    unsafe {
        *rc =
            unsafe {
                (unsafe {
...
extern "C" fn multiplex_sub_close(p_group_1: &multiplexGroup, i_chunk_1: i32,
    p_orig_vfs_1: *mut sqlite3_vfs) -> () {
    let p_sub_open: *mut sqlite3_file =
        unsafe { (*(*p_group_1).a_real.offset(i_chunk_1 as isize)).p };
...
extern "C" fn multiplex_open(p_vfs_1: *mut sqlite3_vfs, z_name_1: *const i8,
    ...
    unsafe {
        ...
        if rc == 0 {
            ...
            if rc == 0 {
                let mut sz64: sqlite3_int64 = 0 as sqlite3_int64;
                rc =
                    unsafe {
                        (unsafe {
                ...
                if rc == 0 && !(z_name_1).is_null() {
                    ...
                    if flags & 16384 != 0 {
                        ...
                        if rc == 0 && b_exists != 0 &&
                                        sz64 == sz64 & 4294901760u32 as sqlite3_int64 &&
                                    sz64 > 0 as i64 &&
...
extern "C" fn multiplex_delete(p_vfs_1: *mut sqlite3_vfs, z_name_1: *const i8,
    sync_dir_1: i32) -> i32 {
    unsafe {
        let mut rc: i32 = 0;
        let p_orig_vfs: *mut sqlite3_vfs = g_multiplex.p_orig_vfs;
        ...
        if rc == 0 {
            ...
            let mut z: *mut i8 = core::ptr::null_mut();
            z =
                unsafe { sqlite3_malloc64((n_name + 5) as sqlite3_uint64) } as
                    *mut i8;
...
extern "C" fn multiplex_access(a: *mut sqlite3_vfs, b: *const i8, c: i32,
    d: *mut i32) -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_full_pathname(a: *mut sqlite3_vfs, b: *const i8,
    c: i32, d: *mut i8) -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_dl_open(a: *mut sqlite3_vfs, b: *const i8)
    -> *mut () {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_dl_error(a: *mut sqlite3_vfs, b: i32, c: *mut i8)
    -> () {
    unsafe {
        unsafe {
            (unsafe {
...
extern "C" fn multiplex_dl_sym(a: *mut sqlite3_vfs, b: *mut (), c: *const i8)
    -> unsafe extern "C" fn() -> () {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_dl_close(a: *mut sqlite3_vfs, b: *mut ()) -> () {
    unsafe {
        unsafe {
            (unsafe {
...
extern "C" fn multiplex_randomness(a: *mut sqlite3_vfs, b: i32, c: *mut i8)
    -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_sleep(a: *mut sqlite3_vfs, b: i32) -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_current_time(a: *mut sqlite3_vfs, b: *mut f64)
    -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_get_last_error(a: *mut sqlite3_vfs, b: i32,
    c: *mut i8) -> i32 {
    unsafe {
...
extern "C" fn multiplex_current_time_int64(a: *mut sqlite3_vfs,
    b: *mut sqlite3_int64) -> i32 {
    unsafe {
        return unsafe {
                (unsafe {
...
extern "C" fn multiplex_close(p_conn_1: *mut sqlite3_file) -> i32 {
...
extern "C" fn multiplex_read(p_conn_1: *mut sqlite3_file,
    mut p_buf_1: *mut (), mut i_amt_1: i32, mut i_ofst_1: sqlite3_int64)
    -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let p_group: *mut multiplexGroup = unsafe { (*p).p_group };
    ...
    if (unsafe { (*p_group).b_enabled } == 0) as i32 != 0 {
        ...
        while i_amt_1 > 0 {
            let i: i32 =
                (i_ofst_1 / unsafe { (*p_group).sz_chunk } as sqlite3_int64)
                    as i32;
            let mut p_sub_open_1: *mut sqlite3_file = core::ptr::null_mut();
            p_sub_open_1 =
            ...
            if !(p_sub_open_1).is_null() {
                let mut extra: i32 =
                    (((i_ofst_1 %
                                            unsafe { (*p_group).sz_chunk } as sqlite3_int64) as i32 +
                ...
                p_buf_1 =
                    unsafe { (p_buf_1 as *mut i8).offset(i_amt_1 as isize) } as
                        *mut ();
                i_ofst_1 += i_amt_1 as sqlite3_int64;
                i_amt_1 = extra;
...
extern "C" fn multiplex_write(p_conn_1: *mut sqlite3_file,
    mut p_buf_1: *const (), mut i_amt_1: i32, mut i_ofst_1: sqlite3_int64)
    -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let p_group: *mut multiplexGroup = unsafe { (*p).p_group };
    ...
    if (unsafe { (*p_group).b_enabled } == 0) as i32 != 0 {
        ...
        while rc == 0 && i_amt_1 > 0 {
            let i: i32 =
                (i_ofst_1 / unsafe { (*p_group).sz_chunk } as sqlite3_int64)
                    as i32;
            let p_sub_open_1: *mut sqlite3_file =
                multiplex_sub_open(p_group, i, &mut rc,
                    0 as *mut () as *mut i32, 1);
            if !(p_sub_open_1).is_null() {
                let mut extra: i32 =
...
extern "C" fn multiplex_truncate(p_conn_1: *mut sqlite3_file,
    ...
    unsafe {
        ...
        if (unsafe { (*p_group).b_enabled } == 0) as i32 != 0 {
            ...
            let i_base_group: i32 =
                (size / unsafe { (*p_group).sz_chunk } as sqlite3_int64) as
                    i32;
            let mut p_sub_open_1: *mut sqlite3_file = core::ptr::null_mut();
            let p_orig_vfs: *mut sqlite3_vfs = g_multiplex.p_orig_vfs;
            {
                    ...
                    rc =
                        unsafe {
                            (unsafe {
                                ...
                                size % unsafe { (*p_group).sz_chunk } as sqlite3_int64)
                        };
                }
            }
            if rc != 0 { rc = 10 | 6 << 8; }
...
extern "C" fn multiplex_sync(p_conn_1: *mut sqlite3_file, flags: i32) -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
...
extern "C" fn multiplex_file_size(p_conn_1: *mut sqlite3_file,
    ...
    if (unsafe { (*p_group).b_enabled } == 0) as i32 != 0 {
        ...
        unsafe { *p_size_1 = 0 as sqlite3_int64 };
        {
            i = 0;
            '__b12: loop {
                if !(rc == 0) { break '__b12; }
                '__c12: loop {
                    ...
                    unsafe {
                        *p_size_1 =
                            i as sqlite3_int64 *
                                    unsafe { (*p_group).sz_chunk } as sqlite3_int64 + sz
                    };
                    break '__c12;
                }
                { let __p = &mut i; let __t = *__p; *__p += 1; __t };
...
extern "C" fn multiplex_lock(p_conn_1: *mut sqlite3_file, lock: i32) -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
    let p_sub_open: *mut sqlite3_file =
...
extern "C" fn multiplex_unlock(p_conn_1: *mut sqlite3_file, lock: i32)
    -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
...
extern "C" fn multiplex_check_reserved_lock(p_conn_1: *mut sqlite3_file,
...
extern "C" fn multiplex_file_control(p_conn_1: *mut sqlite3_file, op: i32,
    p_arg_1: *mut ()) -> i32 {
    unsafe {
        let p: *const multiplexConn =
            p_conn_1 as *mut multiplexConn as *const multiplexConn;
...
extern "C" fn multiplex_sector_size(p_conn_1: *mut sqlite3_file) -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
    let p_sub_open: *mut sqlite3_file =
...
extern "C" fn multiplex_device_characteristics(p_conn_1: *mut sqlite3_file)
    -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
...
extern "C" fn multiplex_shm_map(p_conn_1: *mut sqlite3_file, i_region_1: i32,
...
extern "C" fn multiplex_shm_lock(p_conn_1: *mut sqlite3_file, ofst: i32,
    n: i32, flags: i32) -> i32 {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
...
extern "C" fn multiplex_shm_barrier(p_conn_1: *mut sqlite3_file) -> () {
    let p: *const multiplexConn =
        p_conn_1 as *mut multiplexConn as *const multiplexConn;
    let mut rc: i32 = 0;
    let p_sub_open: *mut sqlite3_file =
...
extern "C" fn multiplex_shm_unmap(p_conn_1: *mut sqlite3_file,
...
extern "C" fn multiplex_control_func(context: *mut sqlite3_context, argc: i32,
...
pub extern "C" fn sqlite3_multiplex_initialize(z_orig_vfs_name: *const i8,
    ...
    unsafe {
        ...
        if !(p_orig_vfs !=
                                &raw mut g_multiplex.s_this_vfs as *mut sqlite3_vfs) as i32
                    as i64 != 0 {
            unsafe {
