...
pub extern "C" fn sqlite3_set_authorizer(db: *mut Sqlite3,
    x_auth_1:
        Option<unsafe extern "C" fn(*mut (), i32, *const i8, *const i8,
            *const i8, *const i8) -> i32>, p_arg_1: *mut ()) -> i32 {
...
extern "C" fn sqlite_auth_bad_return_code(p_parse_1: *mut Parse) -> () {
    unsafe {
        sqlite3_error_msg(p_parse_1,
            c"authorizer malfunction".as_ptr() as *mut i8 as *const i8)
    };
...
pub extern "C" fn sqlite3_auth_read_col(p_parse_1: *mut Parse,
    ...
    rc =
        unsafe {
            (unsafe {
                    (*db).x_auth.unwrap()
                })(unsafe { (*db).p_auth_arg }, 20, z_tab_1, z_col_1,
                z_db as *const i8, unsafe { (*p_parse_1).z_auth_context })
        };
    if rc == 1 {
        let mut z: *mut i8 =
            unsafe {
        ...
        if unsafe { (*db).n_db } > 2 || i_db_1 != 0 {
            z =
                unsafe {
                    sqlite3_mprintf(c"%s.%z".as_ptr() as *mut i8 as *const i8,
                        z_db, z)
                };
        }
        unsafe {
            sqlite3_error_msg(p_parse_1,
                c"access to %z is prohibited".as_ptr() as *mut i8 as
                    *const i8, z)
        };
        unsafe { (*p_parse_1).rc = 23 };
...
pub extern "C" fn sqlite3_auth_read(p_parse_1: *mut Parse,
    ...
    if (*p_expr_1).op as i32 == 78 {
        ...
        { let _ = 0; };
        {
            i_src = 0;
            '__b0: loop {
                if !(i_src < (*p_tab_list_1).n_src) { break '__b0; }
                '__c0: loop {
                    if (*p_expr_1).i_table ==
                            unsafe {
                                (*((*p_tab_list_1).a.as_ptr() as
                                                *mut SrcItem).offset(i_src as isize)).i_cursor
                            } {
                        p_tab =
                            unsafe {
                                (*((*p_tab_list_1).a.as_ptr() as
                                                *mut SrcItem).offset(i_src as isize)).p_s_tab
                            };
                        break '__b0;
                    }
                    break '__c0;
                }
                { let __p = &mut i_src; let __t = *__p; *__p += 1; __t };
            }
        }
    }
    i_col = (*p_expr_1).i_column as i32;
    ...
    } else if unsafe { (*p_tab).i_p_key } as i32 >= 0 {
        { let _ = 0; };
        z_col =
            unsafe {
                    (*unsafe {
    ...
    } else { z_col = c"ROWID".as_ptr() as *mut i8 as *const i8; }
    { let _ = 0; };
    if 2 ==
            sqlite3_auth_read_col(p_parse_1,
                unsafe { (*p_tab).z_name } as *const i8, z_col, i_db) {
...
extern "C" fn real_auth_check(p_parse_1: *mut Parse, code: i32,
    ...
    rc =
        unsafe {
            (unsafe {
                    (*db).x_auth.unwrap()
                })(unsafe { (*db).p_auth_arg }, code, z_arg1_1, z_arg2_1,
                z_arg3_1, unsafe { (*p_parse_1).z_auth_context })
        };
    if rc == 1 {
        unsafe {
            sqlite3_error_msg(p_parse_1,
                c"not authorized".as_ptr() as *mut i8 as *const i8)
        };
        unsafe { (*p_parse_1).rc = 23 };
    } else if rc != 0 && rc != 2 {
        rc = 1;
        sqlite_auth_bad_return_code(p_parse_1);
    }
    return rc;
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_auth_check(p_parse_1: *mut Parse, code: i32,
    z_arg1_1: *const i8, z_arg2_1: *const i8, z_arg3_1: *const i8) -> i32 {
    if unsafe { (*unsafe { (*p_parse_1).db }).x_auth.is_some() } &&
...
pub extern "C" fn sqlite3_auth_context_pop(p_context_1: &mut AuthContext)
    -> () {
    if !((*p_context_1).p_parse).is_null() {
        unsafe {
            (*(*p_context_1).p_parse).z_auth_context =
                (*p_context_1).z_auth_context
        };
