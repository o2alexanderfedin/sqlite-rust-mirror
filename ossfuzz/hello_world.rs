...
extern "C" fn progress_handler(p_client_data_1: *mut ()) -> i32 {
    ...
    let rc: i32 = (i_now >= unsafe { (*p).i_cutoff_time }) as i32;
...
pub extern "C" fn llvm_fuzzer_test_one_input(mut data: *const u8,
    ...
    unsafe {
        ...
        if size < 3 as u64 { return 0; }
        if unsafe { *data.offset(1 as isize) } as i32 == '\n' as i32 {
            u_selector = unsafe { *data.offset(0 as isize) } as u8;
            {
                let __n = 2;
                let __p = &mut data;
            ...
            size -= 2 as u64;
        } else { u_selector = 253 as u8; }
        if unsafe { sqlite3_initialize() } != 0 { return 0; }
        rc =
            unsafe {
                sqlite3_open_v2(c"fuzz.db".as_ptr() as *mut i8 as *const i8,
                    &mut cx.db, 2 | 4 | 128, core::ptr::null())
            };
        if rc != 0 { return 0; }
        cx.i_last_cb = time_of_day();
        cx.i_cutoff_time = cx.i_last_cb + 10000 as sqlite3_int64;
        unsafe {
        ...
        unsafe { sqlite3_hard_heap_limit64(20000000 as sqlite3_int64) };
        ...
        unsafe {
            sqlite3_exec(cx.db, z_sql as *const i8, Some(exec_handler),
                &raw mut cx as *mut (), &mut z_err_msg)
        };
        if m_debug & 4 as u32 != 0 as u32 && !(z_err_msg).is_null() {
            unsafe {
        ...
        unsafe { sqlite3_close(cx.db) };
        if m_debug & 2 as u32 != 0 {
            unsafe {
                printf(c"Progress callback count....... %d\n".as_ptr() as
                            *mut i8 as *const i8, cx.n_cb)
            };
            unsafe {
                printf(c"Max time between callbacks.... %d ms\n".as_ptr() as
                            *mut i8 as *const i8, cx.mx_interval as i32)
            };
        }
        return 0;
    }
