...
pub extern "C" fn rename_all_columns_of_table(db: *mut sqlite3,
    ...
    p_convert_1: *mut sqlite3_str, p_undo_1: *mut sqlite3_str) -> i32 {
    ...
    rc =
        unsafe {
            sqlite3_prepare_v2(db,
                c"SELECT name FROM pragma_table_info(?1);".as_ptr() as *mut i8
                    as *const i8, -1, &mut p_stmt, core::ptr::null_mut())
        };
    if rc != 0 { return rc; }
    unsafe { sqlite3_bind_text(p_stmt, 1, z_tab_1, -1, None) };
    while unsafe { sqlite3_step(p_stmt) } == 100 {
        let z_col: *const i8 =
            unsafe { sqlite3_column_text(p_stmt, 0) } as *const i8;
        { let __p = &mut cnt; let __t = *__p; *__p += 1; __t };
        unsafe {
            sqlite3_str_appendf(p_convert_1,
                c"ALTER TABLE \"%w\" RENAME COLUMN \"%w\" TO \"%w%d\";\n".as_ptr()
                        as *mut i8 as *const i8, z_tab_1, z_col, z_col_prefix_1,
                cnt)
        };
        if !(p_undo_1).is_null() {
            unsafe {
                sqlite3_str_appendf(p_undo_1,
                    c"ALTER TABLE \"%w\" RENAME COLUMN \"%w%d\" TO \"%w\";\n".as_ptr()
                            as *mut i8 as *const i8, z_tab_1, z_col_prefix_1, cnt,
                    z_col)
            };
        }
    }
...
pub extern "C" fn rename_all_tables(db: *mut sqlite3,
    ...
    rc =
        unsafe {
            sqlite3_prepare_v2(db,
                c"SELECT name FROM sqlite_schema WHERE type=\'table\' AND name NOT LIKE \'sqlite_%\';".as_ptr()
    ...
    while unsafe { sqlite3_step(p_stmt) } == 100 {
        let z_tab: *const i8 =
            unsafe { sqlite3_column_text(p_stmt, 0) } as *const i8;
        let mut z_new_tab: *mut i8 = core::ptr::null_mut();
        let mut z_prefix: [i8; 2] = [0; 2];
        z_prefix[0 as usize] = (cnt % 26 + 'a' as i32) as i8;
        z_prefix[1 as usize] = 0 as i8;
        z_new_tab =
            unsafe {
                sqlite3_mprintf(c"tx%d".as_ptr() as *mut i8 as *const i8,
                    { let __p = &mut cnt; *__p += 1; *__p })
            };
        if !(p_undo_1).is_null() {
            unsafe {
                sqlite3_str_appendf(p_undo_1,
                    c"ALTER TABLE \"%s\" RENAME TO \"%w\";\n".as_ptr() as
                            *mut i8 as *const i8, z_new_tab, z_tab)
            };
        }
        rename_all_columns_of_table(db, z_tab,
            &raw mut z_prefix[0 as usize] as *mut i8 as *const i8,
            p_convert_1, p_undo_1);
        unsafe {
            sqlite3_str_appendf(p_convert_1,
                c"ALTER TABLE \"%w\" RENAME TO \"%s\";\n".as_ptr() as *mut i8
                    as *const i8, z_tab, z_new_tab)
        };
        unsafe { sqlite3_free(z_new_tab as *mut ()) };
    }
    unsafe { sqlite3_finalize(p_stmt) };
    return 0;
}
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    unsafe {
        ...
        let mut rc: i32 = 0;
        let mut p_convert: *mut sqlite3_str = core::ptr::null_mut();
        let mut p_undo: *mut sqlite3_str = core::ptr::null_mut();
        let mut z_db_name: *const i8 = core::ptr::null();
        let mut z_sql1: *mut i8 = core::ptr::null_mut();
        ...
        z_db_name = unsafe { *argv.offset(1 as isize) };
        rc = unsafe { sqlite3_open(z_db_name as *const i8, &mut db) };
        if rc != 0 {
            eprintln!("sqlite3_open() returns {}", rc as i32);
            return Err(1);
        }
        p_convert = unsafe { sqlite3_str_new(db) };
        p_undo = unsafe { sqlite3_str_new(db) };
        rename_all_tables(db, p_convert, p_undo);
        z_sql1 = unsafe { sqlite3_str_finish(p_convert) };
        z_sql2 = unsafe { sqlite3_str_finish(p_undo) };
        unsafe { sqlite3_close(db) };
        unsafe { printf(c"BEGIN;\n".as_ptr() as *mut i8 as *const i8) };
        unsafe { printf(c"%s".as_ptr() as *mut i8 as *const i8, z_sql1) };
        unsafe { sqlite3_free(z_sql1 as *mut ()) };
        unsafe {
            printf(c".schema --indent\n".as_ptr() as *mut i8 as *const i8)
        };
        unsafe {
            printf(c"PRAGMA integrity_check;\n".as_ptr() as *mut i8 as
                    *const i8)
        };
        unsafe { printf(c"%s".as_ptr() as *mut i8 as *const i8, z_sql2) };
        unsafe { sqlite3_free(z_sql2 as *mut ()) };
        unsafe {
            printf(c"PRAGMA integrity_check;\n".as_ptr() as *mut i8 as
                    *const i8)
        };
        unsafe { printf(c"ROLLBACK;\n".as_ptr() as *mut i8 as *const i8) };
        return Ok(());
    }
}
#[unsafe(no_mangle)]
