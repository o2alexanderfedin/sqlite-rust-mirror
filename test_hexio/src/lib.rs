...
extern "C" fn hexio_write(client_data_1: *mut (), interp: *mut TclInterp,
    ...
    a_out =
        unsafe { sqlite3_malloc64((1 + n_in / 2) as Sqlite3Uint64) } as
            *mut u8;
        ...
        '__c3: loop {
            unsafe {
                *{
                            ...
                            *__p = unsafe { (*__p).offset(1) };
                            __t
                        } = (vu & 127 as SqliteUint64 | 128 as SqliteUint64) as u8
            };
            vu >>= 7 as SqliteUint64;
            break '__c3;
        }
...
extern "C" fn read_fts3varint(client_data_1: *mut (), interp: *mut TclInterp,
    ...
    n_val =
        get_fts3_varint(z_blob as *mut i8 as *const i8,
            unsafe { &mut *(&raw mut i_val as *mut Sqlite3Int64) });
    unsafe {
...
extern "C" fn make_fts3record(client_data_1: *mut (), interp: *mut TclInterp,
    ...
    let mut n_arg: i32 = 0;
    let mut a_out: *mut u8 = core::ptr::null_mut();
    let mut n_out: Sqlite3Int64 = 0 as Sqlite3Int64;
    ...
    }
    {
        ...
        '__b4: loop {
            ...
            '__c4: loop {
                let mut i_val: TclWideInt = 0 as TclWideInt;
                if 0 ==
                        unsafe {
                            Tcl_GetWideIntFromObj(core::ptr::null_mut(),
                                unsafe { *a_arg.offset(i as isize) }, &mut i_val)
                        } {
                    if n_out + 10 as Sqlite3Int64 > n_alloc {
                        let n_new: i32 =
                            if n_alloc != 0 {
                                    n_alloc * 2 as Sqlite3Int64
                                } else { 128 as Sqlite3Int64 } as i32;
                        let a_new: *mut u8 =
                        ...
                        a_out = a_new;
                        n_alloc = n_new as Sqlite3Int64;
                    }
                    n_out +=
                        put_fts3_varint(unsafe {
                                        &raw mut *a_out.offset(n_out as isize)
                                    } as *mut i8, i_val) as Sqlite3Int64;
                } else {
                    let mut n_val: i32 = 0;
                    let z_val: *const i8 =
                        unsafe {
                    ...
                    while n_out + n_val as Sqlite3Int64 > n_alloc {
                        let n_new_1: Sqlite3Int64 =
                            if n_alloc != 0 {
                                n_alloc * 2 as Sqlite3Int64
                            } else { 128 as Sqlite3Int64 };
                        let a_new_1: *mut u8 =
                            unsafe {
                                    sqlite3_realloc64(a_out as *mut (),
                                        n_new_1 as Sqlite3Uint64)
                                } as *mut u8;
