...
extern "C" fn memjrnl_read(p_jfd_1: *mut Sqlite3File, z_buf_1: *mut (),
    ...
    if i_amt_1 as SqliteInt64 + i_ofst_1 > unsafe { (*p).endpoint.i_offset } {
        return 10 | 2 << 8;
    }
    { let _ = 0; };
    if unsafe { (*p).readpoint.i_offset } != i_ofst_1 || i_ofst_1 == 0 as i64
        {
        let mut i_off: Sqlite3Int64 = 0 as Sqlite3Int64;
        {
            p_chunk = unsafe { (*p).p_first };
            '__b1: loop {
                if !(!(p_chunk).is_null() &&
                                i_off + unsafe { (*p).n_chunk_size } as Sqlite3Int64 <=
                                    i_ofst_1) {
                    break '__b1;
                }
                '__c1: loop {
    ...
    i_chunk_offset =
        (i_ofst_1 % unsafe { (*p).n_chunk_size } as SqliteInt64) as i32;
    '__b2: loop {
        '__c2: loop {
    ...
    unsafe {
        (*p).readpoint.i_offset =
            if !(p_chunk).is_null() {
                i_ofst_1 + i_amt_1 as SqliteInt64
            } else { 0 as SqliteInt64 }
    };
...
extern "C" fn memjrnl_truncate(p_jfd: *mut Sqlite3File, size: SqliteInt64)
    ...
    if size < unsafe { (*p).endpoint.i_offset } {
        ...
        unsafe { (*p).readpoint.i_offset = 0 as Sqlite3Int64 };
    }
    return 0;
}
...
extern "C" fn memjrnl_write(p_jfd_1: *mut Sqlite3File, z_buf_1: *const (),
    ...
    if unsafe { (*p).n_spill } > 0 &&
            i_amt_1 as SqliteInt64 + i_ofst_1 > unsafe { (*p).n_spill } as i64
        {
            ...
            while n_write > 0 {
                ...
                let i_chunk_offset: i32 =
                    (unsafe { (*p).endpoint.i_offset } %
                            unsafe { (*p).n_chunk_size } as Sqlite3Int64) as i32;
                let i_space: i32 =
                ...
                n_write -= i_space;
                unsafe { (*p).endpoint.i_offset += i_space as Sqlite3Int64 };
            }
        }
    }
    return 0;
...
pub extern "C" fn sqlite3_journal_open(p_vfs: *mut Sqlite3Vfs,
    ...
    unsafe {
        (*p_jfd).p_methods =
            &raw const mem_journal_methods as *const Sqlite3IoMethods
    };
