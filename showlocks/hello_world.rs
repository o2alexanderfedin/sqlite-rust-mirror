...
extern "C" fn show_locks_in_range(fd: i32, mut lwr: off_t, mut upr: off_t)
    -> i32 {
    unsafe {
        let mut cnt: i32 = 0;
        let mut x: flock = unsafe { core::mem::zeroed() };
        let mut a_pending: *mut lockRange_N9lockRange = core::ptr::null_mut();
        ...
        a_pending =
            unsafe {
                    malloc(core::mem::size_of::<lockRange_N9lockRange>() as u64)
                } as *mut lockRange_N9lockRange;
                        ...
                        a_pending =
                            unsafe {
                                    realloc(a_pending as *mut (),
                                        core::mem::size_of::<lockRange_N9lockRange>() as u64 *
                                            n_alloc as u64)
                                } as *mut lockRange_N9lockRange;
                    }
                    if a_pending == core::ptr::null_mut() {
                        unsafe {
                            fprintf(__stderrp,
                                c"unable to realloc for %d bytes\n".as_ptr() as *mut i8 as
                                    *const i8,
                                core::mem::size_of::<lockRange_N9lockRange>() as i32 *
                                    (n_pending + 2))
                        };
                        unsafe { exit(1) };
                    }
...
extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    ...
    unsafe {
        ...
        cnt = show_locks_in_range(fd, 0 as off_t, 2147483647 as off_t);
        if cnt == 0 {
