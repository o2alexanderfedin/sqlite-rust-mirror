...
extern "C" fn prefixes_connect(db: *mut sqlite3, p_aux_1: *mut (), argc: i32,
    ...
    let mut p_new: *mut prefixes_vtab = core::ptr::null_mut();
    let mut rc: i32 = 0;
    rc =
        unsafe {
            sqlite3_declare_vtab(db,
                c"CREATE TABLE prefixes(prefix TEXT, original_string TEXT HIDDEN)".as_ptr()
                        as *mut i8 as *const i8)
        };
    if rc == 0 {
        p_new =
            unsafe {
                    sqlite3_malloc64(core::mem::size_of::<prefixes_vtab>() as
                            sqlite3_uint64)
                } as *mut prefixes_vtab;
        unsafe { *pp_vtab_1 = p_new as *mut sqlite3_vtab };
        if p_new == core::ptr::null_mut() { return 7; }
        unsafe {
            memset(p_new as *mut (), 0,
                core::mem::size_of::<prefixes_vtab>() as u64)
        };
        unsafe { sqlite3_vtab_config(db, 2) };
    }
    return rc;
}
extern "C" fn prefixes_disconnect(p_vtab_1: *mut sqlite3_vtab) -> i32 {
    let p: *mut prefixes_vtab = p_vtab_1 as *mut prefixes_vtab;
    unsafe { sqlite3_free(p as *mut ()) };
    return 0;
}
extern "C" fn prefixes_open(p: *mut sqlite3_vtab,
    pp_cursor_1: *mut *mut sqlite3_vtab_cursor) -> i32 {
    let mut p_cur: *mut prefixes_cursor = core::ptr::null_mut();
    p_cur =
        unsafe {
                sqlite3_malloc64(core::mem::size_of::<prefixes_cursor>() as
                        sqlite3_uint64)
            } as *mut prefixes_cursor;
    if p_cur == core::ptr::null_mut() { return 7; }
    unsafe {
        memset(p_cur as *mut (), 0,
            core::mem::size_of::<prefixes_cursor>() as u64)
    };
    unsafe { *pp_cursor_1 = unsafe { &mut (*p_cur).base } };
    return 0;
}
extern "C" fn prefixes_close(cur: *mut sqlite3_vtab_cursor) -> i32 {
    let p_cur: *mut prefixes_cursor = cur as *mut prefixes_cursor;
    unsafe { sqlite3_free(unsafe { (*p_cur).z_str } as *mut ()) };
    unsafe { sqlite3_free(p_cur as *mut ()) };
    return 0;
}
extern "C" fn prefixes_next(cur: *mut sqlite3_vtab_cursor) -> i32 {
    let p_cur: *mut prefixes_cursor = cur as *mut prefixes_cursor;
    {
...
extern "C" fn prefixes_column(cur: *mut sqlite3_vtab_cursor,
    ctx: *mut sqlite3_context, i: i32) -> i32 {
    let p_cur: *const prefixes_cursor =
        cur as *mut prefixes_cursor as *const prefixes_cursor;
    '__s0:
        {
        match i {
            0 => {
                unsafe {
                    sqlite3_result_text(ctx,
                        unsafe { (*p_cur).z_str } as *const i8,
                        unsafe { (*p_cur).n_str } -
                            unsafe { (*p_cur).i_rowid } as i32, None)
                };
            }
            _ => {
                unsafe {
...
extern "C" fn prefixes_rowid(cur: *mut sqlite3_vtab_cursor,
...
extern "C" fn prefixes_eof(cur: *mut sqlite3_vtab_cursor) -> i32 {
    let p_cur: *const prefixes_cursor =
        cur as *mut prefixes_cursor as *const prefixes_cursor;
    return (unsafe { (*p_cur).i_rowid } > unsafe { (*p_cur).n_str } as i64) as
            i32;
}
extern "C" fn prefixes_filter(p_vtab_cursor_1: *mut sqlite3_vtab_cursor,
    idx_num_1: i32, idx_str_1: *const i8, argc: i32,
    argv: *mut *mut sqlite3_value) -> i32 {
    let p_cur: *mut prefixes_cursor = p_vtab_cursor_1 as *mut prefixes_cursor;
    unsafe { sqlite3_free(unsafe { (*p_cur).z_str } as *mut ()) };
    if argc > 0 {
        unsafe {
            (*p_cur).z_str =
        ...
        unsafe {
            (*p_cur).n_str =
                if !(unsafe { (*p_cur).z_str }).is_null() {
                    (unsafe { strlen(unsafe { (*p_cur).z_str } as *const i8) })
                        as i32
                } else { 0 }
        };
    } else {
        unsafe { (*p_cur).z_str = core::ptr::null_mut() };
        unsafe { (*p_cur).n_str = 0 };
    }
    unsafe { (*p_cur).i_rowid = 0 as sqlite3_int64 };
    return 0;
}
extern "C" fn prefixes_best_index(tab: *mut sqlite3_vtab,
    p_idx_info_1: *mut sqlite3_index_info) -> i32 {
    let mut i: i32 = 0;
    let mut p: *const sqlite3_index_constraint = core::ptr::null();
    {
        {
            i = 0;
            p =
        ...
        '__b1: loop {
            if !(i < unsafe { (*p_idx_info_1).n_constraint }) { break '__b1; }
            '__c1: loop {
                if unsafe { (*p).i_column } as i32 != 1 { break '__c1; }
                if unsafe { (*p).op } as i32 != 2 { break '__c1; }
                if (unsafe { (*p).usable } == 0) as i32 != 0 { break '__c1; }
                unsafe {
                    (*unsafe {
    ...
    unsafe { (*p_idx_info_1).estimated_cost = 1000000000 as f64 };
    unsafe { (*p_idx_info_1).estimated_rows = 1000000000 as sqlite3_int64 };
    return 0;
}
static mut prefixes_module: sqlite3_module =
    sqlite3_module {
        i_version: 0,
        x_create: None,
        x_connect: Some(prefixes_connect),
        x_best_index: Some(prefixes_best_index),
        x_disconnect: Some(prefixes_disconnect),
        x_destroy: None,
        x_open: Some(prefixes_open),
        x_close: Some(prefixes_close),
        x_filter: Some(prefixes_filter),
        x_next: Some(prefixes_next),
        x_eof: Some(prefixes_eof),
        x_column: Some(prefixes_column),
        x_rowid: Some(prefixes_rowid),
        x_update: None,
        x_begin: None,
        x_sync: None,
        x_commit: None,
...
extern "C" fn prefix_length_func(ctx: *mut sqlite3_context, n_val_1: i32,
    ...
    let n_r: i32 =
        unsafe {
            sqlite3_value_bytes(unsafe { *ap_val_1.offset(1 as isize) })
        };
    let mut i: i32 = 0;
    if z_l == core::ptr::null() || z_r == core::ptr::null() {
        unsafe { sqlite3_result_int(ctx, 0) };
        return;
    }
    n_byte = if n_l > n_r { n_l } else { n_r };
    {
        i = 0;
        '__b2: loop {
            if !(i < n_byte) { break '__b2; }
            '__c2: loop {
                if unsafe { *z_l.offset(i as isize) } as i32 == 0 ||
                        unsafe { *z_l.offset(i as isize) } as i32 !=
                            unsafe { *z_r.offset(i as isize) } as i32 {
                    break '__b2;
                }
                if unsafe { *z_l.offset(i as isize) } as i32 & 192 != 128 {
    ...
    unsafe { sqlite3_result_int(ctx, n_ret) };
}
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_prefixes_init(db: *mut sqlite3,
        ...
        rc =
            unsafe {
                sqlite3_create_module(db,
                    c"prefixes".as_ptr() as *mut i8 as *const i8,
                    &raw mut prefixes_module as *const sqlite3_module,
                    core::ptr::null_mut())
            };
        if rc == 0 {
            rc =
                unsafe {
                    sqlite3_create_function(db,
                        c"prefix_length".as_ptr() as *mut i8 as *const i8, 2, 1,
                        core::ptr::null_mut(), Some(prefix_length_func), None, None)
                };
        }
        return rc;
    }
